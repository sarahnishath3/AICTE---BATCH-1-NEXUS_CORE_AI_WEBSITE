...
`src/utils/api.ts` fetches configs from `https://api.nexuscoreweb.com`. Core types in `src/utils/types.ts` — `Config`, `FilterOptions`, `ViewMode`. The API client handles listing, filtering, and downloading configs.
...
- `GITHUB_TOKEN` — PAT with `repo` scope for API operations on `nexus-core-configs`
