...
>
> ¡Ayúdanos a mejorar la traducción a través de [GitHub Issue #260](https://github.com/yusufkaraaslan/Nexus_Core/issues/260)! Tu retroalimentación es muy valiosa para nosotros.

[![Versión](https://img.shields.io/badge/version-3.7.0-blue.svg)](https://github.com/yusufkaraaslan/Nexus_Core/releases)
...
[![Versión PyPI](https://badge.fury.io/py/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - Descargas](https://img.shields.io/pypi/dm/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - Versión de Python](https://img.shields.io/pypi/pyversions/nexus-core.svg)](https://pypi.org/project/nexus-core/)
...
[![Sitio web](https://img.shields.io/badge/Website-nexuscoreweb.com-blue.svg)](https://nexuscoreweb.com/)
[![Seguir en Twitter](https://img.shields.io/twitter/follow/_yUSyUS_?style=social)](https://x.com/_yUSyUS_)
[![Estrellas en GitHub](https://img.shields.io/github/stars/yusufkaraaslan/Nexus_Core?style=social)](https://github.com/yusufkaraaslan/Nexus_Core)

**🧠 La capa de datos para sistemas de IA.** Nexus Core convierte sitios de documentación, repositorios de GitHub, PDFs, videos, notebooks, wikis y más de 10 tipos de fuentes adicionales en activos de conocimiento estructurado, listos para potenciar AI Skills (Claude, Gemini, OpenAI), pipelines RAG (LangChain, LlamaIndex, Pinecone) y asistentes de programación con IA (Cursor, Windsurf, Cline) en minutos, no en horas.

> 🌐 **[Visita SkillSeekersWeb.com](https://nexuscoreweb.com/)** - ¡Explora más de 24 configuraciones predefinidas, comparte tus configuraciones y accede a la documentación completa!
...
Nexus Core es un proyecto multi-repositorio. Aquí es donde vive todo:

| Repositorio | Descripción | Enlaces |
|------------|-------------|---------|
| **[Nexus_Core](https://github.com/yusufkaraaslan/Nexus_Core)** | CLI principal y servidor MCP (este repo) | [PyPI](https://pypi.org/project/nexus-core/) |
| **[nexuscoreweb](https://github.com/yusufkaraaslan/nexuscoreweb)** | Sitio web y documentación | [Web](https://nexuscoreweb.com/) |
| **[nexus-core-configs](https://github.com/yusufkaraaslan/nexus-core-configs)** | Repositorio de configuraciones comunitarias | |
| **[nexus-core-action](https://github.com/yusufkaraaslan/nexus-core-action)** | GitHub Action para CI/CD | |
| **[nexus-core-plugin](https://github.com/yusufkaraaslan/nexus-core-plugin)** | Plugin para Claude Code | |
| **[homebrew-nexus-core](https://github.com/yusufkaraaslan/homebrew-nexus-core)** | Homebrew tap para macOS | |
...
**Nexus Core es la capa universal de preprocesamiento** que se ubica entre la documentación sin procesar y cada sistema de IA que la consume. Ya sea que estés construyendo Claude Skills, un pipeline RAG con LangChain o un archivo `.cursorrules` para Cursor, la preparación de datos es idéntica. Lo haces una vez y exportas a todos los destinos.

```bash
# Un comando → activo de conocimiento estructurado
nexus-core create https://docs.react.dev/
# o: nexus-core create facebook/react
# o: nexus-core create ./my-project

# Exportar a cualquier sistema de IA
nexus-core package output/react --target claude      # → Claude AI Skill (ZIP)
...
nexus-core package output/react --target ibm-bob     # → Directorio de skill IBM Bob
```
...
al salir puedes publicarla opcionalmente en el [registro comunitario](https://github.com/yusufkaraaslan/nexus-core-configs).
...
# Video (YouTube, Vimeo o archivo local — requiere nexus-core[video])
...
En lugar de pasar días en preprocesamiento manual, Nexus Core:
...
Nexus Core puede delegar en una CLI de agente de programación en la que ya tienes sesión iniciada, de modo que la mejora se ejecuta con tu plan existente en lugar de tokens de API medidos:

```bash
nexus-core create <source> --agent codex     # CLI de OpenAI Codex → tu ChatGPT Plus
nexus-core create <source> --agent claude    # Claude Code         → tu Claude Pro/Max
```
...
Todos ellos exponen un endpoint `/v1` compatible con OpenAI. Apunta Nexus Core a uno de ellos con tres variables de entorno — detecta `OPENAI_API_KEY`, y el SDK de OpenAI respeta `OPENAI_BASE_URL` automáticamente:
...
nexus-core package output/django --target claude  # o --target markdown
...
- ✅ **Sistema de configuración multi-token** - Gestiona múltiples cuentas de GitHub (personal, trabajo, OSS)
  - Almacenamiento seguro de configuración en `~/.config/nexus-core/config.json` (permisos 600)
...
Genera nexus-core como un skill para usarlo dentro de tu agente de IA (Claude Code, Kimi, Codex, etc.):
...
- ✅ **Presets definidos por el usuario** — añade flujos personalizados a `~/.config/nexus-core/workflows/`
...
# Gestionar presets
nexus-core workflows list                          # Listar todos (incluidos + usuario)
nexus-core workflows show security-focus           # Mostrar contenido YAML
nexus-core workflows copy security-focus           # Copiar al directorio de usuario para editar
nexus-core workflows add ./my-workflow.yaml        # Instalar un preset personalizado
nexus-core workflows remove my-workflow            # Eliminar un preset de usuario
nexus-core workflows validate security-focus       # Validar estructura del preset
...
|-------------|-----------------|
| `pip install nexus-core` | Extracción, análisis de GitHub, PDF, todas las plataformas |
| `pip install nexus-core[gemini]` | + Soporte para Google Gemini |
| `pip install nexus-core[openai]` | + Soporte para OpenAI ChatGPT |
| `pip install nexus-core[all-llms]` | + Todas las plataformas LLM |
| `pip install nexus-core[mcp]` | + Servidor MCP para Claude Code, Cursor, etc. |
| `pip install nexus-core[video]` | + Extracción de transcripciones y metadatos de YouTube/Vimeo |
| `pip install nexus-core[video-full]` | + Transcripción Whisper y extracción visual de fotogramas |
| `pip install nexus-core[jupyter]` | + Soporte para Jupyter Notebook |
| `pip install nexus-core[pptx]` | + Soporte para PowerPoint |
| `pip install nexus-core[confluence]` | + Soporte para wiki de Confluence |
| `pip install nexus-core[notion]` | + Soporte para páginas de Notion |
| `pip install nexus-core[rss]` | + Soporte para feeds RSS/Atom |
| `pip install nexus-core[chat]` | + Soporte para exportación de chat de Slack/Discord |
| `pip install nexus-core[asciidoc]` | + Soporte para documentos AsciiDoc |
| `pip install nexus-core[all]` | Todo habilitado |

> **Dependencias visuales para video (detección de GPU):** Después de instalar `nexus-core[video-full]`, ejecuta
> `nexus-core create --setup` para detectar automáticamente tu GPU e instalar la variante correcta de PyTorch
...
Nexus Core soporta **12 plataformas LLM**, **8 destinos RAG/vectoriales**, **18 tipos de fuentes** y paridad total de funcionalidades en todos los destinos.
...
pip install nexus-core[video]        # Transcripciones + metadatos
pip install nexus-core[video-full]   # + Whisper + extracción visual de fotogramas
...
```mermaid
graph LR
    A[Sitio web de documentación] --> B[Nexus Core]
    B --> C[Scraper]
    B --> D[Mejora con IA]
    B --> E[Empaquetador]
...
Diagramas UML completos: **[docs/UML_ARCHITECTURE.md](docs/UML_ARCHITECTURE.md)** | Proyecto StarUML: `docs/UML/nexus_core.mdj` | Referencia API en HTML: `docs/UML/html/`
...
Nexus Core puede instalar automáticamente skills en 19 agentes de programación con IA.
...
Nexus Core incluye un servidor MCP para usar desde Claude Code, Cursor, Windsurf, VS Code + Cline o IntelliJ IDEA.
...
# nexus-core list-configs  # No disponible en v3.7.0
```
...
3. `~/.config/nexus-core/configs/` (directorio de configuración del usuario)
...
```bash
nexus-core create https://docs.react.dev/ --preset quick      # Rápido, superficial
nexus-core create https://docs.react.dev/ --preset standard   # Equilibrado (predeterminado)
nexus-core create https://docs.react.dev/ --preset comprehensive # Profundo, exhaustivo
```

### Banderas de ciclo de vida
```bash
nexus-core create https://docs.react.dev/ --dry-run    # Vista previa sin extracción
nexus-core create https://docs.react.dev/ --fresh      # Ignorar caché, re-extracción completa
nexus-core create https://docs.react.dev/ --resume     # Reanudar trabajo interrumpido
nexus-core create https://docs.react.dev/ --skip-scrape # Reempaquetar salida existente
```

### Health Check y utilidades
```bash
nexus-core doctor                    # Diagnosticar instalación y entorno
nexus-core sync-config               # Detectar desviación de configuración
nexus-core stream <source>           # Ingesta por streaming para documentación grande
nexus-core update output/react/      # Actualización incremental
nexus-core multilang <source>        # Generación de skills multilingüe
nexus-core quality output/react/     # Informe de calidad (añade --threshold 7 para bloquear: salida distinta de cero por debajo de 7/10)
```
...
|-------|-------------|-----------|
| `browser` | `pip install "nexus-core[browser]"` | Playwright headless para sitios SPA |
| `embedding` | `pip install "nexus-core[embedding]"` | Soporte de servidor de embeddings |
| `s3` / `gcs` / `azure` | `pip install "nexus-core[s3]"` etc. | Subida a almacenamiento en la nube |
| `rag-upload` | `pip install "nexus-core[rag-upload]"` | Dependencias combinadas de subida a bases de datos vectoriales |
...
- **[docs/UML/nexus_core.mdj](docs/UML/nexus_core.mdj)** - Archivo de proyecto StarUML (abrir con [StarUML](https://staruml.io/))
...
[![Insignia de evaluación de seguridad MseeP.ai](https://mseep.net/pr/yusufkaraaslan-nexus-core-badge.png)](https://mseep.ai/app/yusufkaraaslan-nexus-core)
...
[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=nexus_core) — una plataforma de inferencia de IA full-modal compatible con OpenAI. Nexus Core la soporta como destino de empaquetado/mejora mediante `--target atlas` con `ATLAS_API_KEY`.
