...
Comprehensive context file for AI coding agents working with the Nexus Core project.

---

## Project Overview

**Nexus Core** (v3.6.0) is a Python CLI tool and MCP server that converts documentation sites, GitHub repositories, PDFs, videos, notebooks, wikis, and 17+ source types into structured AI-ready skills for 21+ LLM platforms and RAG pipelines.
...
| Resource | Link |
|----------|------|
| Website | https://nexuscoreweb.com/ |
| PyPI | https://pypi.org/project/nexus-core/ |
| GitHub | https://github.com/yusufkaraaslan/Nexus_Core |
| Configs | https://github.com/yusufkaraaslan/nexus-core-configs |
...
```
Nexus_Core/
├── src/nexus_core/           # Main package (src/ layout)
...
**Note:** `tests/conftest.py` checks that `nexus_core` is importable and calls `sys.exit(1)` if not.
...
# With coverage
pytest tests/ --cov=src/nexus_core --cov-report=term
```
...
# Type check (mypy)
mypy src/nexus_core --show-error-codes --pretty
```
...
nexus-core package output/react --target claude      # Claude AI (ZIP)
nexus-core package output/react --target gemini      # Gemini (tar.gz)
nexus-core package output/react --target openai      # OpenAI
nexus-core package output/react --target cursor      # .cursorrules

# Multi-source unified scraping
nexus-core create configs/react_unified.json
```
...
from nexus_core.cli.adaptors import ClaudeAdaptor
...
**Problem:** `ModuleNotFoundError: No module named 'nexus_core'`
...
| `src/nexus_core/cli/main.py` | Unified CLI entry point |
| `src/nexus_core/cli/source_detector.py` | Auto-detect source types |
| `src/nexus_core/cli/config_validator.py` | Config validation |
| `src/nexus_core/cli/unified_scraper.py` | Multi-source orchestrator |
| `src/nexus_core/cli/adaptors/base.py` | Adaptor interface |
| `src/nexus_core/cli/adaptors/__init__.py` | Adaptor registry |
| `src/nexus_core/mcp/server.py` | MCP server |
...
- **Version source:** `src/nexus_core/_version.py` reads from `pyproject.toml`
...
| Repository | Purpose |
|------------|---------|
| [Nexus_Core](https://github.com/yusufkaraaslan/Nexus_Core) | Core CLI & MCP (this repo) |
| [nexuscoreweb](https://github.com/yusufkaraaslan/nexuscoreweb) | Website & docs |
| [nexus-core-configs](https://github.com/yusufkaraaslan/nexus-core-configs) | Community configs |
| [nexus-core-action](https://github.com/yusufkaraaslan/nexus-core-action) | GitHub Action |
| [nexus-core-plugin](https://github.com/yusufkaraaslan/nexus-core-plugin) | Claude Code plugin |
| [homebrew-nexus-core](https://github.com/yusufkaraaslan/homebrew-nexus-core) | Homebrew tap |
...
pytest tests/ --cov=src/nexus_core

# Usage
nexus-core create https://docs.python.org/
nexus-core package output/python --target claude
nexus-core create configs/react_unified.json
```
