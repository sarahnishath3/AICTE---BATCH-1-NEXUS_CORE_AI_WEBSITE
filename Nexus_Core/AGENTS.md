...
Comprehensive reference for AI coding agents. Nexus Core is a Python CLI tool (v3.6.0) that converts documentation sites, GitHub repos, PDFs, videos, notebooks, wikis, and more into AI-ready skills for 21+ LLM platforms and RAG pipelines.

## Project Overview

**Nexus Core** is a universal preprocessing layer that transforms raw documentation and code into structured knowledge assets. It supports 17+ source types and exports to 21+ AI platforms including Claude, Gemini, OpenAI, LangChain, LlamaIndex, and various vector databases.
...
Note: `tests/conftest.py` checks that `nexus_core` is importable and calls `sys.exit(1)` if not. Always install in editable mode first.
...
### Imports
- Sort with isort (via ruff); `nexus_core` is first-party
...
```bash
# Core commands
nexus-core create <source>              # Create skill from any source (auto-detects type)
nexus-core scan <dir>                   # AI-detect a project's tech stack and emit per-framework configs
nexus-core enhance <directory>          # AI-powered enhancement
nexus-core package <directory>          # Package skill for target platform
nexus-core upload <file>                # Upload skill to target platform
nexus-core install <source>             # One-command workflow (scrape + enhance + package + upload)

# Utilities
nexus-core estimate <source>            # Estimate page count before scraping
nexus-core doctor                       # Health check for dependencies
nexus-core config                       # Configure API keys and settings
nexus-core workflows                    # List and apply workflow presets
nexus-core resume <job_id>              # Resume interrupted scraping

# Advanced
nexus-core stream <source>              # Streaming ingestion
nexus-core update <directory>           # Incremental update
nexus-core multilang <directory>        # Multi-language support
```
...
pytest tests/ --cov=src/nexus_core --cov-report=term-missing
...
# Run CLI
docker run -v $(pwd)/output:/output nexus-core create https://docs.example.com

# Run MCP server
docker run -p 8765:8765 nexus-core nexus-core-mcp
```
...
- **Documentation:** https://nexuscoreweb.com/
- **PyPI:** https://pypi.org/project/nexus-core/
- **Repository:** https://github.com/yusufkaraaslan/Nexus_Core
- **Config Browser:** https://nexuscoreweb.com/
