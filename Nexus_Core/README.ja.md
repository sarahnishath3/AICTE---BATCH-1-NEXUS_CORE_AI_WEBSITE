...
>
> 翻訳の改善にご協力いただける方は、[GitHub Issue #260](https://github.com/yusufkaraaslan/Nexus_Core/issues/260) からフィードバックをお寄せください。

[![バージョン](https://img.shields.io/badge/version-3.7.0-blue.svg)](https://github.com/yusufkaraaslan/Nexus_Core/releases)
...
[![PyPI バージョン](https://badge.fury.io/py/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - ダウンロード数](https://img.shields.io/pypi/dm/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - Python バージョン](https://img.shields.io/pypi/pyversions/nexus-core.svg)](https://pypi.org/project/nexus-core/)
...
[![公式サイト](https://img.shields.io/badge/Website-nexuscoreweb.com-blue.svg)](https://nexuscoreweb.com/)
...
**🧠 AI システムのデータレイヤー。** Nexus Core はドキュメントサイト、GitHub リポジトリ、PDF、動画、ノートブック、Wiki など 18 種類のソースタイプを構造化されたナレッジアセットに変換します。AI スキル（Claude、Gemini、OpenAI）、RAG パイプライン（LangChain、LlamaIndex、Pinecone）、AI コーディングアシスタント（Cursor、Windsurf、Cline）を、数時間ではなく数分で構築できます。

> 🌐 **[SkillSeekersWeb.com にアクセス](https://nexuscoreweb.com/)** - 24 以上のプリセット設定を閲覧、設定の共有、完全なドキュメントへのアクセス！

> 📋 **[開発ロードマップとタスクを確認](https://github.com/users/yusufkaraaslan/projects/2)** - 10 カテゴリで 134 タスク、好きなものを選んで貢献できます！

## 🌐 エコシステム

Nexus Core はマルチリポジトリプロジェクトです。各リポジトリの役割：

| リポジトリ | 説明 | リンク |
|-----------|------|-------|
| **[Nexus_Core](https://github.com/yusufkaraaslan/Nexus_Core)** | コア CLI & MCP サーバー（このリポジトリ） | [PyPI](https://pypi.org/project/nexus-core/) |
| **[nexuscoreweb](https://github.com/yusufkaraaslan/nexuscoreweb)** | ウェブサイト＆ドキュメント | [サイト](https://nexuscoreweb.com/) |
| **[nexus-core-configs](https://github.com/yusufkaraaslan/nexus-core-configs)** | コミュニティ設定リポジトリ | |
| **[nexus-core-action](https://github.com/yusufkaraaslan/nexus-core-action)** | GitHub Action CI/CD | |
| **[nexus-core-plugin](https://github.com/yusufkaraaslan/nexus-core-plugin)** | Claude Code プラグイン | |
| **[homebrew-nexus-core](https://github.com/yusufkaraaslan/homebrew-nexus-core)** | macOS Homebrew tap | |

> **貢献したいですか？** ウェブサイトと設定リポジトリは新しい貢献者に最適です！

## 🧠 AI システムのデータレイヤー

**Nexus Core は汎用的な前処理レイヤー**であり、生のドキュメントとそれを利用するすべての AI システムの間に位置します。Claude スキル、LangChain RAG パイプライン、Cursor の `.cursorrules` ファイルのいずれを構築する場合でも、データの準備作業は同じです。一度実行すれば、すべてのターゲットにエクスポートできます。

```bash
# 1コマンド → 構造化ナレッジアセット
nexus-core create https://docs.react.dev/
# または: nexus-core create facebook/react
# または: nexus-core create ./my-project

# 任意の AI システムにエクスポート
nexus-core package output/react --target claude      # → Claude AI スキル (ZIP)
...
nexus-core package output/react --target ibm-bob     # → IBM Bob スキルディレクトリ
```
...
検出結果に既存のプリセットがない場合は AI が新しい設定を生成します。終了時に [コミュニティレジストリ](https://github.com/yusufkaraaslan/nexus-core-configs) への公開を任意で選択できます。
...
# 動画（YouTube、Vimeo、またはローカルファイル — nexus-core[video] が必要）
...
Nexus Core は **AI システムのデータレイヤー**であり、18 種類のソースタイプ——ドキュメントサイト、GitHub リポジトリ、PDF、動画、Jupyter Notebook、Word/EPUB/AsciiDoc ドキュメント、OpenAPI 仕様、PowerPoint プレゼンテーション、RSS フィード、Man ページ、Confluence Wiki、Notion ページ、Slack/Discord チャットエクスポートなど——をすべての AI ターゲットに適した構造化ナレッジアセットに変換します：
...
Nexus Core は数日かかる手動前処理の代わりに以下を行います：
...
Nexus Core はログイン済みのコーディングエージェント CLI を呼び出せるため、従量課金の API トークンではなく既存のプランで強化を実行できます：

```bash
nexus-core create <source> --agent codex     # OpenAI Codex CLI → ChatGPT Plus プラン
nexus-core create <source> --agent claude    # Claude Code      → Claude Pro/Max プラン
```
...
これらはすべて OpenAI 互換の `/v1` エンドポイントを公開しています。3 つの環境変数を設定するだけで利用できます — Nexus Core は `OPENAI_API_KEY` を検出し、OpenAI SDK が `OPENAI_BASE_URL` を自動的に尊重します：
...
nexus-core package output/django --target claude  # または --target markdown
...
- ✅ **マルチトークン設定システム** - 複数の GitHub アカウント（個人、仕事、OSS）を管理
  - セキュアな設定ストレージ `~/.config/nexus-core/config.json`（パーミッション 600）
...
nexus-core 自体をスキルとして生成し、AI エージェント（Claude Code、Kimi、Codex など）内で使用できます：
...
- ✅ **ユーザー定義プリセット** — `~/.config/nexus-core/workflows/` にカスタムワークフローを追加
...
# プリセットの管理
nexus-core workflows list                          # すべて一覧表示（組み込み + ユーザー）
nexus-core workflows show security-focus           # YAML 内容を表示
nexus-core workflows copy security-focus           # 編集用にユーザーディレクトリにコピー
nexus-core workflows add ./my-workflow.yaml        # カスタムプリセットをインストール
nexus-core workflows remove my-workflow            # ユーザープリセットを削除
nexus-core workflows validate security-focus       # プリセット構造を検証
...
| インストールコマンド | 機能 |
|-------------------|------|
| `pip install nexus-core` | スクレイピング、GitHub 分析、PDF、全プラットフォーム |
| `pip install nexus-core[gemini]` | + Google Gemini サポート |
| `pip install nexus-core[openai]` | + OpenAI ChatGPT サポート |
| `pip install nexus-core[all-llms]` | + すべての LLM プラットフォーム |
| `pip install nexus-core[mcp]` | + Claude Code や Cursor などの MCP サーバー |
| `pip install nexus-core[video]` | + YouTube/Vimeo 字幕 & メタデータ抽出 |
| `pip install nexus-core[video-full]` | + Whisper 文字起こし & ビジュアルフレーム抽出 |
| `pip install nexus-core[jupyter]` | + Jupyter Notebook サポート |
| `pip install nexus-core[pptx]` | + PowerPoint サポート |
| `pip install nexus-core[confluence]` | + Confluence Wiki サポート |
| `pip install nexus-core[notion]` | + Notion ページサポート |
| `pip install nexus-core[rss]` | + RSS/Atom フィードサポート |
| `pip install nexus-core[chat]` | + Slack/Discord チャットエクスポートサポート |
| `pip install nexus-core[asciidoc]` | + AsciiDoc ドキュメントサポート |
| `pip install nexus-core[all]` | 全機能 |

> **動画ビジュアル依存関係（GPU 対応）：** `nexus-core[video-full]` をインストールした後、
> `nexus-core create --setup` を実行して GPU を自動検出し、正しい PyTorch
...
Nexus Core は **12 の LLM プラットフォーム**、**8 つの RAG/ベクトルターゲット**、**18 種類のソースタイプ**をサポートし、すべてのターゲットで完全な機能パリティを提供します。
...
```bash
# 動画サポートのインストール
pip install nexus-core[video]        # 字幕 + メタデータ
pip install nexus-core[video-full]   # + Whisper 文字起こし + ビジュアルフレーム抽出
...
完全な UML 図：**[docs/UML_ARCHITECTURE.md](docs/UML_ARCHITECTURE.md)** | StarUML プロジェクト：`docs/UML/nexus_core.mdj` | HTML API リファレンス：`docs/UML/html/`
...
Nexus Core は 19 の AI コーディングエージェントにスキルを自動インストールできます。
...
Nexus Core は Claude Code、Cursor、Windsurf、VS Code + Cline、IntelliJ IDEA で使用できる MCP サーバーを提供します。
...
```bash
# すべてのプリセットを一覧表示
# nexus-core list-configs  # v3.7.0 では利用不可
```
...
3. `~/.config/nexus-core/configs/`（ユーザー設定ディレクトリ）
...
```bash
nexus-core create https://docs.react.dev/ --preset quick      # 高速、表面レベル
nexus-core create https://docs.react.dev/ --preset standard   # バランス（デフォルト）
nexus-core create https://docs.react.dev/ --preset comprehensive # 深く、網羅的
```

### ライフサイクルフラグ
```bash
nexus-core create https://docs.react.dev/ --dry-run    # スクレイピングなしでプレビュー
nexus-core create https://docs.react.dev/ --fresh      # キャッシュを無視して完全に再スクレイピング
nexus-core create https://docs.react.dev/ --resume     # 中断したジョブを再開
nexus-core create https://docs.react.dev/ --skip-scrape # 既存の出力を再パッケージング
```

### ヘルスチェックとユーティリティ
```bash
nexus-core doctor                    # インストールと環境の診断
nexus-core sync-config               # 設定のドリフト検出
nexus-core stream <source>           # 大規模ドキュメントのストリーミング取り込み
nexus-core update output/react/      # 増分更新
nexus-core multilang <source>        # 多言語スキル生成
nexus-core quality output/react/     # 品質レポート（--threshold 7 でゲート化: 7/10 未満なら非ゼロ終了）
```
...
| Extra | インストール | 用途 |
|-------|---------|---------|
| `browser` | `pip install "nexus-core[browser]"` | SPA サイト向けヘッドレス Playwright |
| `embedding` | `pip install "nexus-core[embedding]"` | エンベディングサーバーサポート |
| `s3` / `gcs` / `azure` | `pip install "nexus-core[s3]"` など | クラウドストレージアップロード |
| `rag-upload` | `pip install "nexus-core[rag-upload]"` | ベクトル DB アップロード依存関係一式 |
...
- **[docs/UML/nexus_core.mdj](docs/UML/nexus_core.mdj)** - StarUML プロジェクトファイル（[StarUML](https://staruml.io/) で開く）
...
[![MseeP.ai セキュリティ評価バッジ](https://mseep.net/pr/yusufkaraaslan-nexus-core-badge.png)](https://mseep.ai/app/yusufkaraaslan-nexus-core)
...
[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=nexus_core) — フルモーダルで OpenAI 互換の AI 推論プラットフォーム。Nexus Core は `--target atlas` と `ATLAS_API_KEY` によるパッケージング/強化ターゲットとしてサポートしています。
