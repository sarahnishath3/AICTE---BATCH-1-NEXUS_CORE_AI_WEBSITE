...
[![الإصدار](https://img.shields.io/badge/version-3.7.0-blue.svg)](https://github.com/yusufkaraaslan/Nexus_Core/releases)
...
[![إصدار PyPI](https://badge.fury.io/py/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - التنزيلات](https://img.shields.io/pypi/dm/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - إصدار Python](https://img.shields.io/pypi/pyversions/nexus-core.svg)](https://pypi.org/project/nexus-core/)
...
[![الموقع الرسمي](https://img.shields.io/badge/Website-nexuscoreweb.com-blue.svg)](https://nexuscoreweb.com/)
[![متابعة على Twitter](https://img.shields.io/twitter/follow/_yUSyUS_?style=social)](https://x.com/_yUSyUS_)
[![نجوم GitHub](https://img.shields.io/github/stars/yusufkaraaslan/Nexus_Core?style=social)](https://github.com/yusufkaraaslan/Nexus_Core)

**🧠 طبقة البيانات لأنظمة الذكاء الاصطناعي.** يحوّل Nexus Core مواقع التوثيق ومستودعات GitHub وملفات PDF والفيديوهات ودفاتر Jupyter والويكي و18 نوعًا من المصادر إلى أصول معرفية منظمة — جاهزة لتشغيل مهارات الذكاء الاصطناعي (Claude وGemini وOpenAI) وخطوط أنابيب RAG (مثل LangChain وLlamaIndex وPinecone) ومساعدات البرمجة بالذكاء الاصطناعي (مثل Cursor وWindsurf وCline) في دقائق بدلاً من ساعات.

> 🌐 **[زيارة SkillSeekersWeb.com](https://nexuscoreweb.com/)** - تصفح أكثر من 24 إعدادًا مسبقًا، وشارك إعداداتك، واطّلع على التوثيق الكامل!

> 📋 **[عرض خارطة الطريق والمهام](https://github.com/users/yusufkaraaslan/projects/2)** - 134 مهمة عبر 10 فئات، اختر أيًا منها للمساهمة!

## 🌐 المنظومة

Nexus Core هو مشروع متعدد المستودعات. إليك أين يوجد كل شيء:

| المستودع | الوصف | الروابط |
|----------|-------|---------|
| **[Nexus_Core](https://github.com/yusufkaraaslan/Nexus_Core)** | CLI الأساسي وخادم MCP (هذا المستودع) | [PyPI](https://pypi.org/project/nexus-core/) |
| **[nexuscoreweb](https://github.com/yusufkaraaslan/nexuscoreweb)** | الموقع والتوثيق | [الموقع](https://nexuscoreweb.com/) |
| **[nexus-core-configs](https://github.com/yusufkaraaslan/nexus-core-configs)** | مستودع إعدادات المجتمع | |
| **[nexus-core-action](https://github.com/yusufkaraaslan/nexus-core-action)** | GitHub Action لـ CI/CD | |
| **[nexus-core-plugin](https://github.com/yusufkaraaslan/nexus-core-plugin)** | إضافة Claude Code | |
| **[homebrew-nexus-core](https://github.com/yusufkaraaslan/homebrew-nexus-core)** | Homebrew tap لـ macOS | |

> **تريد المساهمة؟** مستودعات الموقع والإعدادات هي نقاط بداية رائعة للمساهمين الجدد!

## 🧠 طبقة البيانات لأنظمة الذكاء الاصطناعي

**Nexus Core هو طبقة المعالجة المسبقة العامة** التي تقع بين التوثيق الخام وجميع أنظمة الذكاء الاصطناعي التي تستهلكه. سواء كنت تبني مهارات Claude أو خط أنابيب RAG باستخدام LangChain أو ملف `.cursorrules` لـ Cursor — فإن تحضير البيانات متطابق. تقوم بذلك مرة واحدة وتصدّر إلى جميع المنصات المستهدفة.

```bash
# أمر واحد → أصل معرفي منظم
nexus-core create https://docs.react.dev/
# أو: nexus-core create facebook/react
# أو: nexus-core create ./my-project

# التصدير إلى أي نظام ذكاء اصطناعي
nexus-core package output/react --target claude      # → مهارة Claude AI (ZIP)
...
nexus-core package output/react --target ibm-bob     # → مجلد مهارة IBM Bob
```
...
إذا لم يكن للاكتشاف إعداد مسبق موجود، يولّد الذكاء الاصطناعي إعدادًا جديدًا؛ وعند الخروج يمكنك اختياريًا نشره في [سجل المجتمع](https://github.com/yusufkaraaslan/nexus-core-configs).
...
# الفيديو (YouTube أو Vimeo أو ملف محلي — يتطلب nexus-core[video])
...
بدلاً من قضاء أيام في المعالجة اليدوية المسبقة، يقوم Nexus Core بـ:
...
يمكن لـ Nexus Core استدعاء واجهة سطر أوامر لوكيل برمجة سجّلت الدخول إليه بالفعل، بحيث يعمل التعزيز على خطتك الحالية بدلاً من رموز API المدفوعة حسب الاستخدام:

```bash
nexus-core create <source> --agent codex     # OpenAI Codex CLI → اشتراكك في ChatGPT Plus
nexus-core create <source> --agent claude    # Claude Code      → اشتراكك في Claude Pro/Max
```
...
كل هؤلاء يوفرون نقطة نهاية `/v1` متوافقة مع OpenAI. وجّه Nexus Core إلى أحدها بثلاثة متغيرات بيئة — فهو يكتشف `OPENAI_API_KEY`، وتحترم مكتبة OpenAI SDK متغير `OPENAI_BASE_URL` تلقائيًا:
...
nexus-core package output/django --target claude  # أو --target markdown
...
- ✅ **نظام إعداد متعدد الرموز** - إدارة حسابات GitHub متعددة (شخصي وعمل ومفتوح المصدر)
  - تخزين آمن للإعدادات في `~/.config/nexus-core/config.json` (صلاحيات 600)
...
توليد nexus-core نفسه كمهارة لاستخدامها داخل وكيل الذكاء الاصطناعي الخاص بك (Claude Code وKimi وCodex وغيرها):
...
- ✅ **إعدادات مخصصة** — إضافة سير عمل مخصص إلى `~/.config/nexus-core/workflows/`
...
nexus-core workflows list                          # عرض الكل (مُضمّنة + مخصصة)
nexus-core workflows show security-focus           # عرض محتوى YAML
nexus-core workflows copy security-focus           # نسخ إلى مجلد المستخدم للتعديل
nexus-core workflows add ./my-workflow.yaml        # تثبيت إعداد مخصص
nexus-core workflows remove my-workflow            # حذف إعداد مخصص
nexus-core workflows validate security-focus       # التحقق من بنية الإعداد
...
| أمر التثبيت | الميزات |
|------------|---------|
| `pip install nexus-core` | الاستخراج وتحليل GitHub وPDF وجميع المنصات |
| `pip install nexus-core[gemini]` | + دعم Google Gemini |
| `pip install nexus-core[openai]` | + دعم OpenAI ChatGPT |
| `pip install nexus-core[all-llms]` | + جميع منصات LLM |
| `pip install nexus-core[mcp]` | + خادم MCP لـ Claude Code وCursor وغيرهما |
| `pip install nexus-core[video]` | + استخراج نصوص وبيانات YouTube/Vimeo |
| `pip install nexus-core[video-full]` | + نسخ Whisper + استخراج الإطارات المرئية |
| `pip install nexus-core[jupyter]` | + دعم دفاتر Jupyter |
| `pip install nexus-core[pptx]` | + دعم PowerPoint |
| `pip install nexus-core[confluence]` | + دعم ويكي Confluence |
| `pip install nexus-core[notion]` | + دعم صفحات Notion |
| `pip install nexus-core[rss]` | + دعم خلاصات RSS/Atom |
| `pip install nexus-core[chat]` | + دعم تصدير محادثات Slack/Discord |
| `pip install nexus-core[asciidoc]` | + دعم مستندات AsciiDoc |
| `pip install nexus-core[all]` | تفعيل كل شيء |

> **المكونات المرئية للفيديو (مدركة لـ GPU):** بعد تثبيت `nexus-core[video-full]`، شغّل
> `nexus-core create --setup` لاكتشاف GPU تلقائيًا وتثبيت إصدار PyTorch
...
يدعم Nexus Core **12 منصة LLM** و**8 أهداف RAG/متجهات** و**18 نوعًا من المصادر** مع تكافؤ كامل في الميزات عبر جميع الأهداف.
...
```bash
# تثبيت دعم الفيديو
pip install nexus-core[video]        # النصوص + البيانات الوصفية
pip install nexus-core[video-full]   # + نسخ Whisper + استخراج الإطارات المرئية
...
مخططات UML الكاملة: **[docs/UML_ARCHITECTURE.md](docs/UML_ARCHITECTURE.md)** | مشروع StarUML: `docs/UML/nexus_core.mdj` | مرجع HTML API: `docs/UML/html/`
...
يمكن لـ Nexus Core تثبيت المهارات تلقائيًا في 19 وكيل برمجة بالذكاء الاصطناعي.
...
يأتي Nexus Core مع خادم MCP للاستخدام من Claude Code وCursor وWindsurf وVS Code + Cline أو IntelliJ IDEA.
...
# nexus-core list-configs  # غير متاح في v3.7.0
```
...
3. `~/.config/nexus-core/configs/` (مجلد إعدادات المستخدم)
...
```bash
nexus-core create https://docs.react.dev/ --preset quick       # سريع، سطحي
nexus-core create https://docs.react.dev/ --preset standard    # متوازن (الافتراضي)
nexus-core create https://docs.react.dev/ --preset comprehensive  # عميق وشامل
```

### أعلام دورة الحياة
```bash
nexus-core create https://docs.react.dev/ --dry-run    # معاينة بدون استخراج
nexus-core create https://docs.react.dev/ --fresh      # تجاهل ذاكرة التخزين المؤقت، إعادة استخراج كاملة
nexus-core create https://docs.react.dev/ --resume     # استئناف المهمة المقاطعة
nexus-core create https://docs.react.dev/ --skip-scrape # إعادة تعبئة الناتج الموجود
```

### فحص الصحة والأدوات المساعدة
```bash
nexus-core doctor                    # تشخيص التثبيت والبيئة
nexus-core sync-config               # كشف انحراف التكوين
nexus-core stream <source>           # الاستهلاك المتدفق للمستندات الكبيرة
nexus-core update output/react/      # التحديث التزايدي
nexus-core multilang <source>        # إنشاء مهارات متعددة اللغات
nexus-core quality output/react/     # تقرير الجودة (أضف --threshold 7 لفرض حد أدنى: رمز خروج غير صفري عند أقل من 7/10)
```
...
|---------|---------|-------|
| `browser` | `pip install "nexus-core[browser]"` | Playwright بدون واجهة لمواقع SPA |
| `embedding` | `pip install "nexus-core[embedding]"` | دعم خادم التضمينات |
| `s3` / `gcs` / `azure` | `pip install "nexus-core[s3]"` وغيرها | الرفع إلى التخزين السحابي |
| `rag-upload` | `pip install "nexus-core[rag-upload]"` | تبعيات الرفع المجمعة لقواعد بيانات المتجهات |
...
- **[docs/UML/nexus_core.mdj](docs/UML/nexus_core.mdj)** - ملف مشروع StarUML (افتحه باستخدام [StarUML](https://staruml.io/))
...
[![شارة تقييم أمان MseeP.ai](https://mseep.net/pr/yusufkaraaslan-nexus-core-badge.png)](https://mseep.ai/app/yusufkaraaslan-nexus-core)
...
[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=nexus_core) — منصة استدلال ذكاء اصطناعي كاملة الوسائط ومتوافقة مع OpenAI. يدعمها Nexus Core كهدف للتعبئة/التعزيز عبر `--target atlas` مع `ATLAS_API_KEY`.
