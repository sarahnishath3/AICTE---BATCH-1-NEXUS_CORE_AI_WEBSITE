...
<a href="https://trendshift.io/repositories/18329" target="_blank"><img src="https://trendshift.io/api/badge/repositories/18329" alt="yusufkaraaslan%2FNexus_Core | Trendshift" style="width: 250px; height: 55px;" width="250" height="55"/></a>

**🧠 The data layer for AI systems.** Nexus Core turns documentation sites, GitHub repos, PDFs, videos, notebooks, wikis, and 10+ more source types into structured knowledge assets—ready to power AI Skills (Claude, Gemini, OpenAI), RAG pipelines (LangChain, LlamaIndex, Pinecone), and AI coding assistants (Cursor, Windsurf, Cline) in minutes, not hours.

> 🌐 **[Visit SkillSeekersWeb.com](https://nexuscoreweb.com/)** - Browse 24+ preset configs, share your configs, and access complete documentation!
...
Nexus Core is a multi-repo project. Here's where everything lives:

| Repository | Description | Links |
|-----------|-------------|-------|
| **[Nexus_Core](https://github.com/yusufkaraaslan/Nexus_Core)** | Core CLI & MCP server (this repo) | [PyPI](https://pypi.org/project/nexus-core/) |
| **[nexuscoreweb](https://github.com/yusufkaraaslan/nexuscoreweb)** | Website & documentation | [Live](https://nexuscoreweb.com/) |
| **[nexus-core-configs](https://github.com/yusufkaraaslan/nexus-core-configs)** | Community config repository | |
| **[nexus-core-action](https://github.com/yusufkaraaslan/nexus-core-action)** | GitHub Action for CI/CD | |
| **[nexus-core-plugin](https://github.com/yusufkaraaslan/nexus-core-plugin)** | Claude Code plugin | |
| **[homebrew-nexus-core](https://github.com/yusufkaraaslan/homebrew-nexus-core)** | Homebrew tap for macOS | |
...
**Nexus Core is the universal preprocessing layer** that sits between raw documentation and every AI system that consumes it. Whether you are building Claude skills, a LangChain RAG pipeline, or a Cursor `.cursorrules` file — the data preparation is identical. You do it once, and export to all targets.

```bash
# One command → structured knowledge asset
nexus-core create https://docs.react.dev/
# or: nexus-core create facebook/react
# or: nexus-core create ./my-project
...
nexus-core package output/react --target ibm-bob     # → IBM Bob skill directory
```
...
exit you can optionally publish it back to the [community registry](https://github.com/yusufkaraaslan/nexus-core-configs).
...
# Video (YouTube, Vimeo, or local file — requires nexus-core[video])
...
Instead of spending days on manual preprocessing, Nexus Core:
...
Nexus Core can shell out to a coding-agent CLI you're already logged into, so enhancement runs on your existing plan instead of metered API tokens:

```bash
nexus-core create <source> --agent codex     # OpenAI Codex CLI → your ChatGPT Plus
nexus-core create <source> --agent claude    # Claude Code      → your Claude Pro/Max
```
...
All of these expose an OpenAI-compatible `/v1` endpoint. Point Nexus Core at one with three env vars — it detects `OPENAI_API_KEY`, and the OpenAI SDK honors `OPENAI_BASE_URL` automatically:
...
nexus-core package output/django --target claude  # or --target markdown
...
- ✅ **Multi-Token Configuration System** - Manage multiple GitHub accounts (personal, work, OSS)
  - Secure config storage at `~/.config/nexus-core/config.json` (600 permissions)
...
Generate nexus-core as a skill to use within your AI agent (Claude Code, Kimi, Codex, etc.):
...
- ✅ **User-Defined Presets** — add custom workflows to `~/.config/nexus-core/workflows/`
...
# Manage presets
nexus-core workflows list                          # List all (bundled + user)
nexus-core workflows show security-focus           # Print YAML content
nexus-core workflows copy security-focus           # Copy to user dir for editing
nexus-core workflows add ./my-workflow.yaml        # Install a custom preset
nexus-core workflows remove my-workflow            # Remove a user preset
nexus-core workflows validate security-focus       # Validate preset structure
...
| Install | Features |
|---------|----------|
| `pip install nexus-core` | Scraping, GitHub analysis, PDF, all platforms |
| `pip install nexus-core[gemini]` | + Google Gemini support |
| `pip install nexus-core[openai]` | + OpenAI ChatGPT support |
| `pip install nexus-core[all-llms]` | + All LLM platforms |
| `pip install nexus-core[mcp]` | + MCP server for Claude Code, Cursor, etc. |
| `pip install nexus-core[video]` | + YouTube/Vimeo transcript & metadata extraction |
| `pip install nexus-core[video-full]` | + Whisper transcription & visual frame extraction |
| `pip install nexus-core[jupyter]` | + Jupyter Notebook support |
| `pip install nexus-core[pptx]` | + PowerPoint support |
| `pip install nexus-core[confluence]` | + Confluence wiki support |
| `pip install nexus-core[notion]` | + Notion pages support |
| `pip install nexus-core[rss]` | + RSS/Atom feed support |
| `pip install nexus-core[chat]` | + Slack/Discord chat export support |
| `pip install nexus-core[asciidoc]` | + AsciiDoc document support |
| `pip install nexus-core[all]` | Everything enabled |

> **Video visual deps (GPU-aware):** After installing `nexus-core[video-full]`, run
> `nexus-core create --setup` to auto-detect your GPU and install the correct PyTorch
...
Nexus Core supports **12 LLM platforms**, **8 RAG/vector targets**, **18 source types**, and full feature parity across all targets.
...
pip install nexus-core[video]        # Transcripts + metadata
pip install nexus-core[video-full]   # + Whisper + visual frame extraction
...
```mermaid
graph LR
    A[Documentation Website] --> B[Nexus Core]
    B --> C[Scraper]
    B --> D[AI Enhancement]
    B --> E[Packager]
...
Full UML diagrams: **[docs/UML_ARCHITECTURE.md](docs/UML_ARCHITECTURE.md)** | StarUML project: `docs/UML/nexus_core.mdj` | HTML API reference: `docs/UML/html/`
...
Nexus Core can automatically install skills to 19 AI coding agents.
...
Nexus Core ships an MCP server for use from Claude Code, Cursor, Windsurf, VS Code + Cline, or IntelliJ IDEA.
...
```bash
# List all presets
# nexus-core list-configs  # Not available in v3.7.0
```
...
3. `~/.config/nexus-core/configs/` (user config directory)
...
```bash
nexus-core create https://docs.react.dev/ --preset quick      # Fast, surface-level
nexus-core create https://docs.react.dev/ --preset standard   # Balanced (default)
nexus-core create https://docs.react.dev/ --preset comprehensive # Deep, exhaustive
```

### Lifecycle Flags
```bash
nexus-core create https://docs.react.dev/ --dry-run    # Preview without scraping
nexus-core create https://docs.react.dev/ --fresh      # Ignore cache, full re-scrape
nexus-core create https://docs.react.dev/ --resume     # Resume interrupted job
nexus-core create https://docs.react.dev/ --skip-scrape # Re-package existing output
```

### Health Check & Utilities
```bash
nexus-core doctor                    # Diagnose installation & environment
nexus-core sync-config               # Detect config drift
nexus-core stream <source>           # Streaming ingestion for large docs
nexus-core update output/react/      # Incremental update
nexus-core multilang <source>        # Multi-language skill generation
nexus-core quality output/react/     # Quality report (add --threshold 7 to gate: non-zero exit below 7/10)
```
...
|-------|---------|---------|
| `browser` | `pip install "nexus-core[browser]"` | Headless Playwright for SPA sites |
| `embedding` | `pip install "nexus-core[embedding]"` | Embedding server support |
| `s3` / `gcs` / `azure` | `pip install "nexus-core[s3]"` etc. | Cloud storage upload |
| `rag-upload` | `pip install "nexus-core[rag-upload]"` | Combined vector DB upload deps |
...
- **[docs/UML/nexus_core.mdj](docs/UML/nexus_core.mdj)** - StarUML project file (open with [StarUML](https://staruml.io/))
...
[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=nexus_core) — a full-modal, OpenAI-compatible AI inference platform. Nexus Core supports it as a packaging/enhancement target via `--target atlas` with `ATLAS_API_KEY`.
