...
**⚠️ IMPORTANT:** Nexus Core uses a two-branch workflow.
...
Nexus Core spans multiple repositories. Make sure you're contributing to the right one:

| What you want to work on | Repository |
|--------------------------|-----------|
| Core CLI, scrapers, MCP tools, adaptors | [Nexus_Core](https://github.com/yusufkaraaslan/Nexus_Core) (this repo) |
| Website, docs, UI/UX | [nexuscoreweb](https://github.com/yusufkaraaslan/nexuscoreweb) |
| Preset configs, community configs | [nexus-core-configs](https://github.com/yusufkaraaslan/nexus-core-configs) |
| GitHub Action integration | [nexus-core-action](https://github.com/yusufkaraaslan/nexus-core-action) |
| Claude Code plugin | [nexus-core-plugin](https://github.com/yusufkaraaslan/nexus-core-plugin) |
| Homebrew formula | [homebrew-nexus-core](https://github.com/yusufkaraaslan/homebrew-nexus-core) |
...
Before creating bug reports, please check the [existing issues](https://github.com/yusufkaraaslan/Nexus_Core/issues) to avoid duplicates.
...
2. Run `nexus-core create --config configs/test.json`
3. See error
...
Enhancement suggestions are tracked as [GitHub issues](https://github.com/yusufkaraaslan/Nexus_Core/issues).
...
# Run with coverage
python -m pytest tests/ --cov=src/nexus_core --cov-report=term
```
...
**Scraper pattern (18 source types):** Each source type is a `SkillConverter` subclass in `cli/<type>_scraper.py` (document-shaped sources subclass `DocumentSkillBuilder`, which provides the whole build side), reached via `nexus-core create` auto-detection — there is no per-type `main()`. Register new types in: `CONVERTER_REGISTRY` in `skill_converter.py` (this also enables them in unified multi-source configs), `create_command.py:_build_config()`, `source_detector.py`, and `config_validator.py` VALID_SOURCE_TYPES. CLI flags are defined once in the central `parsers/*.py` classes (a drift-guard test enforces this).
...
- **[docs/UML/nexus_core.mdj](docs/UML/nexus_core.mdj)** - StarUML project (open with [StarUML](https://staruml.io/))
...
- 💬 [Open a discussion](https://github.com/yusufkaraaslan/Nexus_Core/discussions)
- 🐛 [Report a bug](https://github.com/yusufkaraaslan/Nexus_Core/issues)
