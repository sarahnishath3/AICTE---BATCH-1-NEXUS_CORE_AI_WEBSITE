...
nexus-core create --video-url  https://www.youtube.com/watch?v=VIDEO_ID
...
nexus-core create --video-url  https://www.youtube.com/watch?v=VIDEO_ID --visual

# Extract with AI enhancement (cleans OCR, synthesizes tutorial)
nexus-core create --video-url  https://www.youtube.com/watch?v=VIDEO_ID --visual --enhance-level 2
```
...
These flags are available on all Nexus Core commands:
...
```bash
nexus-core create --video-url  https://www.youtube.com/watch?v=dQw4w9WgXcQ --name my-tutorial
```

Shortened URLs also work:

```bash
nexus-core create --video-url  https://youtu.be/dQw4w9WgXcQ
```
...
```bash
nexus-core create --video-playlist "https://www.youtube.com/playlist?list=PLxxxxxxx" --name course-name
```
...
```bash
nexus-core create --video-file recording.mp4 --name my-recording
```
...
```bash
nexus-core create --from-json output/my-tutorial_video_extracted.json --name my-tutorial
```
...
nexus-core create --video-url  https://youtu.be/VIDEO_ID --start-time 5:00 --end-time 15:00

# Extract from 2 minutes onward
nexus-core create --video-url  https://youtu.be/VIDEO_ID --start-time 120

# Extract the first 10 minutes
nexus-core create --video-url  https://youtu.be/VIDEO_ID --end-time 10:00
```
...
```bash
nexus-core create --video-url  https://www.youtube.com/watch?v=VIDEO_ID --name react-hooks-tutorial
```

### Visual extraction with on-screen code recognition

```bash
nexus-core create --video-url  https://youtu.be/VIDEO_ID --name godot-signals --visual
```
...
nexus-core create --video-url  https://youtu.be/VIDEO_ID --name django-rest-api \
...
nexus-core create --video-file ./recording.mp4 --name my-lecture
```

### Extract a specific section of a long video

```bash
nexus-core create --video-url  https://youtu.be/VIDEO_ID --name auth-chapter \
...
```bash
nexus-core create --video-playlist "https://www.youtube.com/playlist?list=PLxxxxxxx" \
...
```bash
nexus-core create --from-json output/my-tutorial_video_extracted.json \
...
```bash
export ANTHROPIC_API_KEY=sk-ant-...
nexus-core create --video-url  https://youtu.be/VIDEO_ID --name cpp-tutorial \
