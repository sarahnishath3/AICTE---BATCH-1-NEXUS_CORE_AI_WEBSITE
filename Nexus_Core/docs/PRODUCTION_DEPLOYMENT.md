...
Complete guide for deploying Nexus Core in production environments.
...
mkdir -p ~/.config/nexus-core/{configs,output,logs,cache}

# Set permissions
chmod 700 ~/.config/nexus-core
```
...
# GitHub Tokens (use nexus-core config --github for multiple)
GITHUB_TOKEN=ghp_...
...
```ini
[Unit]
Description=Nexus Core MCP Server
...
ExecStart=/opt/skillseekers/venv/bin/python -m nexus_core.mcp.server_fastmcp --transport http --port 8765
Restart=always
RestartSec=10
...
**What to backup:**
- Configuration files (`~/.config/nexus-core/`)
...
4. **Test functionality**
   ```bash
   nexus-core create --config configs/test.json --max-pages 10
   ```
