...
> Updated 2026-06-11 | StarUML project: `docs/UML/nexus_core.mdj`
>
...
Nexus Core converts documentation from 18 source types into production-ready formats for 24+ AI platforms. The architecture follows a layered module design with 9 core modules and 5 utility modules. Source-type ingestion is routed through a single `nexus-core create` command via the `SkillConverter` base class + factory pattern. A separate `nexus-core scan` command (added in #327) is the AI-driven project knowledge-base bootstrapper that emits one config per detected framework — these configs feed back into `create`.
...
- **CLICore** -- Git-style command dispatcher, entry point for all `nexus-core` commands
...
When `--browser` flag is set, `DocScraper.scrape_page()` delegates to `BrowserRenderer.render_page(url)` instead of `requests.get()`. The renderer auto-installs Chromium on first use, navigates with `wait_until='networkidle'` to let JavaScript execute, then returns the fully-rendered HTML. The rest of the pipeline (BeautifulSoup → `extract_content()` → `save_page()`) remains unchanged. Optional dependency: `pip install "nexus-core[browser]"`.

## File Locations

- **StarUML project**: `docs/UML/nexus_core.mdj`
- **Diagram exports**: `docs/UML/exports/*.png`
- **Source code**: `src/nexus_core/`
