> ⚠️ **DEPRECATED**: This document is outdated and uses old CLI patterns.
> 
> For up-to-date documentation, please see:
> - [Quick Start Guide](docs/getting-started/02-quick-start.md) - 3 commands to first skill
> - [Installation Guide](docs/getting-started/01-installation.md) - Complete installation
> - [Documentation Hub](docs/README.md) - All documentation
>
> *This file is kept for historical reference only.*

---

# Quick Start Guide

## 🚀 3 Steps to Create a Skill

### Step 1: Install Dependencies

```bash
pip3 install requests beautifulsoup4
```

> **Note:** Nexus_Core automatically checks for llms.txt files first, which is 10x faster when available.

### Step 2: Run the Tool

**Option A: Use a Preset (Easiest)**
```bash
nexus-core create --config configs/godot.json
```

**Option B: Interactive Mode**
```bash
nexus-core create --interactive
```

**Option C: Quick Command**
```bash
nexus-core create --name react --url https://react.dev/
```

**Option D: Unified Multi-Source (NEW - v2.0.0)**
```bash
# Combine documentation + GitHub code in one skill
nexus-core create --config configs/react_unified.json
```
*Detects conflicts between docs and code automatically!*

### Step 3: Enhance SKILL.md (Recommended)

```bash
# LOCAL enhancement (no API key, uses Claude Code Max)
nexus-core enhance output/godot/
```

**This takes 60 seconds and dramatically improves the SKILL.md quality!**

### Step 4: Package the Skill

```bash
nexus-core package output/godot/
```

**Done!** You now have `godot.zip` ready to use.

---

## 📋 Available Presets

```bash
# Godot Engine
nexus-core create --config configs/godot.json

# React
nexus-core create --config configs/react.json

# Vue.js
nexus-core create --config configs/vue.json

# Django
nexus-core create --config configs/django.json

# FastAPI
nexus-core create --config configs/fastapi.json

# Unified Multi-Source (NEW!)
nexus-core create --config configs/react_unified.json
nexus-core create --config configs/django_unified.json
nexus-core create --config configs/fastapi_unified.json
nexus-core create --config configs/godot_unified.json
```

---

## ⚡ Using Existing Data (Fast!)

If you already scraped once:

```bash
nexus-core create --config configs/godot.json

# When prompted:
✓ Found existing data: 245 pages
Use existing data? (y/n): y

# Builds in seconds!
```

Or use `--skip-scrape`:
```bash
nexus-core create --config configs/godot.json --skip-scrape
```

---

## 🎯 Complete Example (Recommended Workflow)

```bash
# 1. Install (once)
pip3 install requests beautifulsoup4

# 2. Scrape React docs with LOCAL enhancement
nexus-core create --config configs/react.json --enhance-local
# Wait 15-30 minutes (scraping) + 60 seconds (enhancement)

# 3. Package
nexus-core package output/react/

# 4. Use react.zip in Claude!
```

**Alternative: Enhancement after scraping**
```bash
# 2a. Scrape only (no enhancement)
nexus-core create --config configs/react.json

# 2b. Enhance later
nexus-core enhance output/react/

# 3. Package
nexus-core package output/react/
```

---

## 💡 Pro Tips

### Test with Small Pages First
Edit config file:
```json
{
  "max_pages": 20  // Test with just 20 pages
}
```

### Rebuild Instantly
```bash
# After first scrape, you can rebuild instantly:
nexus-core create --config configs/react.json --skip-scrape
```

### Create Custom Config
```bash
# Copy a preset
cp configs/react.json configs/myframework.json

# Edit it
nano configs/myframework.json

# Use it
nexus-core create --config configs/myframework.json
```

---

## 📁 What You Get

```
output/
├── godot_data/          # Raw scraped data (reusable!)
└── godot/               # The skill
    ├── SKILL.md        # With real code examples!
    └── references/     # Organized docs
```

---

## ❓ Need Help?

See **README.md** for:
- Complete documentation
- Config file structure
- Troubleshooting
- Advanced usage

---

## 🎮 Let's Go!

```bash
# Godot
nexus-core create --config configs/godot.json

# Or interactive
nexus-core create --interactive
```

That's it! 🚀
