...
Nexus Core 可以从 **17 种类型的来源**中提取知识：
...
> **注意：** 在 `selectors` 中省略 `main_content` 可让 Nexus Core 自动检测
...
nexus-core create --setup  # 自动检测 GPU 并安装 PyTorch
...
**要求：**
```bash
pip install nexus-core[video]        # 仅转录
pip install nexus-core[video-full]   # + Whisper、场景检测
```
