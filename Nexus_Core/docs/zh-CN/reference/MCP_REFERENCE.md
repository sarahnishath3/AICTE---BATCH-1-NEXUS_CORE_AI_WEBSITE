...
MCP（Model Context Protocol）允许 Claude Code 等 AI 代理通过标准化接口与 Nexus Core 交互。无需运行 CLI 命令，你可以使用自然语言：
...
| **HTTP** | Cursor、Windsurf、HTTP 客户端 | `nexus-core-mcp --transport http --port 8765` |
...
分类检测）位于 `nexus_core.services` 包中；旧的 `nexus_core.mcp.*`
导入路径作为向后兼容 shim 保留。
