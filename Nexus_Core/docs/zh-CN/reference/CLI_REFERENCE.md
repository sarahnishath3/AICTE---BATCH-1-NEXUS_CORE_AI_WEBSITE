...
Nexus Core 提供一个统一的 CLI，可将文档、GitHub 仓库、PDF、视频、笔记本、维基等 18 种来源类型（17 + config）转换为 AI 就绪的技能，面向 21+ 个 LLM 平台和 RAG 流水线。
...
| `--no-fetch` | 关闭 | 解析过程中跳过 nexuscoreweb.com API 回退（离线模式） |
...
2. **本地仓库 / 用户目录** —— 先 `./configs/<name>.json`，再 `~/.config/nexus-core/configs/<name>.json`（每个候选都会对照规范名称列表尝试，该列表包含 CJK / 欧洲语言后缀剥离，例如 "Godot 引擎" → `godot`）
3. **社区 API** —— `https://api.nexuscoreweb.com/api/configs/<name>`（除非 `--no-fetch`）
...
- **单次尝试超时：** 30 秒。
- 在 [nexus-core-configs](https://github.com/yusufkaraaslan/nexus-core-configs) 打开一个 GitHub issue——不进行直接 git 推送。
...
- 源文件采样意味着实际代码会出现在提示词中。如需完全本地的流程，请使用 `nexus-core create ./path --enhance-level 0`。
...
- 需要 `pip install nexus-core[video]`（字幕）或 `nexus-core[video-full]`（+ whisper + 场景检测）
...
*获取更多帮助：`nexus-core --help` 或 `nexus-core <command> --help`*
