...
Nexus Core 可通过编程方式使用，以便集成到其他工具、自动化脚本和 CI/CD 流水线中。本指南面向希望将 Nexus Core 功能嵌入到自有应用中的开发者，介绍可用的 Python API。

> **稳定性说明 —— 请先阅读**
>
> **PyPI 包的稳定、受支持接口是 `nexus-core` CLI**（以及 MCP 服务器）。本文档介绍的 Python API 真实存在且可以导入 —— 它就是 CLI 运行的同一份代码 —— 但它跟随实现演进：模块路径、签名和配置字典键可能在次要版本之间发生变化。**Semver 保证不覆盖这些内部实现。** 如果你导入这些模块，请固定到精确版本（`nexus-core==3.7.0`），并在升级时重新验证。
...
主要的编程入口与 `nexus-core create` 命令一一对应：工厂函数为 18 种来源类型中的任意一种返回一个 `SkillConverter`，而 `run()` 执行完整的 extract → build 流水线。
...
`SourceDetector` 是 `nexus-core create` 用来从原始输入字符串自动检测来源类型的组件。它返回一个 `SourceInfo` 数据类。
...
其余 15 个转换器遵循相同的模式；各自的模块/类参见 `src/nexus_core/cli/skill_converter.py` 中的 `CONVERTER_REGISTRY`，各自的配置键参见各类的 `__init__`（例如 `word` 读取 `docx_path`，`local` 读取 `directory` 以及 C3.x 的 `detect_patterns`/`extract_test_examples`/…… 开关）。
...
返回字典的键是**平台特定的** —— 请查看具体适配器的 `upload()`（例如 `src/nexus_core/cli/adaptors/claude.py`）了解确切形态。请先检查 `adaptor.supports_upload()`：不支持上传的适配器（例如 `markdown`）会返回一个包含 `"success": False` 和说明性 `"message"` 的结果字典，而不会执行上传。
...
#### Google Gemini 上传 **[network —— 需要 `pip install nexus-core[gemini]`]**
...
#### OpenAI 上传 **[network —— 需要 `pip install nexus-core[openai]`]**
...
`AgentClient`（`nexus_core.cli.agent_client`）路由，它集中处理
...
配置在加载时由 `nexus_core.cli.config_validator.validate_config(config_path)` 验证，CLI 和 `UnifiedScraper` 会替你调用它。
...
不存在 `nexus_core.exceptions` 模块 —— 全程使用标准异常（`ValueError`、`RuntimeError`、`ImportError`、`FileNotFoundError`）。
...
| API | 导入 | 使用场景 |
|-----|--------|----------|
| **技能转换工厂** | `nexus_core.cli.skill_converter.get_converter` | 18 种来源类型中任意一种 → 技能 |
| **转换器注册表** | `nexus_core.cli.skill_converter.CONVERTER_REGISTRY` | 来源类型 → (module, class) 查找 |
| **来源检测** | `nexus_core.cli.source_detector.SourceDetector` | 从原始输入自动检测类型 |
| **网页文档** | `nexus_core.cli.doc_scraper.DocToSkillConverter` | 文档网站 |
| **GitHub 仓库** | `nexus_core.cli.github_scraper.GitHubScraper` | 代码 + 文档 + 社区分析 |
| **PDF** | `nexus_core.cli.pdf_scraper.PDFToSkillConverter` | PDF 文档 |
| **本地代码库** | `nexus_core.cli.codebase_scraper.CodebaseAnalyzer` | 本地目录（C3.x 流水线） |
| **多源** | `nexus_core.cli.unified_scraper.UnifiedScraper` | 合并 18 种来源类型 + 冲突检测 |
| **打包 / 上传 / 增强** | `nexus_core.cli.adaptors.get_adaptor` | 22 个平台目标 |
| **AI 增强** | `nexus_core.cli.agent_client.AgentClient` | API 或本地代理 LLM 调用 |
| **本地代理增强** | `nexus_core.cli.enhance_skill_local.LocalSkillEnhancer` | 通过编码代理免费增强 |
| **设置单例** | `nexus_core.cli.execution_context.ExecutionContext` | 初始化 / 获取 / 覆盖设置 |
| **市场注册表** | `nexus_core.services.marketplace_manager.MarketplaceManager` | 市场 CRUD |
| **配置源** | `nexus_core.services.source_manager.SourceManager` | 配置源注册表 CRUD |
| **配置发布** | `nexus_core.services.config_publisher` | 推送配置；`detect_category()` |
| **Git 配置仓库** | `nexus_core.services.git_repo.GitConfigRepo` | 克隆/拉取 + 配置发现 |
