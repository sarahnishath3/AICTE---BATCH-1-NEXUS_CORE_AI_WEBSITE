...
**使用者：**
- `nexus-core enhance`（API 模式）
- `nexus-core upload`（Claude 目标）
- AI 增强功能
...
**使用者：**
- `nexus-core create` / `scan` / `enhance`（AI 增强，API 模式）
- `nexus-core upload`（OpenAI 目标）
- 向量数据库的 embedding 生成
...
**使用者：**
- `nexus-core create` / `scan` / `enhance`（增强，Kimi/Moonshot）
...
**使用者：**
- `nexus-core create`（GitHub 仓库）
- `nexus-core create --config`（统一多源）
- `nexus-core scan`（本地代码库）
- `nexus-core scan` —— 向社区注册表提交 AI 生成的配置时*必需*（扫描本身无需该令牌即可运行；只是发布提示会被跳过并给出提示信息）
...
**使用者：**
- `nexus-core enhance`
- `nexus-core scan`（可在单次调用中通过 `--agent` 覆盖）
...
**使用者：** 当 `SKILL_SEEKER_AGENT=custom` 时的 `nexus-core create` / `scan` / `enhance`。
...
**用途：** LOCAL 代理增强的递归保护。Nexus Core 会在它生成的每个
本地代理的环境中将此变量设置为 `1`（所有生成路径都如此）；当该变量
已被设置时，LOCAL 增强会拒绝再生成嵌套代理。通常你不需要自己设置它
—— 仅当你将 Nexus Core 包装在另一个代理内、且希望抑制增强时才需要。
