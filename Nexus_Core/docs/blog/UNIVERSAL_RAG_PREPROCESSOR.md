# Nexus Core: The Universal Preprocessor for RAG Systems
...
**Nexus Core is now the universal preprocessing layer for RAG pipelines.** Generate production-ready documentation from any source (websites, GitHub, PDFs, codebases) and export to LangChain, LlamaIndex, Pinecone, or any RAG framework in minutes—not hours.
...
## The Nexus Core Approach (New Reality)
...
Nexus Core sits between your documentation sources and your RAG stack:
...
└──────────────────┬─────────────────────────────────────────┘
                   ...
                   ▼
┌────────────────────────────────────────────────────────────┐
│ Nexus Core (Universal Preprocessor)                     │
│                                                            │
│ Smart Scraping:                                            │
...
**Key insight:** Preprocessing is the same regardless of your RAG stack. Nexus Core handles it once, exports everywhere.
...
**After Nexus Core:**
```bash
nexus-core create --config configs/django.json  # 15 minutes
nexus-core package output/django --target langchain
...
```bash
# Combine all sources
nexus-core create --config \
  --docs-config configs/internal-docs.json \
  --github internal/repos \
  --name knowledge-base

nexus-core package output/knowledge-base --target llama-index
...
nexus-core package output/fastapi --target markdown
...
Under the hood, Nexus Core uses a **platform adaptor pattern** (Strategy Pattern) to support multiple RAG frameworks:
...
| Task | Manual | Nexus Core | Time Saved |
|------|--------|---------------|------------|
...
| Metric | Manual | Nexus Core | Improvement |
|--------|--------|---------------|-------------|
...
nexus-core package output/my-framework --target llama-index
```
...
nexus-core enhance output/fastapi-complete

# Package for multiple platforms
nexus-core package output/fastapi-complete --target langchain
nexus-core package output/fastapi-complete --target llama-index
nexus-core package output/fastapi-complete --target markdown
```
...
for i, doc in enumerate(documents):
    ...
    index.upsert(vectors=[{
        "id": f"doc_{i}",
        "values": embedding,
        "metadata": doc["metadata"]  # Nexus Core metadata preserved!
    }])
```
...
Nexus Core is evolving from "Claude Code skill generator" to **universal RAG infrastructure**. Here's what's coming:
...
**Nexus Core will become the data layer for AI systems:**

```
Documentation → [Nexus Core] → RAG Systems
...
Nexus Core is **open source** and **community-driven**. We're building the infrastructure layer for the AI age.

**Get Involved:**

- ⭐ **Star on GitHub:** [github.com/yusufkaraaslan/Nexus_Core](https://github.com/yusufkaraaslan/Nexus_Core)
...
- 📰 **Website:** [nexuscoreweb.com](https://nexuscoreweb.com/)
- 🐦 **Twitter:** [@_yUSyUS_](https://x.com/_yUSyUS_)
- 📦 **PyPI:** `pip install nexus-core`
