...
MCP (Model Context Protocol) lets AI agents like Claude Code control Nexus Core through natural language:
...
```bash
# In Claude Code, run:
/claude add-mcp-server nexus-core
```
...
```bash
# Debug mode
nexus-core-mcp --verbose

# Custom port
nexus-core-mcp --port 8080
...
# Update
pip install --upgrade nexus-core[mcp]
```
