...
Combine multiple sources into a single, comprehensive skill. Nexus Core supports **18 source types** that can be freely mixed and matched:
...
```bash
nexus-core create --config react-complete.json
```
...
nexus-core create --config react-complete.json --fresh

# Dry run
nexus-core create --config react-complete.json --dry-run

# Rule-based merging
nexus-core create --config react-complete.json --merge-mode rule-based
```
...
```bash
nexus-core create --config my-config.json --merge-mode claude-enhanced
```
...
```bash
nexus-core create --config my-config.json --merge-mode rule-based
```
...
```bash
nexus-core create --config my-config.json \
