...
> **Status: IMPLEMENTED** — `nexus-core create --setup` (see `video_setup.py`, 835 lines, 60 tests)
...
nexus-core-video = "nexus_core.cli.video_scraper:main"      # NEW
```
...
def check_video_dependencies(require_full: bool = False) -> None:
    ...
    if missing:
        raise ImportError(
            ...
            f"Install with: pip install nexus-core[video]"
        )
    ...
    if require_full:
        ...
        if full_missing:
            raise ImportError(
                ...
                f"Install with: pip install nexus-core[video-full]"
            )
```
...
nexus-core config --check-video

# Output:
# Video Dependencies:
#   yt-dlp              ✅ 2025.01.15
#   youtube-transcript-api ✅ 1.2.3
#   faster-whisper      ❌ Not installed (pip install nexus-core[video-full])
...
```dockerfile
# Tier 1 (lightweight)
RUN pip install nexus-core[video]

# Tier 2 (full)
RUN apt-get update && apt-get install -y ffmpeg
RUN pip install nexus-core[video-full]
