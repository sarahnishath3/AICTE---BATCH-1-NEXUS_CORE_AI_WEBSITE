...
**Document:** 04 of 07
**Status:** Implemented
**Note:** Video support is live in Nexus Core v3.6.0 (`video_scraper.py`, `video_models.py`, `video_setup.py`).
...
nexus-core create --video-url  https://youtube.com/watch?v=abc123
nexus-core create --video-playlist https://youtube.com/playlist?list=PLxxx
nexus-core create --video-url --channel https://youtube.com/@channelname
nexus-core create --video-url --path ./recording.mp4
nexus-core create --video-url --directory ./recordings/

# With options
nexus-core create --video-url  <URL> \
...
nexus-core create https://youtube.com/watch?v=abc123
nexus-core create https://youtu.be/abc123
nexus-core create https://youtube.com/playlist?list=PLxxx
nexus-core create https://youtube.com/@channelname
nexus-core create https://vimeo.com/123456789
nexus-core create ./tutorial.mp4
nexus-core create ./recordings/                # Directory of videos

# With universal flags
nexus-core create https://youtube.com/watch?v=abc123 --visual -p comprehensive
nexus-core create ./tutorial.mp4 --enhance-level 2 --dry-run
```

### Registration in main.py

```python
# In src/nexus_core/cli/main.py - COMMAND_MODULES dict

COMMAND_MODULES = {
    # ... existing commands ...
    'video': 'nexus_core.cli.video_scraper',
    # ... rest of commands ...
}
```
...
class SourceDetector:
    ...
    def detect(cls, source: str) -> SourceInfo:
        ...
        raise ValueError(
            ...
            "Examples:\n"
            "  Web:      nexus-core create https://docs.react.dev/\n"
            "  GitHub:   nexus-core create facebook/react\n"
            "  Local:    nexus-core create ./my-project\n"
            "  PDF:      nexus-core create tutorial.pdf\n"
            "  DOCX:     nexus-core create document.docx\n"
            "  Video:    nexus-core create https://youtube.com/watch?v=xxx\n"  # NEW
            "  Playlist: nexus-core create https://youtube.com/playlist?list=xxx\n"  # NEW
            "  Config:   nexus-core create configs/react.json"
        )
...
def _scrape_video_source(self, source: dict, source_index: int) -> dict:
    ...
    """
    from nexus_core.cli.video_scraper import VideoScraper
...
```python
# In src/nexus_core/cli/create_command.py (or equivalent in main.py)
...
```python
# src/nexus_core/cli/parsers/video_parser.py

from nexus_core.cli.parsers.base import SubcommandParser
...
class VideoParser(SubcommandParser):
    ...
    def add_arguments(self, parser):
        ...
        from nexus_core.cli.arguments.common import add_all_standard_arguments
...
```python
# src/nexus_core/cli/arguments/video.py
...
```python
# In src/nexus_core/mcp/tools/scraping_tools.py
...
|------|---------|---------------|
| `src/nexus_core/cli/video_scraper.py` | Main video scraper orchestrator | ~800-1000 lines |
| `src/nexus_core/cli/video_models.py` | All data classes and enums | ~500-600 lines |
| `src/nexus_core/cli/video_transcript.py` | Transcript extraction (YouTube API + Whisper) | ~400-500 lines |
| `src/nexus_core/cli/video_visual.py` | Visual extraction (scene detection + OCR) | ~500-600 lines |
| `src/nexus_core/cli/video_segmenter.py` | Segmentation and stream alignment | ~400-500 lines |
| `src/nexus_core/cli/parsers/video_parser.py` | CLI argument parser | ~80-100 lines |
| `src/nexus_core/cli/arguments/video.py` | Video-specific argument definitions | ~120-150 lines |
...
| File | Changes |
|------|---------|
| `src/nexus_core/cli/source_detector.py` | Add video URL patterns, video file detection, video directory detection |
| `src/nexus_core/cli/main.py` | Register `video` subcommand in COMMAND_MODULES |
| `src/nexus_core/cli/unified_scraper.py` | Add `"video": []` to scraped_data, add `_scrape_video_source()` |
| `src/nexus_core/cli/arguments/create.py` | Add video args to create command, add `--help-video` |
| `src/nexus_core/cli/parsers/__init__.py` | Register VideoParser |
| `src/nexus_core/cli/config_validator.py` | Validate video source entries in unified config |
| `src/nexus_core/mcp/tools/scraping_tools.py` | Add `scrape_video` tool |
| `pyproject.toml` | Add `[video]` and `[video-full]` optional dependencies, add `nexus-core-video` entry point |
