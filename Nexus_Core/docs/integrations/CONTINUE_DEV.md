# Using Nexus Core with Continue.dev
...
Use Nexus Core to create **custom context providers** for Continue.dev:
...
```bash
# Start Nexus Core MCP server
nexus-core-mcp --transport http --port 8765

# Or as systemd service (Linux)
sudo systemctl enable nexus-core-mcp
sudo systemctl start nexus-core-mcp
```
...
```json
{
  "mcpServers": {
    "nexus-core": {
      "command": "python",
      "args": ["-m", "nexus_core.mcp.server_fastmcp", "--transport", "stdio"]
    }
  }
}
```
...
from nexus_core.cli.doc_scraper import load_skill
...
from nexus_core.cli.package_skill import main as package
...
### Continue + Nexus Core MCP Integration

**Full MCP Setup**

```bash
# Install Nexus Core with MCP
pip install nexus-core[mcp]

# Start MCP server
python -m nexus_core.mcp.server_fastmcp --transport stdio
```
...
systemctl restart nexus-core-context-server
...
**Error:**
> "Failed to start MCP server: nexus-core"
...
1. **Try another framework:** `nexus-core create --config configs/react.json`
