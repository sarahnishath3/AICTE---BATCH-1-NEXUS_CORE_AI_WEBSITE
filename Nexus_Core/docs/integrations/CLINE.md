# Using Nexus Core with Cline (VS Code Extension)
...
Use Nexus Core to create **custom rules and MCP tools** for Cline:
...
1. **Install Nexus Core MCP server:**
...
"Use the nexus-core MCP tool to scrape React documentation
...
```python
# setup_cline_rag.py
from nexus_core.cli.doc_scraper import main as scrape
...
```
In Cline:
"Use nexus-core MCP to search Django 5.0 async views documentation"
...
**Error:**
> "Failed to connect to MCP server: nexus-core"
...
1. **Try another framework:** `nexus-core create --config configs/fastapi.json`
