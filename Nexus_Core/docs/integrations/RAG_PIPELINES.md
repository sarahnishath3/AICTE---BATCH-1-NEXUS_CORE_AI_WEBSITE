# Building RAG Pipelines with Nexus Core
...
## ✨ Nexus Core: Universal RAG Preprocessor

Nexus Core automates the **hardest part of RAG**: documentation preparation.
...
└───────────────────┬─────────────────────────────────────────────┘
                    ...
                    ▼
┌─────────────────────────────────────────────────────────────────┐
│ Nexus Core (Preprocessing Engine)                            │
...
for i, doc in enumerate(documents):
    ...
    index.upsert(vectors=[{
        ...
        "metadata": {
            ...
            **doc["metadata"]  # Nexus Core metadata preserved
        }
    }])
...
# Load multiple sources (all preprocessed by Nexus Core)
sources = {
...
for customer in customers:
    # Load customer-specific docs (generated by Nexus Core)
...
**Deployment:**
```bash
# 1. Preprocess docs with Nexus Core
nexus-core create --config configs/product-docs.json
nexus-core package output/product-docs --target langchain
...
# Load documents on startup (from Nexus Core output)
...
Nexus Core provides **smart chunking** based on content type:

```python
# Nexus Core automatically:
...
### Preprocessing Time (Nexus Core)

| Documentation Size | Pages | Nexus Core Time | Manual Time (Est.) |
|-------------------|-------|-------------------|-------------------|
...
| Nexus Core RAG | 9.2 | 95% accurate |
...
nexus-core create --config configs/product-a.json
nexus-core create --config configs/product-b.json
# ... repeat for all products

# 2. Package for LangChain
for product in product-a product-b product-c product-d product-e; do
  nexus-core package output/$product --target langchain
done
...
for merchant in merchants/*; do
  nexus-core scan  $merchant/docs
  nexus-core package output/$merchant --target langchain
done
...
nexus-core create --config configs/internal-kb-unified.json --name internal-kb

# 2. Package for LlamaIndex
nexus-core package output/internal-kb --target llama-index
...
6. **Update regularly** - Automate doc refresh with Nexus Core

---

**Last Updated:** February 5, 2026
**Tested With:** LangChain 0.1.0+, LlamaIndex 0.10.0+, Pinecone 3.0+
**Nexus Core Version:** v3.6.0
