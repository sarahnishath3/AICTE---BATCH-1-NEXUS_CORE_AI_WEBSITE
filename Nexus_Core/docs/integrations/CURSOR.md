# Using Nexus Core with Cursor IDE
...
Use Nexus Core to create **custom documentation** for Cursor's AI:
...
nexus-core create /path/to/repo --preset comprehensive
nexus-core package output/codebase --target markdown
```
...
Cursor has a **200KB limit** for .cursorrules. Nexus Core markdown output is optimized, but for very large documentation:

**Strategy 1: Summarize (Recommended)**
```bash
# Use AI enhancement to create concise version
nexus-core enhance output/django --mode LOCAL
...
python -m nexus_core.cli.split_config configs/django.json --strategy router
...
nexus-core package output/react --target markdown
nexus-core package output/postgresql --target markdown
...
0 0 1 * * cd ~/projects && nexus-core create --config configs/django.json && nexus-core package output/django --target markdown && cp output/django-markdown/SKILL.md ~/.cursorrules
```
...
| Aspect | Without Nexus Core | With Nexus Core |
|--------|---------------------|-------------------|
