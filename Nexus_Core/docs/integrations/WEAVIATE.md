# Weaviate Integration with Nexus Core
...
Nexus Core automates Weaviate integration with structured, production-ready data:
...
### Step 2: Generate Nexus Core Documents
...
```bash
nexus-core create  django/django --name django
nexus-core package output/django --target langchain
```

**Option C: Local Codebase**
```bash
nexus-core scan  /path/to/repo
nexus-core package output/codebase --target langchain
```

**Option D: RAG-Optimized Chunking**
```bash
nexus-core create --config configs/fastapi.json --chunk-for-rag --chunk-tokens 512
nexus-core package output/fastapi --target langchain
```
...
# Define schema with all Nexus Core metadata
schema = {
    "class": "Documentation",
    "description": "Framework documentation from Nexus Core",
    "vectorizer": "text2vec-openai",
    "moduleConfig": {
        "text2vec-openai": {
            "model": "ada-002",
...
| **Data Ingestion** | Custom scraping + parsing logic | One command: `nexus-core create` |
...
- **Support:** https://github.com/yusufkaraaslan/Nexus_Core/discussions
