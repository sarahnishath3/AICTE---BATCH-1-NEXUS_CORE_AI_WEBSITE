# Using Nexus Core with Haystack
...
Use Nexus Core as **essential preprocessing** before Haystack:
...
**Result:**
Nexus Core outputs JSON files with Haystack Document format (`content` + `meta`), ready to load directly into your Haystack pipelines.
...
nexus-core package output/django --target haystack
...
Nexus Core supports multiple documentation sources:
...
# GitHub repository
nexus-core create  tiangolo/fastapi

# PDF documentation
nexus-core create --pdf docs/manual.pdf

# Combine multiple sources via a unified config (sources array with docs + github)
nexus-core create --config configs/fastapi-unified.json \
...
```bash
# Generate Haystack Documents
nexus-core package output/my-framework --target haystack

# With semantic chunking for better retrieval
nexus-core create --config configs/my-framework.json --chunk-for-rag
nexus-core package output/my-framework --target haystack
...
nexus-core create --config configs/complete-knowledge.json \
  --output output/complete-knowledge

nexus-core package output/complete-knowledge --target haystack
...
nexus-core update output/fastapi/ --check-changes
...
nexus-core package output/fastapi --target haystack
```
...
nexus-core create --config configs/fastapi.json --chunk-for-rag
...
```bash
pip install nexus-core haystack-ai
nexus-core create --config configs/your-framework.json --chunk-for-rag
nexus-core package output/your-framework --target haystack
```
