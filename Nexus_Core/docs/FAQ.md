...
**Tool:** Yes, Nexus Core is 100% free and open-source (MIT license).
...
Nexus Core supports **18 source types**:
...
### Why do I get "No module named 'nexus_core'" error?
...
**Using custom URL:**
```bash
nexus-core create https://docs.example.com --name my-framework
```

**From custom config file:**
```bash
nexus-core create --config configs/my-framework.json
```

### How do I create skills for an entire project at once?

Use `nexus-core scan` — an AI agent inspects the project's manifests, README, Dockerfile/CI, and source imports, then emits one config per detected framework plus a `<project>-codebase.json` for your own code:
...
Yes! Nexus Core has powerful GitHub analysis:
...
nexus-core create https://github.com/vercel/next.js --depth full
```
...
nexus-core create --pdf document.pdf
```
...
nexus-core create --space-key  MYSPACE --name my-wiki

# From a Confluence HTML/XML export directory
nexus-core create --conf-export-path  ./confluence-export --name my-wiki
```
...
nexus-core create --database-id  DATABASE_ID --name my-notes

# From a Notion HTML/Markdown export directory
nexus-core create --notion-export-path  ./notion-export --name my-notes
```
...
```bash
# Word document
nexus-core create report.docx --name quarterly-report

# EPUB book
nexus-core create handbook.epub --name dev-handbook

# PowerPoint presentation
nexus-core create slides.pptx --name training-deck

# Or use auto-detection for any of them
nexus-core create report.docx
nexus-core create handbook.epub
...
```bash
# RSS/Atom feed
nexus-core create https://blog.example.com/feed.xml --name blog-feed

# Man page
nexus-core create grep.1 --name grep-manual
```
...
nexus-core create --chat-export-path ./slack-export --platform slack --name team-knowledge

# From a Discord export directory
nexus-core create --chat-export-path ./discord-export --platform discord --name server-archive
```
...
for platform in claude gemini openai; do
  nexus-core install --config react --target $platform
done
```
...
```bash
nexus-core install-agent output/react/ --agent claude
```
...
nexus-core create --directory /path/to/repo

# Skip specific features
nexus-core create --directory . --skip-patterns --skip-how-to-guides
```
...
```bash
python -m nexus_core.cli.generate_router configs/godot_*.json --name godot
```
...
2. Report issue on [GitHub](https://github.com/yusufkaraaslan/Nexus_Core/issues)
...
```
Use nexus-core MCP tools to scrape React documentation
```
...
### Can I use Nexus Core programmatically?

Yes! Full API for Python integration:

```python
from nexus_core.cli.doc_scraper import scrape_all, build_skill
...
```bash
nexus-core create --config configs/your-framework.json
```
3. **Submit PR** on [GitHub](https://github.com/yusufkaraaslan/Nexus_Core)
...
# Single page test
nexus-core create https://docs.example.com/intro --max-pages 1

# Check config (also available as the validate_config MCP tool)
python -m nexus_core.cli.config_validator configs/react.json
```
...
1. **Check existing issues:** https://github.com/yusufkaraaslan/Nexus_Core/issues
2. **Create new issue** with:
   - Nexus Core version (`nexus-core --version`)
...
2. **Create feature request:** https://github.com/yusufkaraaslan/Nexus_Core/issues
3. **Join discussions:** https://github.com/yusufkaraaslan/Nexus_Core/discussions

### Is there a community?

Yes!
- **GitHub Discussions:** https://github.com/yusufkaraaslan/Nexus_Core/discussions
- **Issue Tracker:** https://github.com/yusufkaraaslan/Nexus_Core/issues
...
**Version:** 3.6.0
**Last Updated:** 2026-03-15
**Questions? Ask on [GitHub Discussions](https://github.com/yusufkaraaslan/Nexus_Core/discussions)**
