...
> 🗄️ **Historical — completed** — This plan was fully executed. EPUB input support is live in Nexus Core v3.6.0 (`epub_scraper.py`, `arguments/epub.py`, `parsers/epub_parser.py`, 107+ tests).

# Add EPUB Input Support — Implementation Plan

## Overview

Add `.epub` as an input format for Nexus Core, enabling `nexus-core create book.epub` and `nexus-core epub --epub book.epub`. Follows the established Word/PDF scraper pattern: source detection → routing → extraction → categorize → build skill.
...
Running `nexus-core create book.epub` produces:
```
output/book/
...
```
$ nexus-core create programming-rust.epub
...
📦 Next step: Package with: nexus-core package output/programming-rust/
```

### Verification:
- [x] `nexus-core create book.epub` produces valid output directory
- [x] `nexus-core create book.epub --name mybook` works standalone
- [x] `nexus-core create book.epub --dry-run` shows config without processing
...
#### [x] 2. Create `src/nexus_core/cli/epub_scraper.py`
**File**: `src/nexus_core/cli/epub_scraper.py` (new)
...
Usage:
    nexus-core create book.epub --name myskill
    nexus-core create book_extracted.json
"""
...
def _check_epub_deps():
    ...
    if not EPUB_AVAILABLE:
        raise RuntimeError(
            ...
            'Install with: pip install "nexus-core[epub]"\n'
...
def main():
    ...
    try:
        ...
        from nexus_core.cli.workflow_runner import run_workflows
        run_workflows(args)
...
- [x] `ruff check src/nexus_core/cli/epub_scraper.py` passes
- [x] `ruff format --check src/nexus_core/cli/epub_scraper.py` passes
- [ ] `mypy src/nexus_core/cli/epub_scraper.py` passes (continue-on-error)
...
**File**: `src/nexus_core/cli/source_detector.py`
...
```python
"  EPUB:  nexus-core create ebook.epub\n"
```

#### [x] 2. Argument definitions
**File**: `src/nexus_core/cli/arguments/epub.py` (new)
...
**File**: `src/nexus_core/cli/arguments/create.py`
...
#### [x] 4. Parser class
**File**: `src/nexus_core/cli/parsers/epub_parser.py` (new)
**Changes**: Subcommand parser for standalone `nexus-core epub` command

```python
"""Parser for epub subcommand."""

from .base import SubcommandParser
from nexus_core.cli.arguments.epub import add_epub_arguments
...
**File**: `src/nexus_core/cli/parsers/__init__.py`
...
**File**: `src/nexus_core/cli/create_command.py`
...
def _route_epub(self) -> int:
    ...
    from nexus_core.cli import epub_scraper
...
Add after `nexus-core-word` (line 224):
```toml
nexus-core-epub = "nexus_core.cli.epub_scraper:main"
```
...
#### Automated Verification:
- [x] `ruff check src/nexus_core/cli/source_detector.py src/nexus_core/cli/arguments/epub.py src/nexus_core/cli/parsers/epub_parser.py src/nexus_core/cli/create_command.py` passes
- [x] `ruff format --check src/nexus_core/cli/` passes
- [x] `pip install -e ".[epub]"` installs with all entry points
- [x] `nexus-core create --help` shows EPUB-specific help
- [x] `nexus-core create --help-epub` shows EPUB arguments (via standalone entry point `nexus-core-create`)
- [x] `nexus-core create nonexistent.epub` gives clear error about missing file
...
#### Manual Verification:
- [x] `nexus-core --help` lists `epub` command
- [x] `nexus-core create book.epub --dry-run` shows dry run output
...
try:
    from nexus_core.cli.epub_scraper import (
...
- Optional dependency: `pip install "nexus-core[epub]"`
```
...
2. `nexus-core create book.epub` with a real EPUB file — verify output directory structure
3. `nexus-core create book.epub --dry-run` — verify dry run output
4. `nexus-core create drm-book.epub` — verify DRM error message
5. `nexus-core create nonexistent.epub` — verify file-not-found error
...
- Similar implementation: `src/nexus_core/cli/word_scraper.py` — closest analog
