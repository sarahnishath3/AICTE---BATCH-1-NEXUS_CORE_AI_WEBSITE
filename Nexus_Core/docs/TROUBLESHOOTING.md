...
Comprehensive guide for diagnosing and resolving common issues with Nexus Core.
...
```bash
$ nexus-core --version
bash: nexus-core: command not found
```
...
python3.12 -m pip install nexus-core
...
# Use absolute path
nexus-core create --config /full/path/to/configs/react.json

# Create config directory
mkdir -p ~/.config/nexus-core/configs

# Copy config
cp configs/react.json ~/.config/nexus-core/configs/

# List available configs
nexus-core-config list
```
...
nexus-core-validate configs/myconfig.json
...
nexus-core create --config config.json --verbose
...
nexus-core create --config config.json --async

# Reduce max pages
nexus-core create --config config.json --max-pages 100
...
nexus-core create --config config.json
```
...
# Debug URL discovery
nexus-core create --config config.json --dry-run --verbose
```
...
nexus-core create  facebook/react  # Correct
nexus-core create  Facebook/React  # Wrong

# Check if repo is private (requires token)
export GITHUB_TOKEN=ghp_...
nexus-core create  private/repo
...
nexus-core create --config config.json --enhance-level 0
...
for dir in output/*/; do
  nexus-core enhance "$dir" --mode LOCAL --background
done
```
...
nexus-core create --config config.json --workers 2
...
python -m cProfile -o profile.stats \
  -m nexus_core.cli.doc_scraper --config config.json
```
...
nexus-core-cloud --provider s3 --bucket my-skills-bucket upload output/react/

# Clear cached scrape data for a skill
nexus-core create --config config.json --fresh
```
...
# Save logs to file
nexus-core create --config config.json 2>&1 | tee debug.log
```
...
# Test scraper
python -c "
from nexus_core.cli.doc_scraper import scrape_all
...
# Test GitHub API
python -c "
from nexus_core.cli.github_fetcher import GitHubFetcher
...
# Test embeddings
python -c "
from nexus_core.embedding.generator import EmbeddingGenerator
...
1. **Search existing issues:** https://github.com/yusufkaraaslan/Nexus_Core/issues
2. **Check documentation:** https://nexuscoreweb.com/
3. **Ask on GitHub Discussions:** https://github.com/yusufkaraaslan/Nexus_Core/discussions
...
# Test connection
nexus-core create --space-key  MYSPACE --dry-run
...
# Test connection
nexus-core create --database-id  DATABASE_ID --dry-run
```
...
nexus-core create notebook.ipynb --verbose
```
...
# For remote specs
nexus-core create https://api.example.com/openapi.json --name my-api
```
...
nexus-core create book.epub --verbose
...
nexus-core create --chat-export-path ./slack-export --platform slack
nexus-core create --chat-export-path ./discord-export --platform discord
...
|-------|-------|----------|
| `ModuleNotFoundError` | Package not installed | `pip install nexus-core` |
