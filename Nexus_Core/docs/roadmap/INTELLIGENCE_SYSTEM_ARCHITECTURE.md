# Nexus Core Intelligence System - Technical Architecture
...
The **Nexus Core Intelligence System** is a multi-layered architecture that automatically generates, updates, and intelligently loads codebase knowledge into Claude Code's context.
...
│                     STORAGE LAYER                           │
│  ┌──────────────────────────────────────────────────────┐   │
│  │ • Skill Files (.nexus-core/skills/)               │   │
│  │ • Embeddings Cache (.nexus-core/cache/)           │   │
│  │ • Metadata (.nexus-core/registry.json)            │   │
│  │ • Git Hooks (.nexus-core/hooks/)                  │   │
│  └──────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```
...
```
project-root/
├── .nexus-core/                    # Intelligence system directory
...
```python
# src/nexus_core/intelligence/project_manager.py
...
class ProjectManager:
    ...
    def __init__(self, project_root: Path):
        ...
        self.config_path = project_root / ".nexus-core" / "config.yml"
        self.skills_dir = project_root / ".nexus-core" / "skills"
    ...
    def _create_directories(self):
        """Create .nexus-core directory structure"""
        dirs = [
            ".nexus-core",
            ".nexus-core/skills",
            ".nexus-core/skills/libraries",
            ".nexus-core/skills/codebase",
            ".nexus-core/cache",
            ".nexus-core/cache/embeddings",
            ".nexus-core/cache/metadata",
            ".nexus-core/hooks",
            ".nexus-core/logs",
        ]
...
# Auto-generated by nexus-core
# DO NOT EDIT - regenerate with: nexus-core init-project

CURRENT_BRANCH=$(git rev-parse --abbrev-ref HEAD)
CONFIG_FILE=".nexus-core/config.yml"
...
if echo "$WATCH_BRANCHES" | grep -q "^$CURRENT_BRANCH$"; then
    ...
    nexus-core regenerate-skills --branch "$CURRENT_BRANCH" --silent
    echo "✅ Skills updated"
fi
"""
...
```python
# src/nexus_core/intelligence/modular_generator.py
...
class ModularSkillGenerator:
    ...
    def _generate_skill(self, module_name: str, skill_config: dict) -> Path:
        ...
        from nexus_core.cli.codebase_scraper import analyze_codebase
        ...
        # 4. Save skill file
        output_dir = self.root / ".nexus-core" / "skills" / "codebase" / module_name
...
```python
# src/nexus_core/intelligence/import_clustering.py
...
```python
# src/nexus_core/intelligence/embedding_clustering.py
...
```python
# src/nexus_core/intelligence/hybrid_clustering.py
...
class SkillSeekersIntelligenceAgent:
    ...
    async def regenerate_skills(self, branch: str):
        ...
        # Run: nexus-core regenerate-skills --branch {branch}
        import subprocess

        result = subprocess.run(
            ["nexus-core", "regenerate-skills", "--branch", branch, "--silent"],
            capture_output=True,
            text=True
        )
...
5. **Cache:** Stored locally in `.nexus-core/cache/`
...
- **Existing Code:** `src/nexus_core/cli/codebase_scraper.py` (C3.x features)
