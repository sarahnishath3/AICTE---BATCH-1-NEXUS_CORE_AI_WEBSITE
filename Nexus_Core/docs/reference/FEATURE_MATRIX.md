...
nexus-core install --config django --target gemini

# Install FastAPI skill for OpenAI
nexus-core install --config fastapi --target openai

# Install Vue skill as generic markdown
nexus-core install --config vue --target markdown
```

### Split Unified Config by Source
```bash
# Split multi-source config into separate configs
python -m nexus_core.cli.split_config configs/react_unified.json --strategy source
...
nexus-core create --config react-documentation.json
nexus-core create --config react-github.json
