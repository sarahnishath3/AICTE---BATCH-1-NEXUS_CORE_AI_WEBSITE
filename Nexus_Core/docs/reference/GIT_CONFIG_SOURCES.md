...
User → fetch_config(source="team", config_name="react-custom")
    ↓
SourceManager (~/.nexus-core/sources.json)
    ↓
GitConfigRepo (clone/pull with GitPython)
    ↓
Local cache (~/.nexus-core/cache/team/)
    ↓
...
1. **API Mode** (existing, unchanged)
   ...
   - Fetches from api.nexuscoreweb.com
...
**Cache Directory:**
```
$SKILL_SEEKERS_CACHE_DIR  (default: ~/.nexus-core/cache/)
```
...
**Note:** Does NOT delete cached git repository data. To free disk space, manually delete `~/.nexus-core/cache/{source_name}/`
...
cp ../Nexus_Core/configs/react.json ./react-internal.json
...
# Free disk space
rm -rf ~/.nexus-core/cache/old-team/
```
...
2. Manual cache clear:
   ```bash
   rm -rf ~/.nexus-core/cache/team/
   ```

3. Check auto-pull worked:
   ```bash
   cd ~/.nexus-core/cache/team
...
```bash
export SKILL_SEEKERS_CACHE_DIR=/mnt/large-disk/nexus-core-cache
```

Or pass to GitConfigRepo:
```python
from nexus_core.mcp.git_repo import GitConfigRepo
  ...
  generate:
    ...
    steps:
      ...
      - name: Register config source
        ...
        run: |
          python3 << EOF
          from nexus_core.mcp.source_manager import SourceManager
          sm = SourceManager()
          sm.add_source(
              name="team",
              git_url="https://github.com/myorg/configs.git"
      ...
      - name: Fetch and use config
        ...
        run: |
          ...
          nexus-core create --config <fetched_config>
```
...
**Location:** `src/nexus_core/mcp/git_repo.py`
...
**Location:** `src/nexus_core/mcp/source_manager.py`
...
**Questions?** Open an issue at https://github.com/yusufkaraaslan/Nexus_Core/issues
