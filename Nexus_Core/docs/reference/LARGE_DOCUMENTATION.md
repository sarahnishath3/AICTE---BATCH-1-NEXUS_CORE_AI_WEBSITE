...
python -m nexus_core.cli.split_config configs/godot.json --strategy category
...
python -m nexus_core.cli.split_config configs/godot.json --strategy router
...
python -m nexus_core.cli.split_config configs/bigdocs.json --strategy size --target-pages 5000
...
nexus-core estimate configs/godot.json
...
for config in configs/godot-*.json; do
  nexus-core create --config $config &
done
wait

# 5. Generate router
nexus-core create configs/godot-*.json

# 6. Package all
nexus-core package output/godot*/
...
```bash
python -m nexus_core.cli.split_config configs/godot.json --strategy router --target-pages 5000
...
nexus-core create --config configs/godot-scripting.json &
nexus-core create --config configs/godot-2d.json &
nexus-core create --config configs/godot-3d.json &
nexus-core create --config configs/godot-physics.json &
nexus-core create --config configs/godot-shaders.json &
...
```bash
# 1. Split
python -m nexus_core.cli.split_config configs/vue.json --strategy category
...
python -m nexus_core.cli.split_config config.json --target-pages 3000

# Medium skills (5K-8K pages) - balanced (RECOMMENDED)
python -m nexus_core.cli.split_config config.json --target-pages 5000

# Larger skills (8K-10K pages) - fewer skills, broader
python -m nexus_core.cli.split_config config.json --target-pages 8000
```
...
nexus-core create --config config.json --resume
```
...
python -m nexus_core.cli.split_config configs/aws.json --strategy router --target-pages 5000
...
python -m nexus_core.cli.split_config configs/microsoft.json --strategy category
...
nexus-core create --config config1.json &
nexus-core create --config config2.json &
wait

nexus-core create --config config3.json &
nexus-core create --config config4.json &
wait
```
...
1. ✅ **Estimate first**: `nexus-core estimate config.json`
2. ✅ **Split with router**: `python -m nexus_core.cli.split_config config.json --strategy router`
3. ✅ **Scrape in parallel**: Multiple terminals or background jobs
4. ✅ **Generate router**: `nexus-core create configs/*-*.json`
5. ✅ **Package all**: `nexus-core package output/*/`
