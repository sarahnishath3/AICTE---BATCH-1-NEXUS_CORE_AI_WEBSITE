...
MCP (Model Context Protocol) allows AI agents like Claude Code to interact with Nexus Core through a standardized interface. Instead of running CLI commands, you can use natural language:
...
|------|----------|---------|
| **stdio** | Claude Code, VS Code + Cline | `nexus-core-mcp` |
| **HTTP** | Cursor, Windsurf, HTTP clients | `nexus-core-mcp --transport http --port 8765` |
...
# With custom host
nexus-core-mcp --transport http --host 0.0.0.0 --port 8765
```
...
`nexus_core.services` package; the old `nexus_core.mcp.*` import paths
