...
Nexus Core can be used programmatically for integration into other tools, automation scripts, and CI/CD pipelines. This guide covers the Python APIs available for developers who want to embed Nexus Core functionality into their own applications.

> **Stability note — read this first**
>
> The **stable, supported interface of the PyPI package is the `nexus-core` CLI** (and the MCP server). The Python API documented here is real and importable — it is the same code the CLI runs — but it tracks the implementation: module paths, signatures, and config-dict keys may change between minor releases. **Semver guarantees do not extend to these internals.** If you import these modules, pin an exact version (`nexus-core==3.7.0`) and re-verify on upgrade.
...
The primary programmatic entry point mirrors the `nexus-core create` command: a factory returns a `SkillConverter` for any of the 18 source types, and `run()` executes the full extract → build pipeline.

```python
from nexus_core.cli.skill_converter import get_converter, CONVERTER_REGISTRY
...
```python
from nexus_core.cli.skill_converter import get_converter
...
`SourceDetector` is what `nexus-core create` uses to auto-detect the source type from a raw input string. It returns a `SourceInfo` dataclass.
...
```python
from nexus_core.cli.pdf_scraper import PDFToSkillConverter
...
```python
from nexus_core.cli.github_scraper import GitHubScraper
...
The remaining 15 converters follow the same pattern; see `CONVERTER_REGISTRY` in `src/nexus_core/cli/skill_converter.py` for the module/class of each, and each class's `__init__` for its config keys (e.g. `word` reads `docx_path`, `local` reads `directory` + the C3.x `detect_patterns`/`extract_test_examples`/… toggles).
...
```python
from nexus_core.cli.unified_scraper import UnifiedScraper
...
from nexus_core.cli.adaptors import get_adaptor, ADAPTORS
...
from pathlib import Path
from nexus_core.cli.adaptors import get_adaptor
from nexus_core.cli.adaptors.base import SkillAdaptor, SkillMetadata
...
#### Google Gemini Upload **[network — requires `pip install nexus-core[gemini]`]**
...
#### OpenAI Upload **[network — requires `pip install nexus-core[openai]`]**
...
through the shared `AgentClient` (`nexus_core.cli.agent_client`), which
...
```python
from nexus_core.cli.agent_client import AgentClient
...
```python
from nexus_core.cli.enhance_skill_local import LocalSkillEnhancer
...
### 9. Services Layer (`nexus_core.services`)
...
from nexus_core.services.config_publisher import ConfigPublisher, detect_category
...
```python
mm = MarketplaceManager()        # or MarketplaceManager(config_dir="~/.nexus-core")
...
Configs are validated on load by `nexus_core.cli.config_validator.validate_config(config_path)`, which the CLI and `UnifiedScraper` call for you.
...
There is no `nexus_core.exceptions` module — standard exceptions (`ValueError`, `RuntimeError`, `ImportError`, `FileNotFoundError`) are used throughout.
  ...
  generate-skills:
    ...
    steps:
      ...
      - name: Install Nexus Core
        run: pip install nexus-core[all-llms]
      ...
      - name: Generate Skills
        ...
        run: |
          ...
          nexus-core install --config vue --target gemini
...
generate_skills:
  ...
  script:
    - pip install nexus-core[all-llms]
    - nexus-core install --config react --target claude
    - nexus-core install --config vue --target gemini --no-upload
  artifacts:
    paths:
      - output/
  only:
...
```bash
pip install nexus-core==3.7.0   # internals can shift between minors
```
...
| API | Import | Use Case |
|-----|--------|----------|
| **Skill conversion factory** | `nexus_core.cli.skill_converter.get_converter` | Any of the 18 source types → skill |
| **Converter registry** | `nexus_core.cli.skill_converter.CONVERTER_REGISTRY` | Source type → (module, class) lookup |
| **Source detection** | `nexus_core.cli.source_detector.SourceDetector` | Auto-detect type from raw input |
| **Web docs** | `nexus_core.cli.doc_scraper.DocToSkillConverter` | Documentation websites |
| **GitHub repos** | `nexus_core.cli.github_scraper.GitHubScraper` | Code + docs + community analysis |
| **PDF** | `nexus_core.cli.pdf_scraper.PDFToSkillConverter` | PDF documents |
| **Local codebase** | `nexus_core.cli.codebase_scraper.CodebaseAnalyzer` | Local directories (C3.x pipeline) |
| **Multi-source** | `nexus_core.cli.unified_scraper.UnifiedScraper` | Merge 18 source types + conflict detection |
| **Packaging / upload / enhance** | `nexus_core.cli.adaptors.get_adaptor` | 22 platform targets |
| **AI enhancement** | `nexus_core.cli.agent_client.AgentClient` | API or local-agent LLM calls |
| **Local-agent enhancement** | `nexus_core.cli.enhance_skill_local.LocalSkillEnhancer` | Free enhancement via coding agents |
| **Settings singleton** | `nexus_core.cli.execution_context.ExecutionContext` | Initialize / get / override settings |
| **Marketplace registry** | `nexus_core.services.marketplace_manager.MarketplaceManager` | Marketplace CRUD |
| **Config sources** | `nexus_core.services.source_manager.SourceManager` | Config source registry CRUD |
| **Config publishing** | `nexus_core.services.config_publisher` | Push configs; `detect_category()` |
| **Git config repos** | `nexus_core.services.git_repo.GitConfigRepo` | Clone/pull + config discovery |
