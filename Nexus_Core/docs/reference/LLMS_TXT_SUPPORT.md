...
Nexus_Core now automatically detects and uses llms.txt files when available, providing 10x faster documentation ingestion.
...
1. Before HTML scraping, Nexus_Core checks for llms.txt files
...
```bash
nexus-core create --config configs/hono.json
```
