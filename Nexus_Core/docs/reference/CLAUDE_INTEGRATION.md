...
```python
from nexus_core.cli.generate_router import RouterGenerator
from nexus_core.cli.github_fetcher import GitHubThreeStreamFetcher
...
nexus-core create --config configs/vue.json
...
```bash
nexus-core create --name react --url https://react.dev/ --description "React framework"
```

### Skip scraping (use cached data)
```bash
nexus-core create --config configs/godot.json --skip-scrape
```

### Resume interrupted scrapes
```bash
# If scrape was interrupted
nexus-core create --config configs/godot.json --resume

# Start fresh (clear checkpoint)
nexus-core create --config configs/godot.json --fresh
```
...
nexus-core create --config configs/react.json --enhance-level 2

# Option 2: During scraping (LOCAL, no API key - uses Claude Code Max)
nexus-core create --config configs/react.json --enhance-level 2 --agent claude
...
nexus-core package output/react/ --target claude     # Default
...
nexus-core enhance output/react/ --target claude     # Sonnet 4
nexus-core enhance output/react/ --target gemini     # Gemini 2.0
nexus-core enhance output/react/ --target openai     # GPT-4o
```
...
pytest --cov=src/nexus_core --cov-report=html
```

**Why install first?**
- Tests import from `nexus_core.cli` which requires the package to be installed
...
Nexus_Core automatically detects llms.txt files before HTML scraping:
