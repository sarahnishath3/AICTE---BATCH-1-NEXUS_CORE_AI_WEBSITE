...
Nexus Core provides a unified CLI for converting documentation, GitHub repositories, PDFs, videos, notebooks, wikis, and 18 source types (17 + config) into AI-ready skills for 21+ LLM platforms and RAG pipelines.
...
nexus-core create ./my-project --languages Python,JavaScript

# Skip heavy features for faster analysis
nexus-core create ./my-project --skip-dependency-graph --skip-patterns
```
...
**Syntax:**
```bash
nexus-core create <asciidoc-file> [options]
```
...
```bash
# Single file
nexus-core create guide.adoc --name my-guide

# Directory of AsciiDoc files
nexus-core create ./docs/ --name project-docs
```
...
```bash
# From Slack export
nexus-core create --chat-export-path ./slack-export/ --name team-knowledge

# From Discord export
nexus-core create --chat-export-path ./discord-export/ --platform discord --name discord-docs
```
...
nexus-core create --conf-base-url https://wiki.example.com --space-key DEV --name dev-wiki

# From export
nexus-core create --conf-export-path ./confluence-export/ --name team-docs
```
...
**Syntax:**
```bash
nexus-core enhance SKILL_DIRECTORY [options]
```
...
**Syntax:**
```bash
nexus-core enhance-status SKILL_DIRECTORY [options]
```
...
**Syntax:**
```bash
nexus-core estimate [config] [options]
```
...
nexus-core estimate configs/react.json

# Quick estimate (100 pages)
nexus-core estimate configs/react.json --max-discovery 100

# Discover everything, slower sites get more time
nexus-core estimate configs/react.json --unlimited --timeout 60
...
nexus-core create  facebook/react --token $GITHUB_TOKEN

# Skip issues for faster scraping
nexus-core create  facebook/react --no-issues

# Dry run to preview
nexus-core create  facebook/react --dry-run
...
nexus-core create docs/index.html --name my-docs

# Whole directory of HTML files (auto-detected)
nexus-core create ./mirror_output/site/ --name site-mirror

# Force HTML mode on a mixed/code-heavy directory
nexus-core create ./repo/ --html-path ./repo/docs/build/html/ --name myrepo-docs

# --html-path alone works without a positional source
nexus-core create --html-path ./html-export/ --name exported-docs
```
...
**Syntax:**
```bash
nexus-core install --config CONFIG [options]
```
...
# Skip upload
nexus-core install --config react --no-upload

# Custom config
nexus-core install --config configs/my-project.json
...
**Syntax:**
```bash
nexus-core install-agent SKILL_DIRECTORY --agent AGENT [options]
```
...
# Install to Windsurf
nexus-core install-agent output/react/ --agent windsurf

# Force overwrite
nexus-core install-agent output/react/ --agent cursor --force
```
...
**Syntax:**
```bash
nexus-core create <notebook.ipynb> [options]
```
...
nexus-core create ./notebooks/ --name ml-tutorials
```
...
**Syntax:**
```bash
nexus-core create <manpage.1> [options]
```
...
nexus-core create --man-names ls,grep,find,awk --name unix-essentials

# From directory
nexus-core create --man-path /usr/share/man/man1/ --name section1-cmds
```
...
**Syntax:**
```bash
nexus-core multilang SKILL_DIRECTORY [options]
```
...
nexus-core multilang output/react/ --detect

# Translation coverage report
nexus-core multilang output/react/ --report

# Export per-language trees, English and Spanish only
nexus-core multilang output/react/ --export output/by-lang/ --languages en es
```
...
nexus-core create --database-id abc123 --name team-docs

# From export
nexus-core create --notion-export-path ./notion-export/ --name project-wiki
```
...
**Syntax:**
```bash
nexus-core create <openapi.yaml> [options]
```
...
```bash
# From local file
nexus-core create api/openapi.yaml --name my-api

# From URL
nexus-core create --spec-url https://petstore.swagger.io/v2/swagger.json --name petstore
```
...
**Syntax:**
```bash
nexus-core package SKILL_DIRECTORY [options]
```
...
# Package with upload
nexus-core package output/react/ --target claude --upload

# Streaming mode for large docs
nexus-core package output/large-docs/ --streaming
```
...
nexus-core create --config configs/manual.json

# Enable enhancement
nexus-core create --pdf manual.pdf --enhance-level 2

# Dry run to preview
nexus-core create --pdf manual.pdf --name test --dry-run
```
...
**Syntax:**
```bash
nexus-core create <slides.pptx> [options]
```
...
nexus-core create training-slides.pptx --name training-material

# With enhancement
nexus-core create architecture.pptx --name arch-overview --enhance-level 2
```
...
**Syntax:**
```bash
nexus-core quality SKILL_DIRECTORY [options]
```
...
nexus-core quality output/react/

# Detailed report
nexus-core quality output/react/ --report

# Save report as JSON
nexus-core quality output/react/ --output quality.json

# Quality gate: fail (non-zero exit) if below threshold
nexus-core quality output/react/ --threshold 7.0
```
...
**Syntax:**
```bash
nexus-core resume [JOB_ID] [options]
```
...
**Syntax:**
```bash
nexus-core create <feed.rss> [options]
```
...
```bash
# From URL
nexus-core create https://blog.example.com/feed.xml --name blog-knowledge

# From local file
nexus-core create --feed-path ./feed.rss --name feed-summaries
```
...
| `--no-fetch` | off | Skip the nexuscoreweb.com API fallback during resolution (offline mode) |
...
2. **Local repo / user dir** — `./configs/<name>.json` then `~/.config/nexus-core/configs/<name>.json` (each candidate is tried against the canonical-name list which includes CJK / EU suffix strips, e.g. "Godot 引擎" → `godot`)
3. **Community API** — `https://api.nexuscoreweb.com/api/configs/<name>` (unless `--no-fetch`)
...
- Opens a GitHub issue at [nexus-core-configs](https://github.com/yusufkaraaslan/nexus-core-configs) — no direct git push.
...
- Source-file sampling means actual code is in the prompt. For a fully-local flow use `nexus-core create ./path --enhance-level 0`.
...
# Quick mode
nexus-core create --name react --url https://react.dev/

# Interactive mode
nexus-core create --interactive

# Dry run
nexus-core create --config configs/react.json --dry-run

# Fast async scraping
nexus-core create --config configs/react.json --async --workers 5

# Skip scrape, rebuild from cache
nexus-core create --config configs/react.json --skip-scrape

# Resume interrupted scrape
nexus-core create --config configs/react.json --resume
```
...
**Syntax:**
```bash
nexus-core stream INPUT_FILE [options]
```
...
nexus-core stream big-docs.md

# Custom chunk size and overlap
nexus-core stream big-docs.md --streaming-chunk-chars 1000 --streaming-overlap-chars 100

# Save the collected chunks
nexus-core stream big-docs.md --output chunks.json
```
...
**Syntax:**
```bash
nexus-core create --config FILE [options]
```
...
```bash
# Unified scraping
nexus-core create --config configs/react-unified.json

# Fresh start
nexus-core create --config configs/react-unified.json --fresh

# Rule-based merging
nexus-core create --config configs/react-unified.json --merge-mode rule-based
```
...
**Syntax:**
```bash
nexus-core update SKILL_DIRECTORY [options]
```
...
```bash
# Check for changes
nexus-core update output/react/ --check-changes

# Generate an update package
nexus-core update output/react/ --generate-package update-pkg.json

# Apply an update package
nexus-core update output/react/ --apply-update update-pkg.json
```
...
```bash
# Upload to Claude
nexus-core upload output/react-claude.zip

# Upload to Gemini
nexus-core upload output/react-gemini.tar.gz --target gemini

# Upload to ChromaDB
nexus-core upload output/react-chroma.zip --target chroma

# Upload to Weaviate Cloud
nexus-core upload output/react-weaviate.zip --target weaviate \
...
nexus-core create --video-url  https://www.youtube.com/watch?v=VIDEO_ID --name my-skill

# With visual frame extraction (requires --setup first)
nexus-core create --video-url  VIDEO_URL --name my-skill --visual

# Local video file
nexus-core create --video-url  /path/to/video.mp4 --name my-skill
```
...
- Requires `pip install nexus-core[video]` (transcripts) or `nexus-core[video-full]` (+ whisper + scene detection)
...
**Syntax:**
```bash
nexus-core workflows ACTION [options]
```
...
nexus-core workflows add ./workflow.yaml --name my-custom
...
# 2. Package
nexus-core package output/codebase/ --target claude

# 3. Install to Cursor
nexus-core install-agent output/codebase/ --agent cursor
```
...
```bash
# 1. Extract PDF
nexus-core create --pdf manual.pdf --name product-docs
...
nexus-core create --config configs/my-project.json
...
nexus-core install --config react --destination ./output

# Or with create
nexus-core create https://docs.react.dev/ --preset standard
```
...
```bash
# Increase rate limit
nexus-core create --config react.json --rate-limit 1.0
```

### Out of memory
```bash
# Use streaming mode
nexus-core package output/large/ --streaming
```
...
*For additional help: `nexus-core --help` or `nexus-core <command> --help`*
