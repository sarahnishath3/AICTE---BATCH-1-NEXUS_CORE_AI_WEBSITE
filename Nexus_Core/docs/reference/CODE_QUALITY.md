...
Nexus Core maintains high code quality through automated linting, comprehensive testing, and continuous integration. This document outlines the quality standards, tools, and processes used to ensure reliability and maintainability.
...
# Check specific file
ruff check src/nexus_core/cli/doc_scraper.py
...
ruff format src/nexus_core/cli/doc_scraper.py
```
...
1. **src/nexus_core/cli/config_extractor.py** (SIM102 fixes)
2. **src/nexus_core/cli/config_validator.py** (SIM113 fix)
3. **src/nexus_core/cli/doc_scraper.py** (B007 fix)
4. **src/nexus_core/cli/pattern_recognizer.py** (3 × SIM102 fixes)
5. **src/nexus_core/cli/test_example_extractor.py** (3 × SIM117 fixes)
6. **src/nexus_core/cli/unified_skill_builder.py** (SIM117 fix)
7. **src/nexus_core/cli/pdf_scraper.py** (B904 fix)
...
# Run with coverage
pytest tests/ --cov=src/nexus_core --cov-report=term --cov-report=html
...
Nexus Core uses GitHub Actions for automated quality checks on every commit and PR.
...
jobs:
  ...
  test:
    ...
    steps:
      ...
      - name: Run tests
        run: pytest tests/ --cov=src/nexus_core --cov-report=xml
...
ruff check src/nexus_core/cli/your_file.py --fix

# Run relevant tests
pytest tests/test_your_feature.py -v

# Check formatting
ruff format src/nexus_core/cli/your_file.py
```
