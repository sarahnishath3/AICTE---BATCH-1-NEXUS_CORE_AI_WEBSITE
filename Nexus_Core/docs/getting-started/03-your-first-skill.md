...
```bash
# Ensure nexus-core is installed
nexus-core --version

# Should output: nexus-core 3.6.0
```
...
```bash
nexus-core create https://docs.djangoproject.com/ --dry-run
```
...
Next steps:
  1. Upload to Claude: nexus-core upload output/django-claude.zip
...
nexus-core upload output/django-claude.zip --target claude
```
...
nexus-core create https://docs.djangoproject.com/ --name django --enhance-level 0

# Package
nexus-core package output/django/ --target claude

# Enhances later if needed
nexus-core enhance output/django/
```
...
✅ **Create** - `nexus-core create <source>` auto-detects and scrapes  
✅ **Dry Run** - `--dry-run` previews without executing  
✅ **Enhancement** - AI automatically improves SKILL.md quality  
✅ **Package** - `nexus-core package <dir> --target <platform>`  
...
```bash
nexus-core create manual.pdf --name docs
nexus-core package output/docs/ --target claude
```
...
nexus-core package output/django/ --target claude
nexus-core package output/django/ --target gemini
nexus-core package output/django/ --target openai

# Upload to each
nexus-core upload output/django-claude.zip --target claude
nexus-core upload output/django-gemini.tar.gz --target gemini
```
...
```bash
# Limit pages
nexus-core create https://docs.djangoproject.com/ --max-pages 100
```
...
| Step | Command | Time |
|------|---------|------|
| 1 | `nexus-core create https://docs.djangoproject.com/` | ~15 min |
| 2 | `nexus-core package output/django/ --target claude` | ~5 sec |
| 3 | `nexus-core upload output/django-claude.zip` | ~10 sec |
