...
`nexus-core scan` is the fastest way to bootstrap a knowledge base for a
...
3. **Resolves each detection** in order:
   ...
   - **Local repo / user dir** — `./configs/<name>.json` then
     `~/.config/nexus-core/configs/<name>.json`
   - **Community API** — `https://api.nexuscoreweb.com/api/configs/<name>`
...
nexus-core create ./configs/scanned/my-react-app-codebase.json
```
...
nexus-core scan ./my-monorepo --max-ai-generations 5
...
| `--no-fetch` | off | Skip the nexuscoreweb.com API fallback during resolution |
