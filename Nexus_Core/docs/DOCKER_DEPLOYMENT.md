...
Complete guide for deploying Nexus Core using Docker.
...
CMD ["python", "-m", "nexus_core.mcp.server_fastmcp"]
```
...
COPY . .
CMD ["python", "-m", "nexus_core.mcp.server_fastmcp", "--reload"]
```
...
docker run -d \
  ...
  --restart unless-stopped \
  skillseekers:latest \
  python -m nexus_core.embedding.server --host 0.0.0.0 --port 8000
```
...
docker run -d \
  ...
  --restart unless-stopped \
  skillseekers:latest \
  nexus-core-sync start --config configs/react.json
```
...
services:
  ...
  embedding-server:
    ...
    command: ["python", "-m", "nexus_core.embedding.server", "--host", "0.0.0.0"]
    restart: unless-stopped
    healthcheck:
...
**Need help?** Open an issue on [GitHub](https://github.com/yusufkaraaslan/Nexus_Core/issues).
