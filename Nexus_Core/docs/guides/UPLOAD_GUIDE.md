...
Nexus Core supports automatic upload to **10 LLM platforms** — Claude AI, Google Gemini, OpenAI ChatGPT, MiniMax AI, Kimi, DeepSeek, Qwen, OpenRouter, Together AI, Fireworks AI — and to the **Chroma, Weaviate, and Pinecone** vector databases. The valid `--target` values are derived from the adaptor registry (`nexus-core upload --help`). Generic Markdown is **export-only** (package it, no upload).
