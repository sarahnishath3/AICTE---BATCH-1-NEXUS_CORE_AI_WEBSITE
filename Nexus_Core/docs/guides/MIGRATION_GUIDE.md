...
This guide helps you upgrade Nexus Core between major versions. Each section covers breaking changes, new features, and step-by-step migration instructions.
...
# Check for updates
pip show nexus-core | grep Version
...
# Verify
nexus-core --version  # Should show 3.6.0
...
nexus-core codebase --directory . --build-api-reference --build-dependency-graph
```

**After (current — opt-out, via the unified `create` command):**
```bash
# All features enabled by default
nexus-core create --directory .
...
python -m nexus_core.mcp.server
...
nexus-core create https://github.com/facebook/react
nexus-core create --pdf manual.pdf
```

**Migration:**
- Replace command prefixes with `nexus-core <subcommand>`
...
- Use `nexus-core-mcp` instead of custom server script
...
**Problem:**
```
ModuleNotFoundError: No module named 'nexus_core.mcp'
```
...
```bash
# Rollback to v3.5.0
pip install nexus-core==3.5.0

# Rollback to v2.7.0
pip install nexus-core==2.7.0
...
- **[GitHub Issues](https://github.com/yusufkaraaslan/Nexus_Core/issues)** - Report problems
...
**What I did:**
1. Upgraded with pip install --upgrade nexus-core
2. Ran nexus-core create --config react
