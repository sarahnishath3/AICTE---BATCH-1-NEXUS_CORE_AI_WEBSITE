...
{
  "command": "python",
  "args": ["-m", "nexus_core.mcp.server_fastmcp", "--http", "--port", "3000"]
}
```
...
# python3 -m nexus_core.mcp.server_fastmcp --transport http --port 3000
#
...
python -m nexus_core.mcp.server_fastmcp --http --host 0.0.0.0 --port 8080
```
...
- Start HTTP server: `python -m nexus_core.mcp.server_fastmcp --http --port 3000`
...
```bash
# Test stdio mode
timeout 3 python3 -m nexus_core.mcp.server_fastmcp || echo "Server OK (timeout expected)"
...
# Example output: /Users/username/Projects/Nexus_Core
# or: /home/username/Nexus_Core
```
...
```bash
# Terminal 1 - Run HTTP server
python -m nexus_core.mcp.server_fastmcp --http --port 3002
```
...
Nexus Core supports packaging for **12 LLM platforms**: Claude AI, Google Gemini, OpenAI ChatGPT, MiniMax AI, OpenCode, Kimi, DeepSeek, Qwen, OpenRouter, Together AI, Fireworks AI, and Generic Markdown (export-only). Automatic upload is available for the API-backed LLM platforms (all of the above except OpenCode and Markdown) plus the Chroma, Weaviate, and Pinecone vector databases.
...
3. **Test server manually:**
   ...
   **For stdio:**
   ```bash
   timeout 3 python3 -m nexus_core.mcp.server_fastmcp
   # Should exit cleanly or timeout (both OK)
   ```

   **For HTTP:**
   ```bash
   python3 -m nexus_core.mcp.server_fastmcp --http --port 8000
...
5. **Completely restart agent:**
   ...
   - Kill any background processes: `pkill -f nexus_core`
   - Reopen agent
...
- Error: "ModuleNotFoundError: No module named 'nexus_core'"

**Solution 1: Install Package and MCP Dependencies**

```bash
# Navigate to Nexus Core directory
cd /path/to/Nexus_Core
...
```bash
# Test module import
python3 -c "from nexus_core.mcp import server_fastmcp; print('✓ Module OK')"

# Test server startup
cd /path/to/Nexus_Core
python3 -m nexus_core.mcp.server_fastmcp
...
2. Kill any background processes: `pkill -f nexus_core`
...
**Symptoms:**
- `python -m nexus_core.mcp.server_fastmcp --http` fails
...
python -m nexus_core.mcp.server_fastmcp --http --port 8001
```
...
2. **Verify CLI tools exist:**
   ```bash
   python3 -m nexus_core.cli.doc_scraper --help
   python3 -m nexus_core.cli.package_skill --help
   ```

3. **Test tool directly:**
   ```bash
   # Test in Python
   python3 -c "from nexus_core.mcp.tools import list_configs_impl; print('OK')"
   ```
...
3. **Test with different host:**
   ...
   python -m nexus_core.mcp.server_fastmcp --http --host 0.0.0.0
   ```
...
```json
{
  "mcpServers": {
    "skill-seeker": {
      "command": "/path/to/Nexus_Core/venv/bin/python3",
      "args": ["-m", "nexus_core.mcp.server_fastmcp"]
    }
  }
}
...
Type=simple
User=yourusername
WorkingDirectory=/path/to/Nexus_Core
ExecStart=/usr/bin/python3 -m nexus_core.mcp.server_fastmcp --http --port 8000
Restart=on-failure
...
<dict>
    ...
    <array>
        ...
        <string>-m</string>
        <string>nexus_core.mcp.server_fastmcp</string>
        <string>--http</string>
        <string>--port</string>
        <string>8000</string>
    </array>
    <key>WorkingDirectory</key>
    <string>/path/to/Nexus_Core</string>
    <key>RunAtLoad</key>
    <true/>
    <key>KeepAlive</key>
    <true/>
...
```bash
python -m nexus_core.mcp.server_fastmcp --http --port 8000
```
...
- **Agent Detection**: See [agent_detector.py](../src/nexus_core/mcp/agent_detector.py)
...
- Test stdio: timeout 3 python -m nexus_core.mcp.server_fastmcp
- Test HTTP: curl http://localhost:8000/health
- Logs (Claude Code): ~/Library/Logs/Claude/
- Kill servers: pkill -f nexus_core
```
