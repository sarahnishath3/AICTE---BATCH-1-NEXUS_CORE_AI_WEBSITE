...
Nexus Core has comprehensive test coverage with **1,880+ tests** spanning unit tests, integration tests, end-to-end tests, and MCP integration tests. This guide covers everything you need to know about testing in the project.
...
def test_detect_language():
    ...
    from nexus_core.cli.doc_scraper import detect_language
...
def test_unified_scraping_integration(tmp_path):
    ...
    from nexus_core.cli.unified_scraper import unified_scrape
...
# With coverage
pytest tests/test_*_integration.py --cov=src/nexus_core
```
...
def test_install_workflow_end_to_end(tmp_path):
    ...
    from nexus_core.cli.install_skill import install_skill
...
async def test_mcp_list_configs():
    ...
    from nexus_core.mcp.server_fastmcp import app
...
pytest tests/ --cov=src/nexus_core --cov-report=xml
...
-----------------------------------------------------------------
src/nexus_core/__init__.py                     8      0   100%
src/nexus_core/cli/doc_scraper.py           420     35    92%
src/nexus_core/cli/github_scraper.py        310     20    94%
src/nexus_core/cli/adaptors/claude.py       125      5    96%
-----------------------------------------------------------------
TOTAL                                         3500    280    92%
```
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Run tests
        run: |
          pytest tests/ -v --cov=src/nexus_core --cov-report=xml
...
pytest tests/ -n auto --cov=src/nexus_core
```
...
**Problem:**
```
ImportError: No module named 'nexus_core'
```
