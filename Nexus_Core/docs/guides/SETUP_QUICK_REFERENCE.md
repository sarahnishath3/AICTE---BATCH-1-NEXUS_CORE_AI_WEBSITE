...
# Manual start
python3 -m nexus_core.mcp.server_fastmcp --http --port 3000
```
...
# Find and kill
pkill -f "nexus_core.mcp.server_fastmcp"
```

### View Logs
```bash
tail -f /tmp/nexus-core-mcp.log
```
...
from nexus_core.mcp.agent_detector import AgentDetector
...
import sys
sys.path.insert(0, 'src')
from nexus_core.mcp.agent_detector import generate_config
print(generate_config('claude-code', 'nexus-core mcp'))
"
```
...
# Use different port
python3 -m nexus_core.mcp.server_fastmcp --http --port 8080
```
...
python3 -c "import sys; sys.path.insert(0, 'src'); from nexus_core.mcp.agent_detector import detect_agents; print(detect_agents())"

# Generate config for agent
python3 -c "import sys; sys.path.insert(0, 'src'); from nexus_core.mcp.agent_detector import generate_config; print(generate_config('cursor', 'nexus-core mcp', 3000))"

# Validate config JSON
jq empty ~/.config/claude-code/mcp.json

# Start HTTP server in background
nohup python3 -m nexus_core.mcp.server_fastmcp --http --port 3000 > /tmp/nexus-core-mcp.log 2>&1 &
...
# Find server process
ps aux | grep nexus_core.mcp.server_fastmcp
...
```
src/nexus_core/mcp/agent_detector.py
```

### MCP Server
```
src/nexus_core/mcp/server_fastmcp.py
```
