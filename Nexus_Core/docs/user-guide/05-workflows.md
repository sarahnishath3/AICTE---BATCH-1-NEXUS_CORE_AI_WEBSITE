...
Nexus Core includes 6 built-in workflow presets:
...
**Example:**
```bash
nexus-core create oauth2-server --enhance-workflow security-focus
```
...
**Example:**
```bash
nexus-core create kubernetes/kubernetes \
...
**Example:**
```bash
nexus-core create https://api.example.com/docs \
  --enhance-workflow api-documentation
```
...
**Example:**
```bash
nexus-core create --config configs/multi-source.json \
...
# With custom name
nexus-core workflows add my-workflow.yaml --name perf-guide
```

### Use Custom Workflow

```bash
nexus-core create <source> --enhance-workflow performance-focus
```
...
```bash
# Edit the file, then:
nexus-core workflows add my-workflow.yaml --name performance-focus
```

### Remove Workflow

```bash
nexus-core workflows remove performance-focus
```
...
# Copy multiple
nexus-core workflows copy security-focus api-documentation minimal

# Edit the copy
nano ~/.config/nexus-core/workflows/security-focus.yaml
```
...
nexus-core create auth-library --enhance-workflow security-focus

# API project
nexus-core create api-framework --enhance-workflow api-documentation
```
...
nexus-core workflows add ml-workflow.yaml
nexus-core create ml-framework --enhance-workflow ml-focus
```
...
```bash
# Check stage details
nexus-core workflows show my-workflow
...
Workflows are supported by **all 18 source types** in Nexus Core:
...
nexus-core create https://docs.example.com --enhance-workflow security-focus

# GitHub repository
nexus-core create  owner/repo --enhance-workflow api-documentation

# Local codebase
nexus-core create ./my-project --enhance-workflow architecture-comprehensive

# PDF document
nexus-core create --pdf manual.pdf --enhance-workflow minimal

# Unified config (multi-source)
nexus-core create --config configs/multi-source.json --enhance-workflow security-focus
...
nexus-core create --config config.json --enhance-workflow api-documentation
```
