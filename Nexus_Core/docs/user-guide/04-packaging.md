...
for platform in $PLATFORMS; do
  ...
  nexus-core package "$SKILL_DIR" --target "$platform"
...
nexus-core package output/my-skill/ --skip-quality-check
```
...
nexus-core package output/my-skill/ --no-open
```
...
# Custom chunk size
nexus-core package output/large-skill/ \
  --streaming \
...
nexus-core upload output/my-skill-openai.zip --target openai
```
...
```bash
# Local ChromaDB
nexus-core upload output/my-skill-chroma.zip --target chroma

# With custom URL
nexus-core upload output/my-skill-chroma.zip \
  --target chroma \
...
```bash
# Local Weaviate
nexus-core upload output/my-skill-weaviate.zip --target weaviate

# Weaviate Cloud
nexus-core upload output/my-skill-weaviate.zip \
  --target weaviate \
  --use-cloud \
...
nexus-core install-agent output/my-skill/ --agent cursor
```
...
```bash
nexus-core install-agent output/my-skill/ --agent windsurf
```
...
```bash
# Check quality
nexus-core quality output/my-skill/
...
nexus-core quality output/my-skill/ --threshold 7.0
```
...
# Rebuild if needed
nexus-core create --config my-config --skip-scrape
...
```bash
# Use streaming mode
nexus-core package output/my-skill/ --streaming

# Smaller chunks
nexus-core package output/my-skill/ --streaming --streaming-chunk-chars 1000
```
...
```bash
# Validate first
nexus-core quality output/my-skill/ --threshold 6.0
