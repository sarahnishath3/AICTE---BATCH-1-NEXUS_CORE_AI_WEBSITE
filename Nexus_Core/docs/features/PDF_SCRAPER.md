...
# Run scraper
nexus-core create --config configs/my_manual.json
```
...
📦 Next step: Package with: nexus-core package output/mymanual/
```
...
nexus-core create --pdf manual.pdf --name mymanual --description "My Manual Docs"
```
...
python -m nexus_core.cli.pdf_extractor_poc manual.pdf -o manual_extracted.json --extract-images

# Step 2: Build skill from JSON (fast, can iterate)
nexus-core create --from-json manual_extracted.json
```
...
# 2. Run PDF scraper
nexus-core create --config configs/api_manual.json

# 3. Package skill
nexus-core package output/api_manual/

# 4. Upload to Claude (if ANTHROPIC_API_KEY set)
nexus-core package output/api_manual/ --upload
...
nexus-core enhance output/api_manual/
