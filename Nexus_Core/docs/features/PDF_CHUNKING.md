...
nexus-core create input.pdf

# Custom chunk size (20 pages per chunk)
python -m nexus_core.cli.pdf_extractor_poc input.pdf --pdf-pages-per-chunk 20

# Disable chunking (single chunk with all pages)
python -m nexus_core.cli.pdf_extractor_poc input.pdf --pdf-pages-per-chunk 0
```
...
python -m nexus_core.cli.pdf_extractor_poc manual.pdf -o manual.json
...
python -m nexus_core.cli.pdf_extractor_poc large_manual.pdf --pdf-pages-per-chunk 50 -o output.json -v
...
python -m nexus_core.cli.pdf_extractor_poc small_doc.pdf --pdf-pages-per-chunk 0 -o output.json
```
...
```bash
python -m nexus_core.cli.pdf_extractor_poc test.pdf -o test.json --pdf-pages-per-chunk 20 -v
...
```bash
python -m nexus_core.cli.pdf_extractor_poc test.pdf -o test.json -v
...
python -m nexus_core.cli.pdf_extractor_poc large_manual.pdf --pdf-pages-per-chunk 25 -o manual.json

# Build skill from the extracted JSON
nexus-core create --from-json manual.json
