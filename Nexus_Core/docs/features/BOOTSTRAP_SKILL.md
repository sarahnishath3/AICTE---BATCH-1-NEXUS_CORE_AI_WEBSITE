...
The **Bootstrap Skill** feature allows Nexus Core to analyze **itself** and generate a Claude Code skill containing its own documentation, API reference, code patterns, and usage examples. This is the ultimate form of "dogfooding" - using the tool to document itself.

**What You Get:**
- Complete Nexus Core documentation as a Claude Code skill
...
**Use Cases:**
- Learn Nexus Core by having it explain itself to Claude
...
nexus-core create \
  --directory . \
  --output output/nexus-core \
  --name nexus-core

# 2. Merge with custom header (optional)
cat scripts/skill_header.md output/nexus-core/SKILL.md > output/nexus-core/SKILL_MERGED.md
mv output/nexus-core/SKILL_MERGED.md output/nexus-core/SKILL.md

# 3. Install to Claude Code
nexus-core install-agent output/nexus-core --agent claude
```
...
The `codebase_scraper.py` module analyzes the Nexus Core source code:

```bash
nexus-core create --directory . --output output/nexus-core
```

**What Gets Analyzed:**
- **Python source files** (`src/nexus_core/**/*.py`)
- **Test files** (`tests/**/*.py`)
...
cat scripts/skill_header.md output/nexus-core/SKILL.md > merged.md
```
...
description: |
  ...
  Nexus Core codebase, CLI commands, or API integration.
tags: [documentation, scraping, ai-skills, mcp]
---

# Nexus Core - Documentation to AI Skills
...
if ! grep -q "^---$" output/nexus-core/SKILL.md; then
...
python -c "
import yaml
with open('output/nexus-core/SKILL.md') as f:
    content = f.read()
...
The final skill is created in `output/nexus-core/`:
...
**Error:**
```
❌ Missing YAML frontmatter in output/nexus-core/SKILL.md
```
...
**Error:**
```
❌ nexus-core codebase failed with exit code 1
```

**Solution:**
```bash
# Run analysis manually to see error
nexus-core create --directory . --output output/test
...
**Debug Checklist:**
1. ✅ Nexus Core installed: `nexus-core --version`
...
def test_bootstrap_skill_e2e(tmp_path):
    ...
    # Setup
    output_dir = tmp_path / "nexus-core"
...
# Output:
# ✅ Analyzing Nexus Core codebase...
# ✅ Detected 15 design patterns
# ✅ Extracted 45 test examples
# ✅ Generated 12 how-to guides
# ✅ Merging with header...
# ✅ Validating skill...
# ✅ Bootstrap skill created: output/nexus-core/SKILL.md
```
...
# "Use the nexus-core skill to explain how to scrape documentation"
```

### Example 4: Programmatic Usage

```python
from nexus_core.cli.codebase_scraper import scrape_codebase
from nexus_core.cli.install_agent import install_to_agent

# 1. Analyze codebase
result = scrape_codebase(
    directory='.',
    output_dir='output/nexus-core',
    name='nexus-core',
...
# 3. Install to Claude Code
install_to_agent(
    skill_dir='output/nexus-core',
    agent_dir='~/.claude/skills/nexus-core'
)
