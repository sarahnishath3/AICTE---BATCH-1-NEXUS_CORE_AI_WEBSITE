...
nexus-core enhance output/steam-inventory/

# Option 2: Integrated with scraper (LOCAL mode is used when no API key is set)
nexus-core create --config configs/steam-inventory.json
```
...
`nexus-core` automatically. (All API enhancement now routes through the
...
nexus-core create --config configs/steam-inventory.json --enhance-level 2
...
Nexus Core supports enhancement for Claude AI, Google Gemini, and OpenAI ChatGPT using platform-specific AI models.
