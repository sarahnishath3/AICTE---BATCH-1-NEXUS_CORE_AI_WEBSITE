...
[![Версия](https://img.shields.io/badge/version-3.7.0-blue.svg)](https://github.com/yusufkaraaslan/Nexus_Core/releases)
...
[![PyPI версия](https://badge.fury.io/py/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - Загрузки](https://img.shields.io/pypi/dm/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - Версия Python](https://img.shields.io/pypi/pyversions/nexus-core.svg)](https://pypi.org/project/nexus-core/)
...
[![Веб-сайт](https://img.shields.io/badge/Website-nexuscoreweb.com-blue.svg)](https://nexuscoreweb.com/)
...
**🧠 Слой данных для ИИ-систем.** Nexus Core преобразует документацию сайтов, репозитории GitHub, PDF, видео, Jupyter-ноутбуки, вики и более 10 других типов источников в структурированные базы знаний — готовые к использованию в ИИ-навыках (Claude, Gemini, OpenAI), RAG-конвейерах (LangChain, LlamaIndex, Pinecone) и ИИ-помощниках для программирования (Cursor, Windsurf, Cline) за считанные минуты.

> 🌐 **[Посетите SkillSeekersWeb.com](https://nexuscoreweb.com/)** — просматривайте 24+ готовых конфигураций, делитесь своими настройками и получайте доступ к полной документации!
...
Nexus Core — это мульти-репозиторный проект. Вот где находится каждая часть:

| Репозиторий | Описание | Ссылки |
|------------|----------|--------|
| **[Nexus_Core](https://github.com/yusufkaraaslan/Nexus_Core)** | Основной CLI и MCP сервер (этот репозиторий) | [PyPI](https://pypi.org/project/nexus-core/) |
| **[nexuscoreweb](https://github.com/yusufkaraaslan/nexuscoreweb)** | Веб-сайт и документация | [Сайт](https://nexuscoreweb.com/) |
| **[nexus-core-configs](https://github.com/yusufkaraaslan/nexus-core-configs)** | Репозиторий конфигураций сообщества | |
| **[nexus-core-action](https://github.com/yusufkaraaslan/nexus-core-action)** | GitHub Action для CI/CD | |
| **[nexus-core-plugin](https://github.com/yusufkaraaslan/nexus-core-plugin)** | Плагин Claude Code | |
| **[homebrew-nexus-core](https://github.com/yusufkaraaslan/homebrew-nexus-core)** | Homebrew tap для macOS | |
...
nexus-core package output/react --target ibm-bob     # → Директория навыка IBM Bob
```
...
при выходе её можно опционально опубликовать в [реестре сообщества](https://github.com/yusufkaraaslan/nexus-core-configs).
...
# Видео (YouTube, Vimeo или локальный файл — требуется nexus-core[video])
...
Nexus Core заменяет дни ручной предобработки следующими шагами:
...
Nexus Core может вызывать CLI кодинг-агента, в который вы уже вошли, поэтому улучшение работает на вашем существующем тарифе вместо оплачиваемых API-токенов:

```bash
nexus-core create <source> --agent codex     # OpenAI Codex CLI → ваш ChatGPT Plus
nexus-core create <source> --agent claude    # Claude Code      → ваш Claude Pro/Max
```
...
Все они предоставляют OpenAI-совместимый эндпоинт `/v1`. Направьте Nexus Core на один из них с помощью трёх переменных окружения — он обнаруживает `OPENAI_API_KEY`, а OpenAI SDK автоматически учитывает `OPENAI_BASE_URL`:
...
nexus-core package output/django --target claude  # или --target markdown
...
- ✅ **Система конфигурации с несколькими токенами** — управление несколькими аккаунтами GitHub (личный, рабочий, open source)
  - Безопасное хранение конфигурации в `~/.config/nexus-core/config.json` (права 600)
...
Генерация nexus-core как навыка для использования в вашем ИИ-агенте (Claude Code, Kimi, Codex и т. д.):
...
- ✅ **Пользовательские пресеты** — добавляйте собственные рабочие процессы в `~/.config/nexus-core/workflows/`
...
nexus-core workflows list                          # Список всех (встроенные + пользовательские)
nexus-core workflows show security-focus           # Показать содержимое YAML
nexus-core workflows copy security-focus           # Скопировать в пользовательскую директорию для редактирования
nexus-core workflows add ./my-workflow.yaml        # Установить пользовательский пресет
nexus-core workflows remove my-workflow            # Удалить пользовательский пресет
nexus-core workflows validate security-focus       # Проверить структуру пресета
...
|-------------------|-----------------|
| `pip install nexus-core` | Сканирование, анализ GitHub, PDF, все платформы |
| `pip install nexus-core[gemini]` | + Поддержка Google Gemini |
| `pip install nexus-core[openai]` | + Поддержка OpenAI ChatGPT |
| `pip install nexus-core[all-llms]` | + Все LLM-платформы |
| `pip install nexus-core[mcp]` | + MCP-сервер |
| `pip install nexus-core[video]` | + Извлечение субтитров и метаданных YouTube/Vimeo |
| `pip install nexus-core[video-full]` | + Транскрипция Whisper и извлечение визуальных кадров |
| `pip install nexus-core[jupyter]` | + Поддержка Jupyter-ноутбуков |
| `pip install nexus-core[pptx]` | + Поддержка PowerPoint |
| `pip install nexus-core[confluence]` | + Поддержка вики Confluence |
| `pip install nexus-core[notion]` | + Поддержка страниц Notion |
| `pip install nexus-core[rss]` | + Поддержка RSS/Atom-лент |
| `pip install nexus-core[chat]` | + Поддержка экспорта чатов Slack/Discord |
| `pip install nexus-core[asciidoc]` | + Поддержка документов AsciiDoc |
| `pip install nexus-core[all]` | Всё включено |

> **Визуальные зависимости для видео (с поддержкой GPU):** После установки `nexus-core[video-full]` запустите
> `nexus-core create --setup` для автоопределения вашего GPU и установки правильной сборки PyTorch
...
Nexus Core поддерживает **12 LLM-платформ**, **8 RAG/векторных целей**, **18 типов источников** и полный паритет функций по всем целевым платформам.
...
pip install nexus-core[video]        # Субтитры + метаданные
pip install nexus-core[video-full]   # + Whisper транскрипция + извлечение визуальных кадров
...
```mermaid
graph LR
    A[Документация сайта] --> B[Nexus Core]
    B --> C[Сканер]
    B --> D[ИИ-улучшение]
    B --> E[Упаковщик]
...
Полные UML-диаграммы: **[docs/UML_ARCHITECTURE.md](docs/UML_ARCHITECTURE.md)** | Проект StarUML: `docs/UML/nexus_core.mdj` | HTML-справка по API: `docs/UML/html/`
...
Nexus Core может автоматически устанавливать навыки в 19 ИИ-агентов для программирования.
...
Nexus Core поставляется с MCP-сервером для использования из Claude Code, Cursor, Windsurf, VS Code + Cline или IntelliJ IDEA.
...
# nexus-core list-configs  # Недоступно в v3.7.0
```
...
3. `~/.config/nexus-core/configs/` (пользовательская директория конфигурации)
...
```bash
nexus-core create https://docs.react.dev/ --preset quick      # Быстрый, поверхностный
nexus-core create https://docs.react.dev/ --preset standard   # Сбалансированный (по умолчанию)
nexus-core create https://docs.react.dev/ --preset comprehensive # Глубокий, исчерпывающий
```

### Флаги жизненного цикла
```bash
nexus-core create https://docs.react.dev/ --dry-run    # Предпросмотр без сканирования
nexus-core create https://docs.react.dev/ --fresh      # Игнорировать кэш, полное пересканирование
nexus-core create https://docs.react.dev/ --resume     # Возобновить прерванную задачу
nexus-core create https://docs.react.dev/ --skip-scrape # Переупаковать существующий результат
```

### Проверка здоровья и утилиты
```bash
nexus-core doctor                    # Диагностика установки и окружения
nexus-core sync-config               # Обнаружение отклонений конфигурации
nexus-core stream <source>           # Потоковое поглощение для больших документов
nexus-core update output/react/      # Инкрементальное обновление
nexus-core multilang <source>        # Генерация навыков на нескольких языках
nexus-core quality output/react/     # Отчёт о качестве (добавьте --threshold 7 для контроля: ненулевой код выхода ниже 7/10)
```
...
|------------|-----------|------------|
| `browser` | `pip install "nexus-core[browser]"` | Безголовый Playwright для SPA-сайтов |
| `embedding` | `pip install "nexus-core[embedding]"` | Поддержка сервера эмбеддингов |
| `s3` / `gcs` / `azure` | `pip install "nexus-core[s3]"` и т. д. | Загрузка в облачное хранилище |
| `rag-upload` | `pip install "nexus-core[rag-upload]"` | Комбинированные зависимости загрузки в векторные БД |
...
- **[docs/UML/nexus_core.mdj](docs/UML/nexus_core.mdj)** — Файл проекта StarUML (откройте в [StarUML](https://staruml.io/))
...
[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=nexus_core) — полномодальная, OpenAI-совместимая платформа ИИ-инференса. Nexus Core поддерживает её как цель упаковки/улучшения через `--target atlas` с `ATLAS_API_KEY`.
