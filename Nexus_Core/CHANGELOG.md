...
- **Unified-CLI flags that were silently rejected now work** — `estimate --unlimited/--timeout`, `update --generate-package/--apply-update`, `quality --output`, `stream --streaming-overlap-chars/--batch-size/--checkpoint`, `multilang --report/--export`, `install --target`, and `extract-test-examples --recursive` were accepted by the standalone modules but rejected with "unrecognized arguments" by `nexus-core <cmd>` (central-parser drift). A programmatic drift-guard test now fails CI if any module flag is missing from its central parser.
...
- **`nexus_core.services` package** — `marketplace_manager`, `marketplace_publisher`, `config_publisher`, `source_manager`, and `git_repo` moved out of `mcp/` so the CLI can import this domain logic without the optional `[mcp]` extra. Back-compat shims remain at the old `nexus_core.mcp.*` paths.
...
- **`--model` flag for `enhance` and `package`** (#395, #398) — override the platform's default model, e.g. `nexus-core enhance output/react/ --target minimax --model MiniMax-M2.7` or `nexus-core package output/react/ --target minimax --model MiniMax-M2.7`. Honored uniformly across all enhancement adaptors and recorded in package metadata. Resurrects the previously-dead `custom_model` config key.
...
- **Import hygiene** — all seven `sys.path.insert` hacks in `mcp/` removed in favor of absolute `nexus_core.*` imports; no more dual module identities.
...
**Theme:** AI-driven project knowledge base (`nexus-core scan`) — bootstrap a complete skill set for a project in one command, with safety/observability/coverage hardening throughout.

### Added
- **`nexus-core scan <dir>` command** (#327) — point at any project; an AI agent inspects manifests, README, Dockerfile/CI, sampled source files (first 2 KB each), and the git remote, then emits one Nexus Core config per detected framework plus a `<project>-codebase.json` for the project's own code. Each config stamped with `metadata.detected_version` so re-scans report added / version-bumped / removed dependencies. Internationalized canonical-name resolver (CJK + EU language suffixes) so detections like "Godot 引擎" resolve `godot`. Out-dir cache means re-scans reuse prior emissions and respect manual edits. Doctor-style report with pluralized counts and resolved / AI-generated / unresolved / archived breakdown.
...
- **Community submission** (opt-in): freshly AI-generated configs can be submitted to the [community registry](https://github.com/yusufkaraaslan/nexus-core-configs) via a native-async flow. Pre-checks `GITHUB_TOKEN`, idempotency-guards against duplicate issues, retries transient failures with backoff.
...
- **Headless browser rendering** (`--browser` flag) — uses Playwright to render JavaScript SPA sites (React, Vue, etc.) that return empty HTML shells. Auto-installs Chromium on first use. Optional dep: `pip install "nexus-core[browser]"` (#321)
- **`nexus-core doctor` command** — 8 diagnostic checks (Python version, package install, git, core/optional deps, API keys, MCP server, output dir) with pass/warn/fail status and `--verbose` flag (#316)
...
- **Ecosystem section** in README linking all Nexus Core repos (PyPI, website, plugin, GitHub Action)
...
Nexus Core now supports 17 source types — up from 7. Every new type is fully integrated into the CLI (`nexus-core <type>`), `create` command auto-detection, unified multi-source configs, config validation, the MCP server, and the skill builder.

- **Jupyter Notebook** — `nexus-core jupyter --notebook file.ipynb` or `nexus-core create file.ipynb`
  ...
  - Optional dependency: `pip install "nexus-core[jupyter]"` (nbformat)
  - Entry point: `nexus-core-jupyter`

- **Local HTML** — `nexus-core html --html-path file.html` or `nexus-core create file.html`
...
- **OpenAPI/Swagger** — `nexus-core openapi --spec spec.yaml` or `nexus-core create spec.yaml`
  ...
  - Optional dependency: `pip install "nexus-core[openapi]"` (pyyaml — already a core dep, guard added for safety)

- **AsciiDoc** — `nexus-core asciidoc --asciidoc-path file.adoc` or `nexus-core create file.adoc`
  ...
  - Optional dependency: `pip install "nexus-core[asciidoc]"` (asciidoc library for advanced rendering)

- **PowerPoint (.pptx)** — `nexus-core pptx --pptx file.pptx` or `nexus-core create file.pptx`
  ...
  - Optional dependency: `pip install "nexus-core[pptx]"` (python-pptx)

- **RSS/Atom Feeds** — `nexus-core rss --feed-url <url>` / `--feed-path file.rss` or `nexus-core create feed.rss`
  ...
  - Optional dependency: `pip install "nexus-core[rss]"` (feedparser)

- **Man Pages** — `nexus-core manpage --man-names git,curl` / `--man-path dir/` or `nexus-core create git.1`
...
- **Confluence** — `nexus-core confluence --base-url <url> --space-key <key>` or `--export-path dir/`
  ...
  - Optional dependency: `pip install "nexus-core[confluence]"` (atlassian-python-api)

- **Notion** — `nexus-core notion --database-id <id>` / `--page-id <id>` or `--export-path dir/`
  ...
  - Optional dependency: `pip install "nexus-core[notion]"` (notion-client)

- **Slack/Discord Chat** — `nexus-core chat --export-path dir/` or `--token <token> --channel <channel>`
  ...
  - Optional dependency: `pip install "nexus-core[chat]"` (slack-sdk)

#### EPUB Unified Pipeline Integration
- **EPUB (.epub) input support** via `nexus-core create book.epub` or `nexus-core epub --epub book.epub`
  ...
  - Optional dependency: `pip install "nexus-core[epub]"` (ebooklib)
...
- **10 new entry points** in pyproject.toml — `nexus-core-{jupyter,html,openapi,asciidoc,pptx,rss,manpage,confluence,notion,chat}`
...
#### Sync Config Command
- **`nexus-core sync-config`** — New subcommand that crawls a docs site's navigation, diffs discovered URLs against a config's `start_urls`, and optionally writes the updated list back with `--apply` (#306)
...
#### Video Pipeline Core (`nexus-core video`)
- **`nexus-core video --url <youtube-url>`** — New CLI command for video tutorial scraping. Also supports `--video-file` for local files and `--playlist` for YouTube playlists
- **`nexus-core create <youtube-url>`** — Auto-detects YouTube URLs and routes to video scraper
...
- **`nexus-core video --setup`** — One-command GPU auto-detection and dependency installation
...
- **`nexus-core word --docx <file>`** and `nexus-core create document.docx` — Full pipeline: mammoth → HTML → BeautifulSoup → sections → SKILL.md + references/
...
- **`nexus-core-word`** entry point in pyproject.toml
- **`docx` optional dependency group** — `pip install nexus-core[docx]` (mammoth + python-docx)
...
- **`nexus-core-video`** entry point in pyproject.toml
...
- **Video dependency error messages** — `video_scraper.py` and `video_visual.py` now suggest `nexus-core video --setup` as primary fix
...
- **`nexus-core analyze --dry-run`** — Fixed `_handle_analyze_command()` in `main.py` not forwarding `--dry-run`, `--preset`, `--quiet`, `--name`, `--description`, `--api-key`, and workflow flags to codebase_scraper
...
- **`--dry-run` for GitHub scraper** — `nexus-core github --repo owner/repo --dry-run` now previews the operation
- **`--dry-run` for PDF scraper** — `nexus-core pdf --name test --dry-run` now previews the operation
- **`--verbose`/`--quiet` for GitHub and PDF scrapers** — Logging level control now works consistently across all scrapers
- **`--name`/`--description` for codebase analyzer** — Custom skill name and description can now be passed to `nexus-core analyze`
- **`--mode LOCAL` flag for `nexus-core enhance`** — Explicitly forces LOCAL mode even when API keys are present
...
- **Single command for all source types** — auto-detects URL, GitHub repo (`owner/repo`), local directory, PDF file, or multi-source config JSON
  ...
  nexus-core create tutorial.pdf
  ```
...
- **`-p` shortcut** for preset selection: `nexus-core create <source> -p quick|standard|comprehensive`
...
- **65 bundled workflow presets** shipped as YAML files in `nexus_core/workflows/`:
...
- User presets stored in `~/.config/nexus-core/workflows/`
- Subcommands:
  - `nexus-core workflows list` — List all bundled + user workflows with descriptions
  - `nexus-core workflows show <name>` — Print YAML content of a workflow
  - `nexus-core workflows copy <name> [name ...]` — Copy bundled workflow(s) to user dir
  - `nexus-core workflows add <file.yaml> [file ...]` — Install custom YAML file(s) into user dir
  - `nexus-core workflows remove <name> [name ...]` — Delete user workflow(s)
  - `nexus-core workflows validate <name|path>` — Parse and validate a workflow
...
- New entry point: `nexus-core-workflows`

#### Multiple `--enhance-workflow` Flags from CLI
- Chain workflows in a single command: `nexus-core create <source> --enhance-workflow security-focus --enhance-workflow minimal`
- Supported across all scrapers: `scrape`, `github`, `analyze`, `pdf`, `unified`

#### Smart Enhancement Dispatcher (`nexus-core enhance`)
...
### Config Repository (nexus-core-configs)
...
This is our biggest release ever! v3.0.0 establishes Nexus Core as the **universal documentation preprocessor** for the entire AI ecosystem - from RAG pipelines to AI coding assistants to Claude skills.
...
- **AWS S3** - `nexus-core cloud upload --provider s3 --bucket my-bucket`
- **Google Cloud Storage** - `nexus-core cloud upload --provider gcs --bucket my-bucket`
- **Azure Blob Storage** - `nexus-core cloud upload --provider azure --container my-container`
...
#### Docker
```bash
docker run -v $(pwd):/data nexus-core:latest scrape --config /data/config.json
```
...
- **Config Integration**: `default_enhance_level` setting in `~/.config/nexus-core/config.json`
...
- **Auto-Enhancement**: SKILL.md automatically enhanced when using `--enhance` or `--comprehensive`
  - No need for separate `nexus-core enhance` command
...
- **Broken Language Selector Links on PyPI**
  ...
  - **Solution**: Changed to absolute GitHub URL (`https://github.com/yusufkaraaslan/Nexus_Core/blob/main/README.zh-CN.md`)
...
This **documentation release** adds comprehensive Chinese language support, making Nexus Core accessible to the world's largest developer community.
...
- **🇨🇳 Chinese (Simplified) README Translation** (#260)
  ...
  - Impact: Makes Nexus Core accessible to 1+ billion Chinese speakers
...
- Review issue: https://github.com/yusufkaraaslan/Nexus_Core/issues/260
...
- **Issue #258: `install --config` command fails with unified scraper** (#258)
  ...
  - **Impact**: `nexus-core install --config react` now works without "unrecognized arguments" error
  - **Files Fixed**: `src/nexus_core/cli/unified_scraper.py`, `src/nexus_core/cli/main.py`
...
- **Issue #259 (Original): `scrape` command doesn't accept URL and --max-pages** (#259)
  ...
  - **Impact**: `nexus-core scrape https://example.com --max-pages 50` now works
  - **Safety Warnings**:
    - ⚠️ Warning if max-pages > 1000 (may take hours)
    - ⚠️ Warning if max-pages < 10 (incomplete skill)
  - **Files Fixed**: `src/nexus_core/cli/doc_scraper.py`, `src/nexus_core/cli/main.py`
...
- **Issue #259 (Comment A): Version shows 2.7.0 instead of actual version** (#259)
  ...
  - **Impact**: `nexus-core --version` now shows correct version (2.7.2)
  - **Files Fixed**: `src/nexus_core/cli/main.py`
...
- **Issue #259 (Comment B): PDF command shows empty "Error: " message** (#259)
  ...
  - **Files Fixed**: `src/nexus_core/cli/main.py`, `src/nexus_core/cli/pdf_scraper.py`

### Testing

- ✅ Verified `nexus-core install --config react --dry-run` works
- ✅ Verified `nexus-core scrape https://tailwindcss.com/docs/installation --max-pages 50` works
- ✅ Verified `nexus-core --version` shows "2.7.2"
...
- **Critical: Config download 404 errors** - Fixed bug where code was constructing download URLs manually instead of using the `download_url` field from the API response
  ...
  - **Files Fixed**:
    - `src/nexus_core/mcp/tools/source_tools.py` (FastMCP server)
    - `src/nexus_core/mcp/server_legacy.py` (Legacy server)
  - **Impact**: Fixes all config downloads from nexuscoreweb.com API and private Git repositories
  - **Reported By**: User testing `nexus-core install --config godot --unlimited`
...
- **C3.8 Standalone Codebase Scraper SKILL.md Generation** - Complete skill structure for standalone codebase analysis
  ...
  - Integration points:
    - CLI tool: `nexus-core analyze --directory /path/to/code --output /path/to/output`
...
- **Global Setup Script with FastMCP** - setup.sh for end-user global installation
  ...
  - Installs `nexus-core` globally: `pip3 install nexus-core`
  ...
  - Perfect for end users who want to use Nexus Core without development setup
...
- **Enhanced LOCAL Enhancement Modes** - Advanced enhancement execution options (moved from previous unreleased)
  ...
  - **Status Monitoring**: New `enhance-status` command for background/daemon processes
    - Check status once: `nexus-core enhance-status output/react/`
    - Watch in real-time: `nexus-core enhance-status output/react/ --watch`
    - JSON output for scripts: `nexus-core enhance-status output/react/ --json`
  ...
  - **CLI Integration**: All modes accessible via `nexus-core enhance` command
...
- **C3.1 Design Pattern Detection** - Detect 10 common design patterns in code
  ...
  - CLI tool: `nexus-core-patterns --file src/db.py`
...
- **C3.2 Test Example Extraction** - Extract real usage examples from test files
  ...
  - CLI tool: `nexus-core extract-test-examples tests/ --language python`
...
- **C3.3 How-To Guide Generation with Comprehensive AI Enhancement** - Transform test workflows into step-by-step educational guides with professional AI-powered improvements
  ...
  - **Integration Points**:
    - CLI tool: `nexus-core-how-to-guides test_examples.json --group-by ai-tutorial-group --ai-mode auto`
...
- **C3.4 Configuration Pattern Extraction with AI Enhancement** - Analyze and document configuration files across your codebase with optional AI-powered insights
  ...
  - **Integration Points**:
    - CLI tool: `nexus-core-config-extractor --directory . --enhance-local` (with AI)
...
- **C3.6 AI Enhancement** - AI-powered insights for patterns and test examples
  ...
  - **LOCAL Mode** (for SKILL.md enhancement - existing feature):
    - Uses `nexus-core enhance output/skill/` command
...
- **Codebase Scraper Language Stats** - Fixed dict format handling in `_get_language_stats()`
  ...
  - Location: `src/nexus_core/cli/codebase_scraper.py:778`
...
- **Package Discovery**: Switched from manual package listing to automatic discovery in pyproject.toml ([#227](https://github.com/yusufkaraaslan/Nexus_Core/pull/227))
...
- ✅ `nexus_core` (core)
- ✅ `nexus_core.cli` (CLI tools)
- ✅ `nexus_core.cli.adaptors` (platform adaptors)
- ✅ `nexus_core.mcp` (MCP server)
- ✅ `nexus_core.mcp.tools` (MCP tools)
...
- **CRITICAL**: Added missing `nexus_core.cli.adaptors` module to packages list in pyproject.toml ([#221](https://github.com/yusufkaraaslan/Nexus_Core/pull/221))
  - **Issue**: v2.5.0 on PyPI throws `ModuleNotFoundError: No module named 'nexus_core.cli.adaptors'`
  ...
  - **Fix**: Added `nexus_core.cli.adaptors` to packages in pyproject.toml
  - **Credit**: Thanks to [@MiaoDX](https://github.com/MiaoDX) for finding and fixing this issue!

### Package Structure

The `nexus_core.cli.adaptors` module contains the platform adaptor architecture:
...
#### Core Infrastructure
- **Platform Adaptors** (`src/nexus_core/cli/adaptors/`)
...
#### CLI Tools
- **Multi-platform packaging**: `nexus-core package output/skill/ --target gemini`
- **Multi-platform upload**: `nexus-core upload skill.zip --target openai`
- **Multi-platform enhancement**: `nexus-core enhance output/skill/ --target gemini --mode api`
...
- **`src/nexus_core/mcp/README.md`** (UPDATED)
...
- **`[gemini]`** extra: `pip install nexus-core[gemini]`
  - google-generativeai>=0.8.3
  - Required for Gemini enhancement and upload

- **`[openai]`** extra: `pip install nexus-core[openai]`
  - openai>=1.59.6
  - Required for OpenAI enhancement and upload

- **`[all-llms]`** extra: `pip install nexus-core[all-llms]`
...
2. **To use other platforms:**
   ...
   pip install nexus-core[gemini]    # For Gemini
   pip install nexus-core[openai]    # For OpenAI
   pip install nexus-core[all-llms]  # For all platforms
...
2. Update MCP config to use `server_fastmcp`:
   ...
   ```
3. For HTTP agents, start HTTP server: `python -m nexus_core.mcp.server_fastmcp --http`
...
#### Git-Based Config Sources (Issue [#211](https://github.com/yusufkaraaslan/Nexus_Core/issues/211))
...
#### Core Infrastructure
- **GitConfigRepo class** (`src/nexus_core/mcp/git_repo.py`, 283 lines)
...
- **SourceManager class** (`src/nexus_core/mcp/source_manager.py`, 260 lines)
...
#### Storage & Caching
- **Registry file**: `~/.nexus-core/sources.json`
  - Stores source metadata (URL, branch, priority, timestamps)
  - Version-controlled schema (v1.0)
  - Atomic writes prevent corruption

- **Cache directory**: `$SKILL_SEEKERS_CACHE_DIR` (default: `~/.nexus-core/cache/`)
...
### Files Changed
- Modified (2): `pyproject.toml`, `src/nexus_core/mcp/server.py`
...
- [Issue #211](https://github.com/yusufkaraaslan/Nexus_Core/issues/211) - Original feature request
...
### Fixed
- **submit_config MCP tool** - Comprehensive validation and format support ([#11](https://github.com/yusufkaraaslan/Nexus_Core/issues/11))
...
### Added
- **Configurable directory exclusions** for local repository analysis ([#203](https://github.com/yusufkaraaslan/Nexus_Core/issues/203))
...
- **Skip llms.txt option** - Force HTML scraping even when llms.txt is detected ([#198](https://github.com/yusufkaraaslan/Nexus_Core/pull/198))

### Fixed
- Fixed logger initialization error causing `AttributeError: 'NoneType' object has no attribute 'setLevel'` ([#190](https://github.com/yusufkaraaslan/Nexus_Core/issues/190))
...
- **CLI tool** - `nexus-core-quality-checker` with verbose and strict modes
...
#### New Features
- **Headless enhancement** - `nexus-core-enhance` runs in background by default
...
#### New Modules
- `src/nexus_core/cli/quality_checker.py` - Quality validation engine
- `tests/test_quality_checker.py` - 12 comprehensive tests

#### Modified Modules
- `src/nexus_core/cli/enhance_skill_local.py` - Added headless mode
- `src/nexus_core/cli/doc_scraper.py` - Updated enhancement integration
- `src/nexus_core/cli/package_skill.py` - Added quality checks
- `src/nexus_core/mcp/server.py` - Skip quality checks in MCP context
...
- **Headless mode default** - Enhancement now runs in background by default
  ...
  - Affects: `nexus-core-enhance` and `nexus-core scrape --enhance-local`
...
nexus-core-package output/react/ --skip-quality-check
```
...
#### Package Structure
- **Moved to src/ layout:**
  - `src/nexus_core/` - Main package
  - `src/nexus_core/cli/` - CLI tools
  - `src/nexus_core/mcp/` - MCP server
...
- **Fixed MCP server tests** - Updated to use `nexus_core.mcp.server` imports
...
#### For Developers
- Update imports: `from cli.* → from nexus_core.cli.*`
- Use `pip install -e ".[dev]"` for development
- Run tests: `python -m pytest`
- Entry points instead of direct script execution

### Breaking Changes
- **CLI interface changed** - Use `nexus-core` command instead of `python3 cli/...`
- **Import paths changed** - Package now at `nexus_core.*` instead of `cli.*`
- **Installation method changed** - PyPI recommended over git clone

### Deprecations
- **Direct script execution** - Still works but deprecated (use `nexus-core` command)
...
This is the first production-ready release of Nexus Core with complete feature set, full test coverage, and comprehensive documentation.
...
- [v1.2.0](https://github.com/yusufkaraaslan/Nexus_Core/releases/tag/v1.2.0) - PDF Advanced Features
- [v1.1.0](https://github.com/yusufkaraaslan/Nexus_Core/releases/tag/v1.1.0) - Documentation Scraping Enhancements
- [v1.0.0](https://github.com/yusufkaraaslan/Nexus_Core/releases/tag/v1.0.0) - Production Release
- [v0.4.0](https://github.com/yusufkaraaslan/Nexus_Core/releases/tag/v0.4.0) - Large Documentation Support
- [v0.3.0](https://github.com/yusufkaraaslan/Nexus_Core/releases/tag/v0.3.0) - MCP Integration
...
[Unreleased]: https://github.com/yusufkaraaslan/Nexus_Core/compare/v1.2.0...HEAD
[1.2.0]: https://github.com/yusufkaraaslan/Nexus_Core/compare/v1.1.0...v1.2.0
[1.1.0]: https://github.com/yusufkaraaslan/Nexus_Core/compare/v1.0.0...v1.1.0
[1.0.0]: https://github.com/yusufkaraaslan/Nexus_Core/compare/v0.4.0...v1.0.0
[0.4.0]: https://github.com/yusufkaraaslan/Nexus_Core/compare/v0.3.0...v0.4.0
[0.3.0]: https://github.com/yusufkaraaslan/Nexus_Core/compare/v0.2.0...v0.3.0
[0.2.0]: https://github.com/yusufkaraaslan/Nexus_Core/releases/tag/v0.2.0
[0.1.0]: https://github.com/yusufkaraaslan/Nexus_Core/releases/tag/v0.1.0
