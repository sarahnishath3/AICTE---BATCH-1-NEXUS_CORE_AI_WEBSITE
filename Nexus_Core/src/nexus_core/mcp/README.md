...
**Replace `/path/to/Nexus_Core`** with your actual repository path!
...
Agent: 📄 Scraping PDF documentation...
       ...
       📦 Package with: nexus-core package output/api-docs/
...
├── source_manager.py       # Back-compat shim → nexus_core.services.source_manager
├── git_repo.py             # Back-compat shim → nexus_core.services.git_repo
├── marketplace_publisher.py # Back-compat shim → nexus_core.services.marketplace_publisher
├── marketplace_manager.py  # Back-compat shim → nexus_core.services.marketplace_manager
├── config_publisher.py     # Back-compat shim → nexus_core.services.config_publisher
...
lives in the `nexus_core.services` package — importable by the CLI without the
`[mcp]` extra; the `mcp/` modules listed above are thin back-compat shims. There
are no `sys.path` hacks; everything uses absolute `nexus_core.*` imports.
...
3. Test CLI tools directly:
   ```bash
   python -m nexus_core.cli.doc_scraper --help
   ```
...
```json
{
  "mcpServers": {
    "skill-seeker": {
      "command": "python3",
      "args": ["-u", "-m", "nexus_core.mcp.server_fastmcp"],
      "cwd": "/path/to/Nexus_Core",
      "env": {
        "DEBUG": "1"
      }
...
- **Discussions**: [GitHub Discussions](https://github.com/yusufkaraaslan/Nexus_Core/discussions)
