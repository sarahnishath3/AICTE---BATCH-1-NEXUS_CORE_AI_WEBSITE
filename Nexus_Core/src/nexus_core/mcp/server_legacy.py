...
from nexus_core.mcp.tools.subprocess_utils import run_subprocess_with_streaming
...
async def list_tools() -> list[Tool]:
    ...
    return [
        ...
        Tool(
            name="submit_config",
            description="Submit a custom config file to the community. Validates config (legacy or unified format) and creates a GitHub issue in nexus-core-configs repo for review.",
            inputSchema={
                "type": "object",
                "properties": {
                    "config_path": {
...
async def validate_config_tool(args: dict) -> list[TextContent]:
    ...
    try:
        ...
        try:
            ...
            from nexus_core.cli.doc_scraper import validate_config
...
"""
        ...
        # MODE 3: API (existing, backward compatible)
            ...
            API_BASE_URL = "https://api.nexuscoreweb.com"
    ...
    try:
        ...
        if not dry_run:
            ...
            if auto_upload and has_api_key:
                ...
                output_lines.append("   nexus-core upload " + workflow_state["zip_path"])
        else:
...
async def submit_config_tool(args: dict) -> list[TextContent]:
    """Submit a custom config to nexus-core-configs repository via GitHub issue"""
    try:
...
📚 Example configs: https://github.com/yusufkaraaslan/nexus-core-configs/tree/main/official
"""
        ...
        try:
            ...
            repo = gh.get_repo("yusufkaraaslan/nexus-core-configs")
...
📚 All configs: https://github.com/yusufkaraaslan/nexus-core-configs
"""
...
To free up disk space, manually delete: ~/.nexus-core/cache/{name}/
