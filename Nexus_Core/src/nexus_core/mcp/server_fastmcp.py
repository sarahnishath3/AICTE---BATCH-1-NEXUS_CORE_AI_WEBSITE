...
# module and under direct script execution, as long as nexus-core is
# installed — `pip install -e .` is required anyway).
from nexus_core.mcp.tools import (  # noqa: E402  (must follow the FastMCP availability check)
...
async def scrape_generic(
    ...
    corresponding CLI scraper module (e.g., nexus_core.cli.jupyter_scraper).
...
async def push_config(
    ...
    """
    from nexus_core.mcp.tools.source_tools import push_config_tool
...
@safe_tool_decorator(
    description="Create a new user workflow from YAML content. The workflow is saved to ~/.config/nexus-core/workflows/."
)
