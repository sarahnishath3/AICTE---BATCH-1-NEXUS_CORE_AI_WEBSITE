...
- `run_cli_main()`, the in-process replacement for shelling out to
  `python -m nexus_core.cli.<module>` (Phase 5d), and `run_cli_tool()`,
...
# Path to the bundled CLI tools (src/nexus_core/cli), resolved relative to
# this module which lives at src/nexus_core/mcp/tools/.
...
# shared "nexus_core" logger) only capture their OWN log records.
...
def capture_cli_logs(stream: io.StringIO):
    """Capture this call's "nexus_core" log records into ``stream``.

    The handler is attached to the shared "nexus_core" logger, so without
...
def run_cli_main(
    main_func: Callable[[], Any], argv: list[str], prog: str = "nexus-core"
) -> tuple[str, str, int]:
    ...
    - records from the "nexus_core" logger via ``capture_cli_logs``,
...
def run_cli_tool(
    main_func: Callable[[], Any], argv: list[str], progress_msg: str, prog: str = "nexus-core"
) -> list[Any]:
