...
from nexus_core.mcp.tools._common import (
    CLI_DIR,
    TextContent,
    run_cli_main,
    run_cli_tool,
...
async def package_skill_tool(args: dict) -> list[TextContent]:
    ...
    from nexus_core.cli import package_skill
...
async def upload_skill_tool(args: dict) -> list[TextContent]:
    ...
    from nexus_core.cli import upload_skill
    ...
    try:
        ...
        if not dry_run:
            ...
            if auto_upload and has_api_key and target != "markdown":
                ...
                output_lines.append(
                    f"   nexus-core upload {workflow_state['zip_path']} --target {target}"
                )
        else:
