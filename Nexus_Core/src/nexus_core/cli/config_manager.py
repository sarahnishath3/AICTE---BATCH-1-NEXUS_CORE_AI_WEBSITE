"""
Configuration Manager for Nexus Core
...
def _get_config_dir() -> Path:
    ...
    if sys.platform == "win32":
        return Path(os.environ.get("APPDATA", Path.home())) / "nexus-core"
    return Path.home() / ".config" / "nexus-core"


def _get_progress_dir() -> Path:
    """Return platform-appropriate progress/data directory."""
    if sys.platform == "win32":
        return Path(os.environ.get("LOCALAPPDATA", Path.home())) / "nexus-core" / "progress"
    return Path.home() / ".local" / "share" / "nexus-core" / "progress"


class ConfigManager:
    """Manages Nexus Core configuration with multi-token support."""
