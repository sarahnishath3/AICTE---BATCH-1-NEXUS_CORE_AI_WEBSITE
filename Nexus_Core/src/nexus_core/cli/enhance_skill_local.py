...
Usage:
    ...
    nexus-core enhance output/react/ --agent opencode

    # Custom agent command (advanced)
    nexus-core enhance output/react/ --agent custom --agent-cmd "my-agent --prompt {prompt_file}"
...
from nexus_core.cli.agent_client import AGENT_PRESETS, build_local_agent_command
from nexus_core.cli.agent_client import normalize_agent_name as _normalize_agent_name
from nexus_core.cli.constants import LOCAL_CONTENT_LIMIT, LOCAL_PREVIEW_LIMIT
...
class LocalSkillEnhancer:
    ...
    def _run_agent_command(self, prompt_file, timeout, include_permissions_flag, quiet=False):
        ...
        # Mark the child environment so a nested ``nexus-core create``
...
"""
    ...
    def run(self, headless=True, timeout=2700, background=False, daemon=False):
        ...
        # and a test that shells out to ``nexus-core create`` would spawn yet
...
"""
        ...
        print(f"  3. Package: nexus-core package {self.skill_dir}/")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if result.returncode == 75:
                    ...
                    print(f"     nexus-core enhance {self.skill_dir} --target claude")
                    return False
    ...
    def _run_background(self, headless, timeout):
        ...
        print(f"  - Or use: nexus-core enhance-status {self.skill_dir}")
        print()
...
def _detect_api_target() -> tuple[str, str] | None:
    ...
    """
    from nexus_core.cli.agent_client import detect_api_target
...
def _run_api_enhance(target: str, api_key: str, args) -> None:
    ...
    from nexus_core.cli.enhance_skill import main as api_main
...
Examples:
  ...
  nexus-core enhance output/react/ --mode LOCAL --background

  # LOCAL: daemon mode (persistent background process, fully detached)
  nexus-core enhance output/react/ --mode LOCAL --daemon

  # LOCAL: interactive mode (opens terminal window)
  nexus-core enhance output/react/ --mode LOCAL --interactive-enhancement

  # LOCAL: custom timeout
  nexus-core enhance output/react/ --mode LOCAL --timeout 1200
...
""",
    ...
    if getattr(args, "mode", None) != "LOCAL":
        ...
        if api_target is not None:
            from nexus_core.cli.enhance_command import api_sdk_available
