...
Usage:
    nexus-core openapi --spec petstore.yaml --name petstore-api
    nexus-core openapi --spec-url https://petstore3.swagger.io/api/v3/openapi.json --name petstore
    nexus-core openapi --from-json petstore_extracted.json
    python3 -m nexus_core.cli.openapi_scraper --spec spec.yaml --name my-api
"""
...
def _check_yaml_deps():
    ...
    if not YAML_AVAILABLE:
        raise RuntimeError(
            ...
            'Install with: pip install "nexus-core[openapi]"\n'
...
class OpenAPIToSkillConverter(SkillConverter):
    ...
    def build_skill(self) -> None:
        ...
        logger.info("  Next step: Package with: nexus-core package %s/", self.skill_dir)
    ...
    def _generate_skill_md(self, categories: dict[str, list[dict[str, Any]]]) -> None:
        ...
        # Footer
        lines.append("---\n")
        lines.append("**Generated by Nexus Core** | OpenAPI/Swagger Specification Scraper\n")
