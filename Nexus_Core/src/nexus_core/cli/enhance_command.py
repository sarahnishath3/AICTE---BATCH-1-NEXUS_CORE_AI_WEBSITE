...
Routes `nexus-core enhance` to the correct backend:
...
def _get_config_default_agent() -> str | None:
    ...
    try:
        from nexus_core.cli.config_manager import get_config_manager
...
def _run_api_mode(args, target: str) -> int:
    ...
    from nexus_core.cli.enhance_skill import main as enhance_api_main
...
Examples:
  ...
  nexus-core enhance output/react/ --target claude --api-key sk-ant-...
  ...
  nexus-core enhance output/react/ --dry-run
""",
    )
    ...
    if _is_root():
        ...
        print()
        print("   Then retry:")
        print(f"     nexus-core enhance {args.skill_directory}")
        return 1
