...
Usage:
    # API mode
    nexus-core confluence --base-url https://wiki.example.com \\
        --space-key PROJ --username user@example.com --token $CONFLUENCE_TOKEN \\
        --name my-project-wiki

    # Export mode
    nexus-core confluence --export-path ./confluence-export/ --name my-wiki

    # Build from previously extracted JSON
    nexus-core confluence --from-json my-wiki_extracted.json

    # Standalone execution
    python3 -m nexus_core.cli.confluence_scraper --base-url https://wiki.example.com \\
...
from nexus_core.cli.scraper_utils import score_code_quality as _score_code_quality
...
def _check_atlassian_deps() -> None:
    ...
    if not ATLASSIAN_AVAILABLE:
        raise RuntimeError(
            ...
            'Or: pip install "nexus-core[confluence]"'
        )
...
class ConfluenceToSkillConverter(SkillConverter):
    ...
    def build_skill(self) -> None:
        ...
        print(f"\n  Next step: Package with: nexus-core package {self.skill_dir}/")
    ...
    def _generate_skill_md(self, categorised: dict[str, dict[str, Any]]) -> None:
        ...
        with open(filename, "w", encoding="utf-8") as f:
            ...
            # Footer
            f.write("---\n\n")
            f.write("**Generated by Nexus Core** | Confluence Documentation Scraper\n")
