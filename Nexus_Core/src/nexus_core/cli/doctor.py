"""
nexus-core doctor — Environment health check command.
...
def check_package_installed() -> CheckResult:
    """Check 2: nexus-core package importable with version."""
    try:
        from nexus_core._version import __version__

        return CheckResult("nexus-core installed", "pass", f"v{__version__}", critical=True)
    except ImportError:
        return CheckResult(
            "nexus-core installed", "fail", "Cannot import nexus_core", critical=True
        )
...
def check_mcp_server() -> CheckResult:
    ...
    try:
        from nexus_core.mcp import server_fastmcp  # noqa: F401
...
def print_report(results: list[CheckResult], verbose: bool = False) -> int:
    ...
    print()
    print("=" * 50)
    print(f"  Nexus Core Doctor (v{version})")
    print("=" * 50)
    print()
...
class DoctorCommand:
    """Entry point for `nexus-core doctor`. Dispatched from `main.py` with
