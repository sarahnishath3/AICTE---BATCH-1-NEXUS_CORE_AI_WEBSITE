...
Usage:
    ...
    nexus-core install --config configs/custom.json --no-upload
    nexus-core install --config django --unlimited
...
def main(args=None):
    ...
    from nexus_core.cli.exit_codes import EXIT_ERROR, EXIT_INTERRUPT, EXIT_SUCCESS
    ...
    if not MCP_AVAILABLE:
        ...
        print("Install with:")
        print("  pip install nexus-core[mcp]")
        print("\nOr use these alternatives:")
        print("  nexus-core scrape --config react")
        print("  nexus-core package output/react/")
        print()
        sys.exit(EXIT_ERROR)

    if args is None:
        # Single source of flags: the central InstallParser.
        from nexus_core.cli.parsers.install_parser import InstallParser
