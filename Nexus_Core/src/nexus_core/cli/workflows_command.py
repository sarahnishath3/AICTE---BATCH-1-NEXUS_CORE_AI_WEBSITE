...
from nexus_core.cli.enhancement_workflow import (
    WorkflowEngine,
    list_bundled_workflows,
)

USER_WORKFLOWS_DIR = Path.home() / ".config" / "nexus-core" / "workflows"
...
def _bundled_yaml_text(name: str) -> str | None:
    ...
    for suffix in (".yaml", ".yml"):
        try:
            pkg_ref = importlib_files("nexus_core.workflows").joinpath(name + suffix)
...
def cmd_list() -> int:
    ...
    if user:
        print("\nUser workflows (~/.config/nexus-core/workflows/):")
        for name in user:
...
def cmd_show(name: str) -> int:
    ...
    if text is None:
        ...
        print("Use 'nexus-core workflows list' to see available workflows.", file=sys.stderr)
        return 1
    print(text, end="")
    return 0
...
def cmd_remove(names: list[str]) -> int:
    ...
    for name in names:
        if name in bundled:
            ...
            print("Use 'nexus-core workflows copy' to create an editable copy.", file=sys.stderr)
            rc = 1
            continue
...
def main(argv=None, args=None) -> None:
    """Entry point for nexus-core-workflows.

    Two call styles:
      - standalone (`nexus-core-workflows …` / tests): main(argv_list)
    ...
    if args is None:
        ...
        from nexus_core.cli.parsers.workflows_parser import WorkflowsParser

        parser = WorkflowsParser().build_standalone(
            prog="nexus-core-workflows",
    ...
    if action is None:
        if parser is not None:
            ...
            print("Usage: nexus-core workflows {list,show,copy,add,remove,validate}")
        sys.exit(0)
