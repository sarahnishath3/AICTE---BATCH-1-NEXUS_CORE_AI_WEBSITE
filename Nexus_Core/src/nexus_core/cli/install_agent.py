...
This module provides functionality to install Nexus Core-generated skills
...
Usage:
    nexus-core install-agent <skill_directory> --agent <agent_name> [--force] [--dry-run]
...
Examples:
    ...
    nexus-core install-agent output/react/ --agent claude --force
...
def validate_agent_name(agent_name: str) -> tuple[bool, str | None]:
    ...
    error_msg += f"\n\nUsage:\n  nexus-core install-agent <skill_directory> --agent {suggestions[0] if suggestions else 'claude'}"
...
def install_to_agent(
    ...
    if target_path.exists() and not force:
        ...
        error_msg += "Options:\n"
        error_msg += f"  1. Overwrite: nexus-core install-agent {skill_dir} --agent {agent_name} --force\n"
    ...
    if dry_run:
        ...
        msg += f"  nexus-core install-agent {skill_dir} --agent {agent_name}"
...
def main(args=None) -> int:
    ...
    """
    from nexus_core.cli.exit_codes import EXIT_ERROR, EXIT_SUCCESS

    if args is None:
        # Single source of flags: the central InstallAgentParser.
        from nexus_core.cli.parsers.install_agent_parser import InstallAgentParser

        parser = InstallAgentParser().build_standalone(prog="nexus-core-install-agent")
