...
Usage:
    nexus-core opencode-split <skill_directory> [--max-size 50000] [--output-dir output/]
"""
...
from nexus_core.cli.adaptors.opencode import OpenCodeAdaptor
...
class OpenCodeSkillConverter:
    ...
    Converts between Nexus Core format and OpenCode ecosystem format.
    """

    @staticmethod
    def import_opencode_skill(source_dir: str | Path) -> dict[str, Any]:
        """
        Import a skill from OpenCode format into Nexus Core format.

        Reads an OpenCode skill directory and returns a normalized dict
        suitable for further processing by Nexus Core adaptors.
    ...
    def export_to_target(
        ...
        from nexus_core.cli.adaptors.base import SkillMetadata
...
def main():
    parser = argparse.ArgumentParser(
        prog="nexus-core-opencode-split",
        description="Split large skills into OpenCode-compatible sub-skills",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Auto-split a large skill
  nexus-core opencode-split output/react/

  # Custom size threshold
  nexus-core opencode-split output/react/ --max-size 30000

  # Custom output directory
  nexus-core opencode-split output/react/ --output-dir output/react-split/

  # Import an OpenCode skill and convert to Claude format
  nexus-core opencode-convert ~/.opencode/skills/my-skill/ --target claude --output-dir output/

  # Check if splitting is needed
  nexus-core opencode-split output/react/ --dry-run
        """,
    )
