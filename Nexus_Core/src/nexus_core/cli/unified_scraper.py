...
Usage:
    nexus-core unified --config configs/godot_unified.json
    nexus-core unified --config configs/react_unified.json --merge-mode ai-enhanced
"""
...
try:
    from nexus_core.cli.agent_client import get_default_timeout
    from nexus_core.cli.config_validator import validate_config
    from nexus_core.cli.conflict_detector import ConflictDetector
    from nexus_core.cli.defaults import DEFAULTS
    from nexus_core.cli.merge_sources import AIEnhancedMerger, RuleBasedMerger
    from nexus_core.cli.skill_converter import SkillConverter
    from nexus_core.cli.unified_skill_builder import UnifiedSkillBuilder
...
class UnifiedScraper(SkillConverter):
    ...
    def _scrape_documentation(self, source: dict[str, Any]):
        ...
        try:
            from nexus_core.cli.doc_scraper import scrape_documentation
    ...
    def _scrape_github(self, source: dict[str, Any]):
        ...
        try:
            from nexus_core.cli.github_scraper import GitHubToSkillConverter
    ...
    def _scrape_video(self, source: dict[str, Any]):
        ...
        try:
            ...
            logger.error(
                ...
                "  Install with: pip install nexus-core[video]\n"
                "  For visual extraction (frame analysis, OCR): pip install nexus-core[video-full]"
            )
            return
            ...
            if enhancement_enabled and enhancement_level > 0:
                ...
                if not os.path.exists(skill_md_path):
                    ...
                    try:
                        ...
                        logger.info(
                            "   Try manually: nexus-core enhance "
                            + self.output_dir
                            + " --agent kimi"
                        )
                else:
                    ...
                    try:
                        ...
                        if client.client:
                            ...
                            if enhanced:
                                from nexus_core.cli.adaptors.base import (
