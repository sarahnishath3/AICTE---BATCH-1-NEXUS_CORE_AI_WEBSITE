...
Examples:
  ...
  nexus-core-sync start --config configs/react.json --interval 600

  # Start with auto-update
  nexus-core-sync start --config configs/react.json --auto-update

  # Check once (no continuous monitoring)
  nexus-core-sync check --config configs/react.json

  # Check with diffs
  nexus-core-sync check --config configs/react.json --diff -v

  # Show statistics
  nexus-core-sync stats --config configs/react.json

  # Reset state
  nexus-core-sync reset --skill-name react
        """,
    )
