...
class ScanParser(SubcommandParser):
    """Parser for the `nexus-core scan` subcommand.

    `scan` is an AI-driven discovery step: point it at a project directory
    and it emits one Nexus Core config per detected framework/library,
    plus a `<project>-codebase.json` for the project itself. Run
    `nexus-core create <config.json>` afterwards on each emitted file.
    """
    ...
    def description(self) -> str:
        return (
            ...
            "nexuscoreweb.com API) or — if none — generates a fresh "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--no-fetch",
            action="store_true",
            help="Skip the nexuscoreweb.com API fallback during resolution",
        )
        parser.add_argument(
            "--no-generate",
            action="store_true",
