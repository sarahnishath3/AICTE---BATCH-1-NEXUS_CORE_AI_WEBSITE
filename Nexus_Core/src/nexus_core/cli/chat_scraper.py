...
Usage:
    ...
    nexus-core chat --export-path ./slack-export/ --platform slack --name myteam

    # Slack API (live fetch)
    nexus-core chat --platform slack --token xoxb-... --channel C01234 --name myteam

    # Discord export (DiscordChatExporter JSON)
    nexus-core chat --export-path ./discord-export.json --platform discord --name myserver

    # Discord API (live fetch)
    nexus-core chat --platform discord --token Bot ... --channel 12345 --name myserver

    # Build from previously extracted JSON
    nexus-core chat --from-json myteam_extracted.json --name myteam
"""
...
from nexus_core.cli.document_skill_builder import DocumentSkillBuilder
...
def _check_slack_deps() -> None:
    ...
    if not SLACK_AVAILABLE:
        raise RuntimeError(
            ...
            'Install with: pip install "nexus-core[slack]"\n'
...
def _check_discord_deps() -> None:
    ...
    if not DISCORD_AVAILABLE:
        raise RuntimeError(
            ...
            'Install with: pip install "nexus-core[discord]"\n'
