...
Usage:
    # Use global workflow
    nexus-core analyze . --enhance-workflow security-focus

    # Use project workflow
    nexus-core analyze . --enhance-workflow .nexus-core/enhancement.yaml
...
class WorkflowEngine:
    ...
    def _load_workflow(self, workflow_ref: str | Path) -> EnhancementWorkflow:
        ...
        2. ~/.config/nexus-core/workflows/{name}.yaml — user overrides/custom
        3. nexus_core/workflows/{name}.yaml via importlib.resources — bundled defaults
        """
        ...
        if resolved_path is None:
            user_dir = Path.home() / ".config" / "nexus-core" / "workflows"
        ...
        if resolved_path is None:
            ...
            try:
                pkg_ref = importlib_files("nexus_core.workflows").joinpath(bare_name)
                ...
                raise FileNotFoundError(
                    ...
                    "Use 'nexus-core workflows list' to see available workflows."
                ) from exc
    ...
    def _run_builtin_stage(self, stage: WorkflowStage, context: dict) -> dict:
        ...
        from nexus_core.cli.ai_enhancer import PatternEnhancer, TestExampleEnhancer
    ...
    def _run_custom_stage(self, stage: WorkflowStage, context: dict) -> dict:
        ...
        if not self.enhancer:
            from nexus_core.cli.ai_enhancer import AIEnhancer
