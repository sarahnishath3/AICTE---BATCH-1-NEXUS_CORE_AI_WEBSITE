...
from nexus_core.cli.utils import sanitize_url, setup_logging
...
class DocToSkillConverter(SkillConverter):
    ...
    def _extract_markdown_content(self, content: str, url: str) -> dict[str, Any]:
        ...
        try:
            from nexus_core.cli.parsers.extractors import MarkdownParser
    ...
    def _render_with_browser(self, url: str) -> str:
        ...
        if self._browser_renderer is None:
            from nexus_core.cli.browser_renderer import BrowserRenderer
    ...
    def scrape_all(self) -> None:
        ...
        # Multi-threaded mode (parallel scraping)
            ...
            from nexus_core.cli.parallel_batches import context_propagating_submit
    ...
    def _log_scrape_completion(self) -> None:
        ...
        if visited >= 5 and self.pages_saved == 0:
            logger.warning(
                ...
                "   Try: nexus-core create <url> --browser\n"
                "   Install: pip install 'nexus-core[browser]'",
                visited,
            )
...
def load_config(config_path: str) -> dict[str, Any]:
    ...
    if resolved_path is None:
        ...
        user_config_dir = Path.home() / ".config" / "nexus-core" / "configs"
        ...
        if available:
            ...
            logger.error("")
            logger.error("   💡 Use any preset: nexus-core scrape --config <name>.json")
            logger.error("   🌐 Browse all: https://nexuscoreweb.com/")
        else:
            logger.error("   ⚠️  Could not connect to API to list available configs")
            logger.error("   🌐 Visit: https://nexuscoreweb.com/ for available configs")
        sys.exit(1)
    ...
    try:
        ...
        logger.error(
            "\n   Suggestion: Fix the above errors or check https://nexuscoreweb.com/ for examples"
        )
        sys.exit(1)
...
def _run_enhancement(
    ...
    try:
        ...
        if agent_client.mode == "api" and agent_client.client:
            ...
            from nexus_core.cli.agent_client import PROVIDER_TARGET_MAP
