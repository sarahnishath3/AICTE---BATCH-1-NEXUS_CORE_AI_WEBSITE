...
Usage:
    nexus-core video --setup          # Interactive (all modules)
    nexus-core video --setup          # Interactive, choose modules
...
def install_nexus_core(python_exe: str) -> bool:
    """Install nexus-core into the target python environment."""
    cmd = [python_exe, "-m", "pip", "install", "nexus-core"]
    try:
...
def run_setup(interactive: bool = True) -> int:
    ...
    if is_in_venv():
        ...
        if interactive:
            ...
            if answer and answer not in ("y", "yes"):
                ...
                print(f"  Installing nexus-core into venv...")
                if not install_nexus_core(python_exe):
                    print("  FAILED: Could not install nexus-core into venv.")
                    return 1
    ...
    print()
    if all_ok:
        print("Setup complete! You can now use: nexus-core video --url <URL> --visual")
