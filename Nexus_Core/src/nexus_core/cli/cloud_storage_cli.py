#!/usr/bin/env python3
"""
Cloud storage CLI for Nexus Core.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Cloud storage operations for Nexus Core",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Upload skill to S3
  nexus-core-cloud upload --provider s3 --bucket my-bucket \\
    --local-path output/react/ --remote-path skills/react/

  # Download from GCS
  nexus-core-cloud download --provider gcs --bucket my-bucket \\
    --remote-path skills/react/ --local-path output/react/

  # List files in Azure
  nexus-core-cloud list --provider azure --container my-container \\
    --prefix skills/

  # Generate signed URL
  nexus-core-cloud url --provider s3 --bucket my-bucket \\
