...
def check_video_dependencies(require_full: bool = False) -> None:
    ...
    if missing:
        ...
        setup_hint = (
            "\nFor visual deps (GPU-aware): nexus-core video --setup" if require_full else ""
        )
        raise RuntimeError(
            f"Missing video dependencies: {deps}\n"
            f'Install with: pip install "nexus-core{extra}"'
            f"{setup_hint}\n"
...
class VideoToSkillConverter(SkillConverter):
    ...
    def process(self) -> VideoScraperResult:
        ...
        """
        from nexus_core.cli.video_metadata import (
        ...
        if self.visual:
            ...
            if missing:
                raise RuntimeError(
                    ...
                    'Install with: pip install "nexus-core[video-full]"\n'
        ...
        for i, source in enumerate(urls_or_paths):
            ...
            try:
                ...
                if self.visual:
                    from nexus_core.cli.video_visual import (
                        download_video,
                        extract_visual_data,
                    )
