...
Usage:
    # Start server
    python -m nexus_core.embedding.server

    # Or with uvicorn
    uvicorn nexus_core.embedding.server:app --host 0.0.0.0 --port 8000
"""
...
if FASTAPI_AVAILABLE:
    app = FastAPI(
        title="Nexus Core Embedding API",
    ...
    cache_dir = os.getenv(
        "EMBEDDING_CACHE_DIR", os.path.expanduser("~/.cache/nexus-core/embeddings")
    )
    ...
    async def root():
        """Root endpoint."""
        return {
            "service": "Nexus Core Embedding API",
            "version": "1.0.0",
            "docs": "/docs",
            "health": "/health",
        }
...
def main():
    ...
    uvicorn.run("nexus_core.embedding.server:app", host=host, port=port, reload=reload)
