...
>
> Gerne können Sie über [GitHub Issue #260](https://github.com/yusufkaraaslan/Nexus_Core/issues/260) zur Verbesserung der Übersetzung beitragen! Ihr Feedback ist uns sehr wertvoll.
...
[![PyPI-Version](https://badge.fury.io/py/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - Downloads](https://img.shields.io/pypi/dm/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - Python-Version](https://img.shields.io/pypi/pyversions/nexus-core.svg)](https://pypi.org/project/nexus-core/)
...
[![Website](https://img.shields.io/badge/Website-nexuscoreweb.com-blue.svg)](https://nexuscoreweb.com/)
[![Twitter Follow](https://img.shields.io/twitter/follow/_yUSyUS_?style=social)](https://x.com/_yUSyUS_)
[![GitHub Repo Stars](https://img.shields.io/github/stars/yusufkaraaslan/Nexus_Core?style=social)](https://github.com/yusufkaraaslan/Nexus_Core)

**Die Datenschicht für KI-Systeme.** Nexus Core verwandelt Dokumentationswebsites, GitHub-Repositories, PDFs, Videos, Jupyter-Notebooks, Wikis und über 10 weitere Quelltypen in strukturierte Wissensressourcen — bereit für KI-Skills (Claude, Gemini, OpenAI), RAG-Pipelines (LangChain, LlamaIndex, Pinecone) und KI-Programmierassistenten (Cursor, Windsurf, Cline) in Minuten statt Stunden.

> **[Besuchen Sie SkillSeekersWeb.com](https://nexuscoreweb.com/)** - Durchsuchen Sie über 24 vorgefertigte Konfigurationen, teilen Sie Ihre Konfigurationen und greifen Sie auf die vollständige Dokumentation zu!
...
Nexus Core ist ein Multi-Repository-Projekt. Hier finden Sie alles:

| Repository | Beschreibung | Links |
|-----------|-------------|-------|
| **[Nexus_Core](https://github.com/yusufkaraaslan/Nexus_Core)** | Kern-CLI & MCP-Server (dieses Repo) | [PyPI](https://pypi.org/project/nexus-core/) |
| **[nexuscoreweb](https://github.com/yusufkaraaslan/nexuscoreweb)** | Website & Dokumentation | [Web](https://nexuscoreweb.com/) |
| **[nexus-core-configs](https://github.com/yusufkaraaslan/nexus-core-configs)** | Community-Konfigurationsrepository | |
| **[nexus-core-action](https://github.com/yusufkaraaslan/nexus-core-action)** | GitHub Action für CI/CD | |
| **[nexus-core-plugin](https://github.com/yusufkaraaslan/nexus-core-plugin)** | Claude Code Plugin | |
| **[homebrew-nexus-core](https://github.com/yusufkaraaslan/homebrew-nexus-core)** | Homebrew Tap für macOS | |
...
nexus-core package output/react --target ibm-bob     # → IBM Bob Skill-Verzeichnis
```
...
erkanntem Framework sowie eine `<project>-codebase.json` für Ihren eigenen Code. Die erkannte
...
beim Beenden können Sie diese optional in die [Community-Registry](https://github.com/yusufkaraaslan/nexus-core-configs) zurückveröffentlichen.
...
# Video (YouTube, Vimeo oder lokale Datei — erfordert nexus-core[video])
...
Anstatt tagelange manuelle Vorverarbeitung durchzuführen, erledigt Nexus Core dies:
...
<details>
<summary><strong>Eigenen KI-Anbieter verwenden (OpenAI-kompatible Endpunkte + Abonnements, keine Anthropic-Credits nötig)</strong></summary>
...
Nexus Core kann eine Coding-Agent-CLI aufrufen, bei der Sie bereits angemeldet sind, sodass die Verbesserung über Ihren bestehenden Tarif läuft statt über abgerechnete API-Tokens:

```bash
nexus-core create <source> --agent codex     # OpenAI Codex CLI → Ihr ChatGPT Plus
nexus-core create <source> --agent claude    # Claude Code      → Ihr Claude Pro/Max
```
...
Alle diese Anbieter stellen einen OpenAI-kompatiblen `/v1`-Endpunkt bereit. Richten Sie Nexus Core mit drei Umgebungsvariablen darauf aus — es erkennt `OPENAI_API_KEY`, und das OpenAI SDK berücksichtigt `OPENAI_BASE_URL` automatisch:
...
- **Multi-Token-Konfigurationssystem** - Mehrere GitHub-Konten verwalten (Privat, Arbeit, Open Source)
  - Sichere Konfigurationsspeicherung unter `~/.config/nexus-core/config.json` (Berechtigung 600)
...
Nexus Core selbst als Skill generieren, um es innerhalb Ihres KI-Agenten zu verwenden (Claude Code, Kimi, Codex usw.):
...
- **Benutzerdefinierte Presets** — Eigene Workflows unter `~/.config/nexus-core/workflows/` hinzufügen
...
# Presets verwalten
nexus-core workflows list                          # Alle auflisten (mitgeliefert + benutzerdefiniert)
nexus-core workflows show security-focus           # YAML-Inhalt anzeigen
nexus-core workflows copy security-focus           # Zum Benutzerverzeichnis kopieren (zum Bearbeiten)
nexus-core workflows add ./my-workflow.yaml        # Benutzerdefiniertes Preset installieren
nexus-core workflows remove my-workflow            # Benutzerdefiniertes Preset entfernen
nexus-core workflows validate security-focus       # Preset-Struktur validieren
...
|-------------|-----------|
| `pip install nexus-core` | Scraping, GitHub-Analyse, PDF, alle Plattformen |
| `pip install nexus-core[gemini]` | + Google Gemini-Unterstützung |
| `pip install nexus-core[openai]` | + OpenAI ChatGPT-Unterstützung |
| `pip install nexus-core[all-llms]` | + Alle LLM-Plattformen |
| `pip install nexus-core[mcp]` | + MCP-Server |
| `pip install nexus-core[video]` | + YouTube-/Vimeo-Transkript- und Metadatenextraktion |
| `pip install nexus-core[video-full]` | + Whisper-Transkription und visuelle Frameextraktion |
| `pip install nexus-core[jupyter]` | + Jupyter-Notebook-Unterstützung |
| `pip install nexus-core[pptx]` | + PowerPoint-Unterstützung |
| `pip install nexus-core[confluence]` | + Confluence-Wiki-Unterstützung |
| `pip install nexus-core[notion]` | + Notion-Seitenunterstützung |
| `pip install nexus-core[rss]` | + RSS-/Atom-Feed-Unterstützung |
| `pip install nexus-core[chat]` | + Slack-/Discord-Chatexport-Unterstützung |
| `pip install nexus-core[asciidoc]` | + AsciiDoc-Dokumentunterstützung |
| `pip install nexus-core[all]` | Alles aktiviert |

> **Visuelle Video-Abhängigkeiten (GPU-bewusst):** Nach der Installation von `nexus-core[video-full]` führen Sie
> `nexus-core create --setup` aus, um Ihre GPU automatisch zu erkennen und die richtige PyTorch-
...
Nexus Core unterstützt **12 LLM-Plattformen**, **8 RAG-/Vektor-Ziele**, **18 Quelltypen** und vollständige Funktionsparität für alle Ziele.
...
pip install nexus-core[video]        # Transkripte + Metadaten
pip install nexus-core[video-full]   # + Whisper-Transkription + visuelle Frameextraktion
...
```mermaid
graph LR
    A[Dokumentationswebsite] --> B[Nexus Core]
    B --> C[Scraper]
    B --> D[KI-Verbesserung]
    B --> E[Paketierer]
...
![Paketübersicht](docs/UML/exports/00_package_overview.png)
...
Vollständige UML-Diagramme: **[docs/UML_ARCHITECTURE.md](docs/UML_ARCHITECTURE.md)** | StarUML-Projekt: `docs/UML/nexus_core.mdj` | HTML-API-Referenz: `docs/UML/html/`
...
Nexus Core kann Skills automatisch für 19 KI-Programmieragenten installieren.
...
Nexus Core liefert einen MCP-Server für die Verwendung mit Claude Code, Cursor, Windsurf, VS Code + Cline oder IntelliJ IDEA.
...
# nexus-core list-configs  # In v3.7.0 nicht verfügbar
```
...
3. `~/.config/nexus-core/configs/` (Benutzerkonfigurationsverzeichnis)
...
```bash
nexus-core create https://docs.react.dev/ --preset quick      # Schnell, oberflächlich
nexus-core create https://docs.react.dev/ --preset standard   # Ausgewogen (Standard)
nexus-core create https://docs.react.dev/ --preset comprehensive # Tiefgehend, erschöpfend
```

### Lifecycle-Flags
```bash
nexus-core create https://docs.react.dev/ --dry-run    # Vorschau ohne Scraping
nexus-core create https://docs.react.dev/ --fresh      # Cache ignorieren, vollständiges Re-Scraping
nexus-core create https://docs.react.dev/ --resume     # Unterbrochenen Job fortsetzen
nexus-core create https://docs.react.dev/ --skip-scrape # Bestehende Ausgabe neu paketieren
```

### Health Check & Utilities
```bash
nexus-core doctor                    # Installation & Umgebung diagnostizieren
nexus-core sync-config               # Konfigurationsabweichung erkennen
nexus-core stream <source>           # Streaming-Ingestion für große Dokumentationen
nexus-core update output/react/      # Inkrementelles Update
nexus-core multilang <source>        # Mehrsprachige Skill-Generierung
nexus-core quality output/react/     # Qualitätsbericht (mit --threshold 7 als Gate: Exit-Code ungleich null unter 7/10)
```
...
|-------|--------------|-------|
| `browser` | `pip install "nexus-core[browser]"` | Headless Playwright für SPA-Websites |
| `embedding` | `pip install "nexus-core[embedding]"` | Embedding-Server-Unterstützung |
| `s3` / `gcs` / `azure` | `pip install "nexus-core[s3]"` usw. | Cloud-Storage-Upload |
| `rag-upload` | `pip install "nexus-core[rag-upload]"` | Kombinierte Vektordatenbank-Upload-Abhängigkeiten |
...
- **[docs/UML/nexus_core.mdj](docs/UML/nexus_core.mdj)** - StarUML-Projektdatei (mit [StarUML](https://staruml.io/) öffnen)
...
[![MseeP.ai Security Assessment Badge](https://mseep.net/pr/yusufkaraaslan-nexus-core-badge.png)](https://mseep.ai/app/yusufkaraaslan-nexus-core)
...
[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=nexus_core) — eine vollmodale, OpenAI-kompatible KI-Inferenzplattform. Nexus Core unterstützt sie als Paketierungs-/Verbesserungsziel via `--target atlas` mit `ATLAS_API_KEY`.
