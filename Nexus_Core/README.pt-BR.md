...
>
> Ajude a melhorar a tradução através do [GitHub Issue #260](https://github.com/yusufkaraaslan/Nexus_Core/issues/260)! Seu feedback é muito valioso para nós.

[![Versão](https://img.shields.io/badge/version-3.7.0-blue.svg)](https://github.com/yusufkaraaslan/Nexus_Core/releases)
...
[![Versão PyPI](https://badge.fury.io/py/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - Downloads](https://img.shields.io/pypi/dm/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - Versão Python](https://img.shields.io/pypi/pyversions/nexus-core.svg)](https://pypi.org/project/nexus-core/)
...
[![Site Oficial](https://img.shields.io/badge/Website-nexuscoreweb.com-blue.svg)](https://nexuscoreweb.com/)
[![Seguir no Twitter](https://img.shields.io/twitter/follow/_yUSyUS_?style=social)](https://x.com/_yUSyUS_)
[![GitHub Stars](https://img.shields.io/github/stars/yusufkaraaslan/Nexus_Core?style=social)](https://github.com/yusufkaraaslan/Nexus_Core)

**🧠 A camada de dados para sistemas de IA.** O Nexus Core transforma sites de documentação, repositórios GitHub, PDFs, vídeos, Jupyter Notebooks, wikis e mais de 10 outros tipos de fontes em ativos de conhecimento estruturado — prontos para alimentar AI Skills (Claude, Gemini, OpenAI), pipelines RAG (LangChain, LlamaIndex, Pinecone) e assistentes de programação com IA (Cursor, Windsurf, Cline) em minutos, não horas.

> 🌐 **[Visite SkillSeekersWeb.com](https://nexuscoreweb.com/)** - Navegue por mais de 24 configurações predefinidas, compartilhe suas configurações e acesse a documentação completa!
...
Nexus Core é um projeto multi-repositório. Aqui está onde tudo se encontra:

| Repositório | Descrição | Links |
|------------|-----------|-------|
| **[Nexus_Core](https://github.com/yusufkaraaslan/Nexus_Core)** | CLI principal e servidor MCP (este repo) | [PyPI](https://pypi.org/project/nexus-core/) |
| **[nexuscoreweb](https://github.com/yusufkaraaslan/nexuscoreweb)** | Website e documentação | [Site](https://nexuscoreweb.com/) |
| **[nexus-core-configs](https://github.com/yusufkaraaslan/nexus-core-configs)** | Repositório de configurações da comunidade | |
...
nexus-core package output/react --target ibm-bob     # → Diretório de skill IBM Bob
```
...
ao sair, você pode opcionalmente publicá-la de volta no [registro da comunidade](https://github.com/yusufkaraaslan/nexus-core-configs).
...
# Vídeo (YouTube, Vimeo ou arquivo local — requer nexus-core[video])
...
O Nexus Core substitui dias de pré-processamento manual com os seguintes passos:
...
O Nexus Core pode delegar para uma CLI de agente de programação na qual você já está logado, de modo que o aprimoramento roda no seu plano existente em vez de tokens de API cobrados:

```bash
nexus-core create <source> --agent codex     # CLI OpenAI Codex → seu ChatGPT Plus
nexus-core create <source> --agent claude    # Claude Code      → seu Claude Pro/Max
```
...
Todos eles expõem um endpoint `/v1` compatível com OpenAI. Aponte o Nexus Core para um deles com três variáveis de ambiente — ele detecta `OPENAI_API_KEY`, e o SDK da OpenAI respeita `OPENAI_BASE_URL` automaticamente:
...
- ✅ **Sistema de Configuração Multi-Token** - Gerencie múltiplas contas GitHub (pessoal, trabalho, OSS)
  - Armazenamento seguro de configurações em `~/.config/nexus-core/config.json` (permissões 600)
...
Gere o nexus-core como uma skill para uso dentro do seu agente de IA (Claude Code, Kimi, Codex, etc.):
...
- ✅ **Presets Definidos pelo Usuário** — Adicione workflows personalizados em `~/.config/nexus-core/workflows/`
...
# Gerencie presets
nexus-core workflows list                          # Liste todos (incluídos + usuário)
nexus-core workflows show security-focus           # Exiba conteúdo YAML
nexus-core workflows copy security-focus           # Copie para diretório do usuário para edição
nexus-core workflows add ./my-workflow.yaml        # Instale um preset personalizado
nexus-core workflows remove my-workflow            # Remova um preset do usuário
nexus-core workflows validate security-focus       # Valide a estrutura do preset
...
|-----------|----------------|
| `pip install nexus-core` | Coleta, análise GitHub, PDF, todas as plataformas |
| `pip install nexus-core[gemini]` | + Suporte ao Google Gemini |
| `pip install nexus-core[openai]` | + Suporte ao OpenAI ChatGPT |
| `pip install nexus-core[all-llms]` | + Todas as plataformas LLM |
| `pip install nexus-core[mcp]` | + Servidor MCP para Claude Code, Cursor, etc. |
| `pip install nexus-core[video]` | + Extração de transcrições e metadados do YouTube/Vimeo |
| `pip install nexus-core[video-full]` | + Transcrição Whisper e extração visual de frames |
| `pip install nexus-core[jupyter]` | + Suporte a Jupyter Notebook |
| `pip install nexus-core[pptx]` | + Suporte a PowerPoint |
| `pip install nexus-core[confluence]` | + Suporte a wiki Confluence |
| `pip install nexus-core[notion]` | + Suporte a páginas Notion |
| `pip install nexus-core[rss]` | + Suporte a feeds RSS/Atom |
| `pip install nexus-core[chat]` | + Suporte a exportação de chat Slack/Discord |
| `pip install nexus-core[asciidoc]` | + Suporte a documentos AsciiDoc |
| `pip install nexus-core[all]` | Tudo habilitado |

> **Dependências visuais de vídeo (detecção de GPU):** Após instalar `nexus-core[video-full]`, execute
> `nexus-core create --setup` para detectar automaticamente sua GPU e instalar a variante
...
O Nexus Core suporta **12 plataformas LLM**, **8 destinos RAG/vetoriais**, **18 tipos de fontes** e paridade completa de funcionalidades em todos os destinos.
...
pip install nexus-core[video]        # Transcrições + metadados
pip install nexus-core[video-full]   # + Whisper + extração visual de frames
...
```mermaid
graph LR
    A[Site de Documentação] --> B[Nexus Core]
    B --> C[Coletor]
...
Diagramas UML completos: **[docs/UML_ARCHITECTURE.md](docs/UML_ARCHITECTURE.md)** | Projeto StarUML: `docs/UML/nexus_core.mdj` | Referência de API em HTML: `docs/UML/html/`
...
O Nexus Core pode instalar automaticamente skills em 19 agentes de programação com IA.
...
O Nexus Core inclui um servidor MCP para uso com Claude Code, Cursor, Windsurf, VS Code + Cline ou IntelliJ IDEA.
...
# nexus-core list-configs  # Não disponível na v3.7.0
```
...
3. `~/.config/nexus-core/configs/` (diretório de configuração do usuário)
...
nexus-core create https://docs.react.dev/ --preset standard   # Equilibrado (padrão)
nexus-core create https://docs.react.dev/ --preset comprehensive # Profundo, exaustivo
```

### Flags de Ciclo de Vida
```bash
nexus-core create https://docs.react.dev/ --dry-run    # Pré-visualização sem coleta
nexus-core create https://docs.react.dev/ --fresh      # Ignorar cache, recoleta completa
nexus-core create https://docs.react.dev/ --resume     # Retomar trabalho interrompido
nexus-core create https://docs.react.dev/ --skip-scrape # Reempacotar saída existente
```

### Health Check e Utilitários
```bash
nexus-core doctor                    # Diagnosticar instalação e ambiente
nexus-core sync-config               # Detectar divergência de configuração
nexus-core stream <source>           # Ingestão por streaming para documentações grandes
nexus-core update output/react/      # Atualização incremental
nexus-core multilang <source>        # Geração de skills multilíngue
nexus-core quality output/react/     # Relatório de qualidade (adicione --threshold 7 para bloquear: saída diferente de zero abaixo de 7/10)
```
...
|-------|------------|-----------|
| `browser` | `pip install "nexus-core[browser]"` | Playwright headless para sites SPA |
| `embedding` | `pip install "nexus-core[embedding]"` | Suporte a servidor de embeddings |
| `s3` / `gcs` / `azure` | `pip install "nexus-core[s3]"` etc. | Upload para armazenamento em nuvem |
| `rag-upload` | `pip install "nexus-core[rag-upload]"` | Dependências combinadas de upload para bancos vetoriais |
...
- **[docs/UML/nexus_core.mdj](docs/UML/nexus_core.mdj)** - Arquivo de projeto StarUML (abra com [StarUML](https://staruml.io/))
...
[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=nexus_core) — uma plataforma de inferência de IA full-modal compatível com OpenAI. O Nexus Core a suporta como destino de empacotamento/aprimoramento via `--target atlas` com `ATLAS_API_KEY`.
