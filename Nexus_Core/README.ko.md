...
>
> 번역 개선에 도움을 주시려면 [GitHub Issue #260](https://github.com/yusufkaraaslan/Nexus_Core/issues/260)에 참여해 주세요! 여러분의 피드백은 매우 소중합니다.

[![버전](https://img.shields.io/badge/version-3.7.0-blue.svg)](https://github.com/yusufkaraaslan/Nexus_Core/releases)
...
[![PyPI 버전](https://badge.fury.io/py/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - 다운로드](https://img.shields.io/pypi/dm/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - Python 버전](https://img.shields.io/pypi/pyversions/nexus-core.svg)](https://pypi.org/project/nexus-core/)
...
[![공식 웹사이트](https://img.shields.io/badge/Website-nexuscoreweb.com-blue.svg)](https://nexuscoreweb.com/)
...
**🧠 AI 시스템을 위한 데이터 레이어.** Nexus Core는 문서 사이트, GitHub 저장소, PDF, 동영상, 노트북, 위키 등 18가지 소스 유형을 구조화된 지식 자산으로 변환합니다. 몇 시간이 아니라 몇 분 만에 AI 스킬(Claude, Gemini, OpenAI), RAG 파이프라인(LangChain, LlamaIndex, Pinecone), AI 코딩 어시스턴트(Cursor, Windsurf, Cline)에 활용할 수 있습니다.

> 🌐 **[SkillSeekersWeb.com 방문하기](https://nexuscoreweb.com/)** - 24개 이상의 프리셋 설정을 둘러보고, 설정을 공유하고, 전체 문서에 접근하세요!

> 📋 **[개발 로드맵 및 작업 보기](https://github.com/users/yusufkaraaslan/projects/2)** - 10개 카테고리에 걸친 134개 작업, 원하는 것을 선택하여 기여하세요!

## 🌐 에코시스템

Nexus Core는 다중 저장소 프로젝트입니다. 각 저장소의 역할:

| 저장소 | 설명 | 링크 |
|--------|------|------|
| **[Nexus_Core](https://github.com/yusufkaraaslan/Nexus_Core)** | 핵심 CLI & MCP 서버 (이 저장소) | [PyPI](https://pypi.org/project/nexus-core/) |
| **[nexuscoreweb](https://github.com/yusufkaraaslan/nexuscoreweb)** | 웹사이트 & 문서 | [사이트](https://nexuscoreweb.com/) |
| **[nexus-core-configs](https://github.com/yusufkaraaslan/nexus-core-configs)** | 커뮤니티 설정 저장소 | |
| **[nexus-core-action](https://github.com/yusufkaraaslan/nexus-core-action)** | GitHub Action CI/CD | |
| **[nexus-core-plugin](https://github.com/yusufkaraaslan/nexus-core-plugin)** | Claude Code 플러그인 | |
...
**Nexus Core는 범용 전처리 레이어**로, 원시 문서와 이를 활용하는 모든 AI 시스템 사이에 위치합니다. Claude 스킬을 구축하든, LangChain RAG 파이프라인을 만들든, Cursor `.cursorrules` 파일을 작성하든 — 데이터 준비 작업은 동일합니다. 한 번만 수행하면 모든 대상 플랫폼으로 내보낼 수 있습니다.

```bash
# 한 줄 명령 → 구조화된 지식 자산
nexus-core create https://docs.react.dev/
# 또는: nexus-core create facebook/react
# 또는: nexus-core create ./my-project

# 모든 AI 시스템으로 내보내기
nexus-core package output/react --target claude      # → Claude AI 스킬 (ZIP)
...
nexus-core package output/react --target ibm-bob     # → IBM Bob 스킬 디렉터리
```
...
감지 결과에 기존 프리셋이 없으면 AI가 새 설정을 생성합니다. 종료 시 [커뮤니티 레지스트리](https://github.com/yusufkaraaslan/nexus-core-configs)에 선택적으로 게시할 수 있습니다.
...
# 동영상 (YouTube, Vimeo 또는 로컬 파일 — nexus-core[video] 필요)
...
Nexus Core는 **AI 시스템을 위한 데이터 레이어**로, 18가지 소스 유형 — 문서 사이트, GitHub 저장소, PDF, 동영상, Jupyter 노트북, Word/EPUB/AsciiDoc 문서, OpenAPI 스펙, PowerPoint 프레젠테이션, RSS 피드, Man 페이지, Confluence 위키, Notion 페이지, Slack/Discord 내보내기 등 — 을 모든 AI 대상에 적합한 구조화된 지식 자산으로 변환합니다:
...
Nexus Core는 수일간의 수동 전처리 작업을 대체합니다:
...
Nexus Core는 이미 로그인되어 있는 코딩 에이전트 CLI를 호출할 수 있어, 종량제 API 토큰 대신 기존 요금제로 강화를 실행합니다:

```bash
nexus-core create <source> --agent codex     # OpenAI Codex CLI → ChatGPT Plus 요금제
nexus-core create <source> --agent claude    # Claude Code      → Claude Pro/Max 요금제
```
...
이들은 모두 OpenAI 호환 `/v1` 엔드포인트를 제공합니다. 환경 변수 세 개만 설정하면 됩니다 — Nexus Core가 `OPENAI_API_KEY`를 감지하고, OpenAI SDK는 `OPENAI_BASE_URL`을 자동으로 인식합니다:
...
nexus-core package output/django --target claude  # 또는 --target markdown
...
- ✅ **다중 토큰 설정 시스템** - 여러 GitHub 계정 관리 (개인, 업무, 오픈소스)
  - `~/.config/nexus-core/config.json`에 보안 설정 저장 (권한 600)
...
nexus-core 자체를 스킬로 생성하여 AI 에이전트(Claude Code, Kimi, Codex 등) 내에서 사용할 수 있습니다:
...
- ✅ **사용자 정의 프리셋** — `~/.config/nexus-core/workflows/`에 커스텀 워크플로 추가
...
# 프리셋 관리
nexus-core workflows list                          # 모든 항목 나열 (내장 + 사용자)
nexus-core workflows show security-focus           # YAML 내용 출력
nexus-core workflows copy security-focus           # 편집을 위해 사용자 디렉터리에 복사
nexus-core workflows add ./my-workflow.yaml        # 커스텀 프리셋 설치
nexus-core workflows remove my-workflow            # 사용자 프리셋 삭제
nexus-core workflows validate security-focus       # 프리셋 구조 유효성 검사
...
| 설치 명령 | 기능 |
|----------|------|
| `pip install nexus-core` | 스크래핑, GitHub 분석, PDF, 모든 플랫폼 |
| `pip install nexus-core[gemini]` | + Google Gemini 지원 |
| `pip install nexus-core[openai]` | + OpenAI ChatGPT 지원 |
| `pip install nexus-core[all-llms]` | + 모든 LLM 플랫폼 |
| `pip install nexus-core[mcp]` | + Claude Code, Cursor 등을 위한 MCP 서버 |
| `pip install nexus-core[video]` | + YouTube/Vimeo 자막 및 메타데이터 추출 |
| `pip install nexus-core[video-full]` | + Whisper 전사 및 시각 프레임 추출 |
| `pip install nexus-core[jupyter]` | + Jupyter 노트북 지원 |
| `pip install nexus-core[pptx]` | + PowerPoint 지원 |
| `pip install nexus-core[confluence]` | + Confluence 위키 지원 |
| `pip install nexus-core[notion]` | + Notion 페이지 지원 |
| `pip install nexus-core[rss]` | + RSS/Atom 피드 지원 |
| `pip install nexus-core[chat]` | + Slack/Discord 채팅 내보내기 지원 |
| `pip install nexus-core[asciidoc]` | + AsciiDoc 문서 지원 |
| `pip install nexus-core[all]` | 모든 기능 활성화 |

> **동영상 시각 종속성 (GPU 인식):** `nexus-core[video-full]` 설치 후,
> `nexus-core create --setup`을 실행하여 GPU를 자동 감지하고 올바른 PyTorch
...
Nexus Core는 **12개 LLM 플랫폼**, **8개 RAG/벡터 대상**, **18가지 소스 유형**을 지원하며 모든 대상에서 완전한 기능 동등성을 제공합니다.
...
```bash
# 동영상 지원 설치
pip install nexus-core[video]        # 자막 + 메타데이터
pip install nexus-core[video-full]   # + Whisper 전사 + 시각 프레임 추출
...
전체 UML 다이어그램: **[docs/UML_ARCHITECTURE.md](docs/UML_ARCHITECTURE.md)** | StarUML 프로젝트: `docs/UML/nexus_core.mdj` | HTML API 레퍼런스: `docs/UML/html/`
...
Nexus Core는 19개의 AI 코딩 에이전트에 스킬을 자동으로 설치할 수 있습니다.
...
Nexus Core는 Claude Code, Cursor, Windsurf, VS Code + Cline 또는 IntelliJ IDEA에서 사용할 수 있는 MCP 서버를 제공합니다.
...
```bash
# 모든 프리셋 나열
# nexus-core list-configs  # v3.7.0에서는 사용 불가
```
...
1. 제공된 정확한 경로
2. `./configs/` (현재 디렉터리)
3. `~/.config/nexus-core/configs/` (사용자 설정 디렉터리)
...
```bash
nexus-core create https://docs.react.dev/ --preset quick      # 빠름, 표면 수준
nexus-core create https://docs.react.dev/ --preset standard   # 균형 (기본값)
nexus-core create https://docs.react.dev/ --preset comprehensive # 심층, 완전 분석
```

### 라이프사이클 플래그
```bash
nexus-core create https://docs.react.dev/ --dry-run    # 스크래핑 없이 미리보기
nexus-core create https://docs.react.dev/ --fresh      # 캐시 무시, 전체 재스크래핑
nexus-core create https://docs.react.dev/ --resume     # 중단된 작업 재개
nexus-core create https://docs.react.dev/ --skip-scrape # 기존 출력 재패키징
```

### 상태 확인 및 유틸리티
```bash
nexus-core doctor                    # 설치 및 환경 진단
nexus-core sync-config               # 설정 드리프트 감지
nexus-core stream <source>           # 대용량 문서 스트리밍 수집
nexus-core update output/react/      # 증분 업데이트
nexus-core multilang <source>        # 다국어 스킬 생성
nexus-core quality output/react/     # 품질 리포트 (--threshold 7 추가로 게이트 적용: 7/10 미만이면 0이 아닌 종료 코드)
```
...
| Extra | 설치 | 용도 |
|-------|------|------|
| `browser` | `pip install "nexus-core[browser]"` | SPA 사이트를 위한 헤드리스 Playwright |
| `embedding` | `pip install "nexus-core[embedding]"` | 임베딩 서버 지원 |
| `s3` / `gcs` / `azure` | `pip install "nexus-core[s3]"` 등 | 클라우드 스토리지 업로드 |
| `rag-upload` | `pip install "nexus-core[rag-upload]"` | 통합 벡터 DB 업로드 종속성 |
...
- **[docs/UML/nexus_core.mdj](docs/UML/nexus_core.mdj)** - StarUML 프로젝트 파일 ([StarUML](https://staruml.io/)로 열기)
...
[![MseeP.ai 보안 평가 배지](https://mseep.net/pr/yusufkaraaslan-nexus-core-badge.png)](https://mseep.ai/app/yusufkaraaslan-nexus-core)
...
[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=nexus_core) — 풀 모달, OpenAI 호환 AI 추론 플랫폼입니다. Nexus Core는 `--target atlas`와 `ATLAS_API_KEY`를 통해 패키징/강화 대상으로 지원합니다.
