...
Synchronize a Nexus Core config file with the current state of a documentation site. Detects new pages, removed pages, and URL changes.

## Usage

```
/nexus-core:sync-config <config-path-or-name>
```
...
```
/nexus-core:sync-config configs/react.json
