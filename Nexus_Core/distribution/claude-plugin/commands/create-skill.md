...
```
/nexus-core:create-skill <source> [--preset <level>] [--output <dir>]
```

## Instructions

When the user provides a source via `$ARGUMENTS`, run the `nexus-core create` command to generate a skill.
...
4. Run the create command:
   ```bash
   nexus-core create "$SOURCE" --preset quick --output "$OUTPUT"
   ```
...
/nexus-core:create-skill pallets/flask -p standard
/nexus-core:create-skill ./docs/api.pdf
/nexus-core:create-skill https://youtube.com/watch?v=abc123
```
