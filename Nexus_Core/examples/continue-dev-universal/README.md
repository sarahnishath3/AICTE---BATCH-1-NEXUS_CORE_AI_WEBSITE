...
Complete example showing how to use Nexus Core to create IDE-agnostic context providers for Continue.dev across VS Code, JetBrains, and other IDEs.
...
```bash
# Build and run
docker build -t nexus-core-context .
docker run -d -p 8765:8765 nexus-core-context
...
kind: Deployment
metadata:
  name: nexus-core-context
spec:
  replicas: 3
  selector:
    matchLabels:
      app: nexus-core-context
  template:
    metadata:
      labels:
        app: nexus-core-context
    spec:
      containers:
      - name: context-server
        image: nexus-core-context:latest
        ports:
        - containerPort: 8765
---
apiVersion: v1
