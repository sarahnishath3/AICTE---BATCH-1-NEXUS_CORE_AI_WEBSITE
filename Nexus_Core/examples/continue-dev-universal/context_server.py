...
Supports multiple frameworks from Nexus Core output.
...
app = FastAPI(
    title="Nexus Core Context Server",
...
def load_framework_docs(framework: str) -> str:
    """
    Load framework documentation from Nexus Core output.
    ...
    raise FileNotFoundError(
        ...
        f"Run: nexus-core create --config configs/{framework}.json"
    )
...
async def root():
    ...
    return {
        "name": "Nexus Core Context Server",
        "description": "HTTP context provider for Continue.dev",
        "version": "1.0.0",
        "endpoints": {
...
async def list_frameworks() -> Dict[str, List[str]]:
    ...
    if not output_dir.exists():
        return {
            "available": [],
            "message": "No output directory found. Run nexus-core to generate documentation."
        }
...
def main():
    ...
    print("=" * 60)
    print("Nexus Core Context Server for Continue.dev")
    print("=" * 60)
