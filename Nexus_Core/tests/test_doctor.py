"""Tests for nexus-core doctor command (#316)."""
...
from nexus_core.cli.doctor import (
    CheckResult,
    check_api_keys,
    check_core_deps,
    check_git,
...
class TestCheckPackageInstalled:
    ...
    def test_fails_when_import_broken(self):
        with (
            patch.dict("sys.modules", {"nexus_core._version": None}),
...
class TestCheckGit:
    ...
    def test_warns_when_git_missing(self):
        with patch("nexus_core.cli.doctor.shutil.which", return_value=None):
            result = check_git()
