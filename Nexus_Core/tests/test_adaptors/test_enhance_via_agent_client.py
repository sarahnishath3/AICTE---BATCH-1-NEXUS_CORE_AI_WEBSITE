...
from nexus_core.cli.adaptors.claude import ClaudeAdaptor
from nexus_core.cli.adaptors.gemini import GeminiAdaptor
from nexus_core.cli.adaptors.openai import OpenAIAdaptor
...
class TestSharedEnhanceFlow:
    def test_success_saves_atomically_with_backup(self, skill_dir):
        ...
        with patch(
            "nexus_core.cli.agent_client.AgentClient", return_value=_mock_client("# Enhanced")
        ) as cls:
    ...
    def test_truncated_or_failed_call_leaves_skill_md_intact(self, skill_dir):
        ...
        with patch("nexus_core.cli.agent_client.AgentClient", return_value=_mock_client(None)):
    ...
    def test_empty_response_leaves_skill_md_intact(self, skill_dir):
        ...
        with patch("nexus_core.cli.agent_client.AgentClient", return_value=_mock_client("   ")):
    ...
    def test_no_references_fails_before_any_api_call(self, tmp_path):
        ...
        empty.mkdir()
        adaptor = ClaudeAdaptor()
        with patch("nexus_core.cli.agent_client.AgentClient") as cls:
    ...
    def test_sdk_fallback_to_local_is_refused(self, skill_dir):
        ...
        with patch("nexus_core.cli.agent_client.AgentClient", return_value=client):
    ...
    def test_per_adaptor_routing_parameters(self, skill_dir):
        ...
        for adaptor, key, expected in cases:
            with patch(
                "nexus_core.cli.agent_client.AgentClient",
    ...
    def test_openai_compatible_uses_platform_endpoint(self, skill_dir):
        from nexus_core.cli.adaptors.openai_compatible import OpenAICompatibleAdaptor
        ...
        with patch(
            "nexus_core.cli.agent_client.AgentClient", return_value=_mock_client("# E")
        ) as cls:
