...
# to absolute `nexus_core.cli.*` imports (Phase 5a); splitting_tools and
...
def _mod(name):
    return importlib.import_module(f"nexus_core.mcp.tools.{name}")


class TestSharedCommon(unittest.TestCase):
    def test_textcontent_is_shared(self):
        from nexus_core.mcp.tools import _common
