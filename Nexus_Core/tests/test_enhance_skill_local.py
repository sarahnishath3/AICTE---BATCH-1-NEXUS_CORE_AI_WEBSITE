...
from nexus_core.cli.enhance_skill_local import (
    AGENT_PRESETS,
    LocalSkillEnhancer,
    detect_terminal_app,
)
...
def _allow_executable(monkeypatch, name="my-agent"):
    monkeypatch.setattr(
        "nexus_core.cli.enhance_skill_local.shutil.which",
...
class TestMultiAgentSupport:
    ...
    def test_rejects_missing_executable(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("nexus_core.cli.enhance_skill_local.shutil.which", lambda _exe: None)
        skill_dir = _make_skill_dir(tmp_path)
...
class TestEnhanceDispatcher:
    ...
    def test_detect_api_target_anthropic(self, monkeypatch):
        ...
        from nexus_core.cli.enhance_skill_local import _detect_api_target
    ...
    def test_main_delegates_to_api_when_key_set(self, monkeypatch, tmp_path):
        ...
        import sys
        from nexus_core.cli.enhance_skill_local import main
        ...
        monkeypatch.setattr("nexus_core.cli.enhance_command.api_sdk_available", lambda _t: True)
        ...
        monkeypatch.setattr("nexus_core.cli.enhance_skill_local._run_api_enhance", fake_run_api)
        main()
        assert called_with == {
            "target": "gemini",
            "api_key": "AIza-test",
    ...
    def test_api_enhance_with_agent_flag_uses_real_skill_dir(self, monkeypatch, tmp_path):
        ...
        found: kimi'."""
        import sys
        from nexus_core.cli import enhance_skill
        from nexus_core.cli.enhance_skill_local import main
    ...
    def test_main_falls_back_to_local_when_sdk_missing(self, monkeypatch, tmp_path):
        ...
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        monkeypatch.setenv("MOONSHOT_API_KEY", "sk-moonshot-test")
        monkeypatch.setattr("nexus_core.cli.enhance_command.api_sdk_available", lambda _t: False)
        monkeypatch.setattr(sys, "argv", ["enhance_skill_local.py", str(skill_dir)])

        api_called = []
        monkeypatch.setattr(
            "nexus_core.cli.enhance_skill_local._run_api_enhance",
            lambda *a: api_called.append(a),
        )

        with patch("nexus_core.cli.enhance_skill_local.LocalSkillEnhancer") as mock_enhancer:
...
class TestHeadlessSuccessGate:
    ...
    def test_failed_retry_returns_false_without_crashing(self, tmp_path, monkeypatch):
        ...
        import nexus_core.cli.enhance_skill_local as esl
...
class TestRecursionGuard:
    ...
    `nexus-core create` must not spawn another real agent."""
    ...
    def test_run_agent_command_marks_child_env(self, tmp_path, monkeypatch):
        from nexus_core.cli import enhance_skill_local as mod
