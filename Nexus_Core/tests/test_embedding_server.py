...
try:
    ...
    from nexus_core.embedding.server import app as _embedding_app
...
@pytest.fixture
def mock_generator():
    with patch("nexus_core.embedding.server.generator") as mock_gen:
...
@pytest.fixture
def mock_cache():
    with patch("nexus_core.embedding.server.cache") as mock_cache:
...
class TestRoot:
    def test_root_endpoint(self, client):
        ...
        data = response.json()
        assert data["service"] == "Nexus Core Embedding API"
