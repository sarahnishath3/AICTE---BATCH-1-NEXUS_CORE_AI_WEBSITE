...
from nexus_core.cli.storage import (
    get_storage_adaptor,
    BaseStorageAdaptor,
    S3StorageAdaptor,
    GCSStorageAdaptor,
...
def test_get_storage_adaptor_s3():
    ...
    with patch("nexus_core.cli.storage.s3_storage.boto3"):
...
def test_get_storage_adaptor_gcs():
    ...
    with patch("nexus_core.cli.storage.gcs_storage.storage"):
...
def test_get_storage_adaptor_azure():
    ...
    with patch("nexus_core.cli.storage.azure_storage.BlobServiceClient"):
...
def test_s3_upload_file():
    ...
    with patch("nexus_core.cli.storage.s3_storage.boto3") as mock_boto3:
        # Setup mocks
        mock_client = Mock()
...
def test_gcs_upload_file():
    ...
    with patch("nexus_core.cli.storage.gcs_storage.storage") as mock_storage:
        # Setup mocks
        mock_client = Mock()
        mock_bucket = Mock()
        mock_blob = Mock()
...
def test_azure_upload_file():
    ...
    with patch("nexus_core.cli.storage.azure_storage.BlobServiceClient") as mock_blob_service:
        # Setup mocks
