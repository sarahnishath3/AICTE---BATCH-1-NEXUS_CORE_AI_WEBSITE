...
class TestSwiftErrorHandling:
    ...
    def test_pattern_validation_catches_invalid_weight(self):
        ...
        from nexus_core.cli.swift_patterns import _validate_patterns
    ...
    def test_malformed_regex_patterns_are_skipped(self):
        ...
        with patch("nexus_core.cli.language_detector.logger") as mock_logger:
            # Inject a language with a malformed pattern
            import nexus_core.cli.language_detector as ld_module
    ...
    def test_empty_swift_patterns_handled_gracefully(self):
        ...
        # Save all existing nexus_core.cli modules so we can restore them afterward.
        ...
        saved_cli_modules = {k: v for k, v in sys.modules.items() if "nexus_core.cli" in k}

        try:
            # Remove module from cache to force fresh import
            for mod in list(sys.modules.keys()):
                if "nexus_core.cli" in mod:
                    del sys.modules[mod]
            ...
            with patch.dict(
                "sys.modules",
                {
                    "nexus_core.cli.swift_patterns": type(
            ...
            # Remove the freshly imported nexus_core.cli modules from sys.modules
            for mod in list(sys.modules.keys()):
                if "nexus_core.cli" in mod:
                    del sys.modules[mod]
            ...
            # parent packages (e.g. nexus_core.cli.language_detector gets set as
            # an attribute on nexus_core.cli). Restore those attributes too so that
    ...
    def test_swift_validation_error_disables_detection(self):
        ...
        from nexus_core.cli import swift_patterns
