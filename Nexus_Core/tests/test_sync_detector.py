...
from nexus_core.sync.detector import ChangeDetector
from nexus_core.sync.models import ChangeType
...
class TestCheckPage:
    @patch("nexus_core.sync.detector.requests.get")
...
class TestHeaderChanges:
    @patch("nexus_core.sync.detector.requests.head")
...
class TestHeaderChangeNoValidators:
    def test_no_validators_assumes_changed(self):
        ...
        with patch("nexus_core.sync.detector.requests.head", return_value=resp):
            assert (
