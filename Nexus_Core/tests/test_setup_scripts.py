...
class TestSetupMCPScript:
    ...
    def test_references_correct_mcp_directory(self, script_content):
        """Test that script references src/nexus_core/mcp/ (v2.4.0 MCP 2025 upgrade)"""
        ...
        # Allow /mcp/ (as in src/nexus_core/mcp/) but not standalone mcp/
        ...
        # SHOULD reference nexus_core.mcp module (via -m flag) or src/nexus_core/mcp/
        # MCP 2025 uses: python3 -m nexus_core.mcp.server_fastmcp
        new_refs = re.findall(r"nexus_core\.mcp", script_content)
        assert len(new_refs) >= 2, (
            f"Expected at least 2 references to 'nexus_core.mcp' module, found {len(new_refs)}"
        )
    ...
    def test_server_py_path(self, script_content):
        ...
        # MCP 2025 uses: python3 -m nexus_core.mcp.server_fastmcp
        assert "nexus_core.mcp.server_fastmcp" in script_content, (
            "Should reference nexus_core.mcp.server_fastmcp module"
        )

        # Should NOT reference old server.py directly
        old_server_refs = re.findall(r"src/nexus_core/mcp/server\.py", script_content)
    ...
    def test_referenced_files_exist(self):
        ...
        assert Path("src/nexus_core/mcp/server_fastmcp.py").exists(), (
            "src/nexus_core/mcp/server_fastmcp.py should exist (MCP 2025)"
        )
        ...
        assert Path("src/nexus_core/mcp/server.py").exists(), (
            "src/nexus_core/mcp/server.py should exist (compatibility shim)"
        )
    ...
    def test_json_config_path_format(self, script_content):
        ...
        # MCP 2025 uses module import: python3 -m nexus_core.mcp.server_fastmcp
        # v2.7.0 PR #252 uses module reference format, not file path
        # Config should show the module reference: nexus_core.mcp.server_fastmcp
        assert "nexus_core.mcp.server_fastmcp" in script_content, (
            "Config should reference nexus_core.mcp.server_fastmcp module (MCP 2025 upgrade)"
        )
...
def test_mcp_directory_structure():
    ...
    mcp_dir = Path("src/nexus_core/mcp")
    assert mcp_dir.exists(), "src/nexus_core/mcp/ directory should exist"
    assert mcp_dir.is_dir(), "src/nexus_core/mcp should be a directory"
    assert (mcp_dir / "server.py").exists(), "src/nexus_core/mcp/server.py should exist"
    assert (mcp_dir / "__init__.py").exists(), "src/nexus_core/mcp/__init__.py should exist"
    ...
    if old_mcp.exists():
        ...
        assert not (old_mcp / "server.py").exists(), (
            "Old mcp/server.py should not exist - migrated to src/nexus_core/mcp/"
        )
    if old_skill_seeker_mcp.exists():
        assert not (old_skill_seeker_mcp / "server.py").exists(), (
            "Old skill_seeker_mcp/server.py should not exist - migrated to src/nexus_core/mcp/"
        )
