...
from nexus_core.mcp.tools._common import run_cli_main
from nexus_core.mcp.tools.packaging_tools import package_skill_tool, upload_skill_tool
from nexus_core.mcp.tools.scraping_tools import (
...
from nexus_core.mcp.tools.splitting_tools import generate_router, split_config
...
class TestRunCliMain:
    ...
    def test_patches_and_restores_sys_argv(self):
        ...
        assert rc == 0
        assert seen["argv"] == ["nexus-core", "cfg.json", "--max-discovery", "500"]
    ...
    def test_nexus_core_logger_output_is_captured(self):
        ...
        def fake_main():
            logging.getLogger("nexus_core.cli.something").warning("log line here")
            return 0
    ...
    def test_log_handler_removed_after_call(self):
        sk_logger = logging.getLogger("nexus_core")
...
class TestEstimatePagesToolInProcess:
    async def test_happy_path(self, monkeypatch):
        import nexus_core.cli.estimate_pages as estimate_pages
...
class TestDetectPatternsToolInProcess:
    async def test_happy_path_argv(self, monkeypatch):
        import nexus_core.cli.pattern_recognizer as pattern_recognizer
...
class TestExtractTestExamplesToolInProcess:
    async def test_happy_path_argv(self, monkeypatch):
        import nexus_core.cli.test_example_extractor as test_example_extractor
...
class TestBuildHowToGuidesToolInProcess:
    async def test_happy_path_argv(self, monkeypatch):
        import nexus_core.cli.how_to_guide_builder as how_to_guide_builder
...
class TestExtractConfigPatternsToolInProcess:
    async def test_argv_matches_real_parser_flags(self, monkeypatch, tmp_path):
        ...
        import nexus_core.cli.config_extractor as config_extractor
...
class TestSplitConfigToolInProcess:
    async def test_happy_path_argv(self, monkeypatch):
        import nexus_core.cli.split_config as split_config_cli
...
class TestGenerateRouterToolInProcess:
    ...
    async def test_happy_path_argv(self, monkeypatch, tmp_path):
        import nexus_core.cli.generate_router as generate_router_cli
...
class TestPackageSkillToolInProcess:
    async def test_happy_path(self, monkeypatch):
        import nexus_core.cli.package_skill as package_skill_cli
...
class TestUploadSkillToolInProcess:
    async def test_happy_path_system_exit_zero(self, monkeypatch):
        ...
        import nexus_core.cli.upload_skill as upload_skill_cli
