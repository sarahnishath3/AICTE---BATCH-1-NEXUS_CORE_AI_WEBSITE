...
- CLI interface (nexus-core install)
- Real config files
- Real file I/O
...
from nexus_core.mcp.tools.packaging_tools import install_skill_tool  # noqa: E402
...
class TestInstallSkillE2E:
    ...
    async def test_e2e_with_config_path_no_upload(
        ...
        with (
            patch("nexus_core.mcp.tools.scraping_tools.scrape_docs_tool") as mock_scrape,
            patch(
                "nexus_core.mcp.tools.packaging_tools.run_subprocess_with_streaming"
            ) as mock_enhance,
            patch("nexus_core.mcp.tools.packaging_tools.package_skill_tool") as mock_package,
        ):
    ...
    async def test_e2e_with_config_name_fetch(self, tmp_path):
        ...
        with (
            patch("nexus_core.mcp.tools.source_tools.fetch_config_tool") as mock_fetch,
            patch("nexus_core.mcp.tools.scraping_tools.scrape_docs_tool") as mock_scrape,
            patch(
                "nexus_core.mcp.tools.packaging_tools.run_subprocess_with_streaming"
            ) as mock_enhance,
    ...
    async def test_e2e_error_handling_scrape_failure(self, test_config_file):
        ...
        with patch("nexus_core.mcp.tools.scraping_tools.scrape_docs_tool") as mock_scrape:
            # Mock scrape failure
            mock_scrape.return_value = [
...
class TestInstallSkillCLI_E2E:
    """End-to-end tests for nexus-core install CLI"""
    ...
    async def test_cli_dry_run(self, test_config_file):
        ...
        from nexus_core.mcp.server import install_skill_tool
    ...
    def test_cli_validation_error_no_config(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "nexus_core.cli.install_skill"],
            capture_output=True,
            text=True,
        )
    ...
    def test_cli_help(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "nexus_core.cli.install_skill", "--help"],
            capture_output=True,
            text=True,
        )
    ...
    @pytest.mark.asyncio
    @patch("nexus_core.mcp.tools.scraping_tools.scrape_docs_tool")
    @patch("nexus_core.mcp.tools.packaging_tools.run_subprocess_with_streaming")
    @patch("nexus_core.mcp.tools.packaging_tools.package_skill_tool")
    ...
    def test_cli_via_unified_command(self, test_config_file):
        """E2E test: Using 'nexus-core install' unified CLI (dry-run mode)."""

        # Test the unified CLI entry point
        result = subprocess.run(
            ["nexus-core", "install", "--config", test_config_file, "--dry-run"],
            capture_output=True,
            text=True,
            timeout=30,
        )
...
class TestInstallSkillE2E_RealFiles:
    ...
    async def test_e2e_real_scrape_with_mocked_enhancement(self, real_test_config, tmp_path):
        ...
        with (
            ...
            patch("nexus_core.mcp.tools.packaging_tools.upload_skill_tool") as mock_upload,
            ...
            patch(
                "nexus_core.cli.enhance_skill_local.LocalSkillEnhancer.run",
                return_value=True,
            ),
            patch.dict(
                os.environ,
