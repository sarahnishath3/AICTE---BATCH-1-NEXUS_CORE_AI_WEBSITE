...
from nexus_core.cli.quality_checker import QualityReport, SkillQualityChecker
...
class TestQualityCheckerCLI(unittest.TestCase):
    ...
    def test_cli_help_output(self):
        ...
        try:
            result = subprocess.run(
                ["python3", "-m", "nexus_core.cli.quality_checker", "--help"],
                capture_output=True,
                text=True,
                timeout=5,
            )
    ...
    def test_cli_with_nonexistent_directory(self):
        ...
        result = subprocess.run(
            ["python3", "-m", "nexus_core.cli.quality_checker", "/nonexistent/path"],
            capture_output=True,
            text=True,
        )
