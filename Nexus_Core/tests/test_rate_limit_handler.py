...
from nexus_core.cli.rate_limit_handler import (
    RateLimitError,
    RateLimitHandler,
...
class TestRateLimitHandler:
    ...
    @patch("nexus_core.cli.rate_limit_handler.get_config_manager")
...
class TestConfigManagerIntegration:
    ...
    def test_config_manager_creates_default_config(self, tmp_path, monkeypatch):
        ...
        config_dir = tmp_path / ".config" / "nexus-core"
        progress_dir = tmp_path / ".local" / "share" / "nexus-core" / "progress"
    ...
    def test_add_and_retrieve_github_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ConfigManager,
            "PROGRESS_DIR",
            tmp_path / ".local" / "share" / "nexus-core" / "progress",
        )
    ...
    def test_get_next_profile(self, tmp_path, monkeypatch):
        ...
        config_dir = test_dir / ".config" / "nexus-core"
        monkeypatch.setattr(ConfigManager, "CONFIG_DIR", config_dir)
        monkeypatch.setattr(ConfigManager, "CONFIG_FILE", config_dir / "config.json")
        monkeypatch.setattr(
            ConfigManager,
            "PROGRESS_DIR",
            test_dir / ".local" / "share" / "nexus-core" / "progress",
        )
