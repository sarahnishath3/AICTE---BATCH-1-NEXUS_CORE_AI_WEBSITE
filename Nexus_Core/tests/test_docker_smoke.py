...
COPY . .
RUN pip install -e .
CMD ["nexus-core", "--version"]
""")

        subprocess.run(
            ["docker", "build", "-t", "nexus-core-test", "-f", str(dockerfile), "."],
            capture_output=True,
            text=True,
            cwd=str(tmp_path),
            timeout=120,
    ...
    def test_help_output(self, docker_available):
        """Verify nexus-core --help runs without error."""
        try:
            ...
            assert "nexus-core" in result.stdout.lower()
        except FileNotFoundError:
            pytest.skip("nexus-core CLI not on PATH")

    def test_version_output(self, docker_available):
        """Verify nexus-core --version outputs a version string."""
        try:
            result = subprocess.run(
                ["nexus-core", "--version"],
                capture_output=True,
