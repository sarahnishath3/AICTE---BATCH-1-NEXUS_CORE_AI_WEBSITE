"""Tests for the `nexus-core scan` command (issue #327).
...
from nexus_core.cli.scan_command import (
    Detection,
    ScanResult,
...
from nexus_core.cli.signal_collectors import Signal, SignalBundle
...
class TestResolveOrGenerate:
    ...
    def test_local_hit_returns_stamped_copy(self, tmp_path: Path):
        ...
        with patch(
            "nexus_core.cli.scan_command.resolve_config_path",
    ...
    def test_falls_back_to_ai_generation_on_miss(self, tmp_path: Path):
        ...
        with (
            ...
            patch(
                "nexus_core.cli.scan_command.generate_config_with_ai",
    ...
    def test_marks_generated_path_as_freshly_generated(self, tmp_path: Path):
        ...
        with patch(
            ...
            from nexus_core.cli.scan_command import resolve_or_generate_with_status
...
class TestMaybePublish:
    ...
    def test_no_prompt_skips_entirely(self, tmp_path: Path):
        ...
        with patch("nexus_core.cli.scan_command._submit_config", submit):
    ...
    def test_no_generated_configs_no_prompt(self, tmp_path: Path):
        ...
        with (
            patch("nexus_core.cli.scan_command._submit_config", submit),
            patch("nexus_core.cli.scan_command._prompt_async", prompt),
        ):
    ...
    def test_yes_calls_submit_with_path(self, tmp_path: Path):
        ...
        with (
            ...
            patch("nexus_core.cli.scan_command._submit_config", submit),
            patch("nexus_core.cli.scan_command._prompt_async", prompt),
            patch("nexus_core.cli.scan_command._find_existing_issue", find),
        ):
...
class TestRunScan:
    ...
    def test_resolved_path_emits_config_and_codebase(self, tmp_path: Path):
        ...
        with patch("nexus_core.cli.scan_command.resolve_config_path", return_value=canned):
            result = run_scan(
                proj,
                out_dir,
                agent_client=client,
    ...
    def test_unmapped_path_triggers_ai_generation(self, tmp_path: Path):
        ...
        with patch("nexus_core.cli.scan_command.resolve_config_path", return_value=None):
            result = run_scan(
                proj,
                out_dir,
                agent_client=client,
...
class TestArchiveRemovedConfigs:
    ...
    def test_no_removed_means_no_archive_dir(self, tmp_path: Path):
        ...
        client = self._client_returning(["react"])
        with patch("nexus_core.cli.scan_command.resolve_config_path", return_value=keep):
            result = run_scan(
                proj,
                out,
                agent_client=client,
...
class TestDryRun:
    ...
    def test_dry_run_does_not_call_ai_generator(self, tmp_path: Path):
        ...
        gen_mock = MagicMock()
        with (
            patch("nexus_core.cli.scan_command.resolve_config_path", return_value=None),
            patch("nexus_core.cli.scan_command.generate_config_with_ai", gen_mock),
        ):
            run_scan(
                proj,
                out_dir,
...
class TestCliRegistration:
    ...
    def test_scan_command_class_consumes_args_namespace(self):
        ...
        from nexus_core.cli.scan_command import ScanCommand
    ...
    def test_scan_parser_registered(self):
        from nexus_core.cli.parsers import get_parser_names

        assert "scan" in get_parser_names()

    def test_scan_parser_has_expected_arguments(self):
        import argparse
        from nexus_core.cli.parsers.scan_parser import ScanParser
...
class TestResolverUsesCanonicalCandidates:
    ...
    def test_resolver_tries_canonical_form_when_exact_misses(self, tmp_path: Path):
        ...
        with patch("nexus_core.cli.scan_command.resolve_config_path", side_effect=fake_resolve):
            from nexus_core.cli.scan_command import resolve_or_generate_with_status
    ...
    def test_resolver_reuses_existing_out_dir_config(self, tmp_path: Path):
        ...
        with (
            patch("nexus_core.cli.scan_command.resolve_config_path", resolve_mock),
            patch("nexus_core.cli.scan_command.generate_config_with_ai", gen_mock),
        ):
            from nexus_core.cli.scan_command import resolve_or_generate_with_status
...
class TestGenerateSchemaHintDepth:
    ...
    def test_schema_hint_depth_is_a_valid_level(self):
        ...
        from nexus_core.cli.config_validator import UniSkillConfigValidator
        from nexus_core.cli.scan_command import _GENERATE_SCHEMA_HINT
