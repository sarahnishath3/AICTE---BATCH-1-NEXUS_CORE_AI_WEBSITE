...
from nexus_core.cli.estimate_pages import estimate_pages
...
class TestEstimateParserDefault(unittest.TestCase):
    ...
    estimate_pages() treats as UNLIMITED — so `nexus-core estimate <cfg>`
    crawled the whole site instead of stopping at the 1000-page cap."""

    def test_max_discovery_has_finite_default(self):
        import argparse

        from nexus_core.cli.constants import DEFAULT_MAX_DISCOVERY
        from nexus_core.cli.parsers.estimate_parser import EstimateParser
...
class TestEstimatePagesCLI(unittest.TestCase):
    ...
    def test_cli_help_output(self):
        """Test that nexus-core estimate --help works"""
        import subprocess

        try:
            result = subprocess.run(
                ["nexus-core", "estimate", "--help"], capture_output=True, text=True, timeout=5
            )
            ...
            self.skipTest("nexus-core command not installed")

    def test_cli_executes_with_help_flag(self):
        """Test that nexus-core-estimate entry point works"""
        import subprocess
        ...
        try:
            ...
            self.skipTest("nexus-core-estimate command not installed")
    ...
    def test_cli_requires_config_argument(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["nexus-core", "estimate"], capture_output=True, text=True, timeout=5
            )
    ...
    def test_cli_all_flag_lists_configs(self):
        ...
        try:
            # Run with --all flag
            result = subprocess.run(
                ["nexus-core", "estimate", "--all"], capture_output=True, text=True, timeout=10
            )
    ...
    def test_cli_all_flag_with_direct_entry_point(self):
        """Test --all flag works with nexus-core-estimate entry point"""
        import subprocess
