"""Tests for signal collectors used by `nexus-core scan` (issue #327).
...
from nexus_core.cli.signal_collectors import (
...
class TestCollectManifests:
    ...
    def test_reads_only_max_bytes_from_disk(self, tmp_path: Path):
        ...
        2 KB."""
        from nexus_core.cli.signal_collectors import _safe_read_text
...
class TestCollectSourceSamples_legacy_alias:
    ...
    def test_alias_returns_same_signals(self, tmp_path: Path):
        from nexus_core.cli.signal_collectors import collect_source_imports
...
class TestGetGitRemote:
    ...
    def test_returns_remote_when_git_repo(self, tmp_path: Path):
        ...
        with patch("nexus_core.cli.signal_collectors.subprocess.run") as run:
    ...
    def test_returns_none_on_subprocess_failure(self, tmp_path: Path):
        with patch(
            "nexus_core.cli.signal_collectors.subprocess.run",
