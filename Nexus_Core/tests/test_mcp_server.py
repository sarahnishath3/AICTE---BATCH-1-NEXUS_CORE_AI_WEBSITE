...
if MCP_AVAILABLE:
    ...
    try:
        from nexus_core.mcp import server as skill_seeker_server
...
class TestEstimatePagesTool(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("nexus_core.mcp.tools._common.run_cli_main")
...
class TestScrapeDocsTool(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("nexus_core.mcp.tools.scraping_tools._run_converter")
    @patch("nexus_core.cli.skill_converter.get_converter")
    async def test_scrape_docs_basic(self, mock_get_converter, mock_run_converter):
        ...
        from nexus_core.mcp.tools.scraping_tools import TextContent
