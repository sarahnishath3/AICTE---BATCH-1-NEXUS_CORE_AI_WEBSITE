...
if MCP_AVAILABLE:
    from nexus_core.mcp.server import scrape_docs_tool, validate_config_tool
else:
...
async def test_mcp_scrape_docs_unified_skip_scrape_warns():
    ...
    try:
        with (
            patch("nexus_core.cli.skill_converter.get_converter") as mock_get_converter,
            patch("nexus_core.mcp.tools.scraping_tools._run_converter") as mock_run_converter,
        ):
