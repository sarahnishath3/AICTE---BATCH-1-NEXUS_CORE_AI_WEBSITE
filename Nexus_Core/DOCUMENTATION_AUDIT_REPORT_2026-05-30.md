# Nexus Core Documentation Audit Report
**Date:** 2026-05-30  
...
|---|---|
| `nexus-core scrape --config ...` | `nexus-core create --config ...` |
| `nexus-core scrape --url ...` | `nexus-core create https://...` |
| `nexus-core video --url ...` | `nexus-core create ... --video-url ...` |
| `nexus-core video --setup` | `nexus-core create --setup` |
| `nexus-core pdf --pdf ...` | `nexus-core create ... --pdf ...` |
| `nexus-core github --repo ...` | `nexus-core create ...` (auto-detect) |
| `nexus-core unified --config ...` | `nexus-core create configs/..._unified.json` |
| `nexus-core confluence --space ...` | `nexus-core create ... --space-key ...` |
| `nexus-core notion --database-id ...` | `nexus-core create ... --database-id ...` |
| `nexus-core chat --export-dir ...` | `nexus-core create ... --chat-export-path ...` |
| `nexus-core list-configs` | **Does not exist** |
| `nexus-core analyze --directory ...` | **Does not exist** |

#### C. Wrong CLI Flags in Examples
- `nexus-core video --url ...` → should use `--video-url`
- `nexus-core video --playlist ...` → should use `--video-playlist`
- `nexus-core confluence --space TEAM` → should use `--space-key`
- `nexus-core chat --export-dir ./slack-export` → should use `--chat-export-path`
- `nexus-core package ... --format chroma/faiss/qdrant` → should use `--target`
...
- `nexus-core doctor` (health check)
- `nexus-core sync-config` (config drift detection)
- `nexus-core stream` (streaming ingestion)
- `nexus-core update` (incremental updater)
- `nexus-core multilang` (multi-language support)
- `nexus-core quality` (quality scoring)
- `nexus-core extract-test-examples`
...
- Coverage paths wrong: `--cov=cli --cov=mcp` should be `--cov=src/nexus_core`
...
- Shows `nexus-core package --target langchain` — `langchain` is a `--format` value, not `--target`
...
- `KUBERNETES_GUIDE.md` uses `skillseekers` namespace; `KUBERNETES_DEPLOYMENT.md` uses `nexus-core`
- **Recommendation**: Merge into single file
...
- `integrations/INTEGRATIONS.md` says "Nexus Core Version: v2.10.0+"
- `integrations/LANGCHAIN.md` says "Nexus Core Version: v2.9.0+"
...
Intelligence System roadmap (4 files, Jan 2026). No `nexus_core/intelligence/` package exists yet. These are **future design docs** — correctly kept as research/planning.
...
- `nexus-core scrape --config ...` → should be `nexus-core create --config ...`
- `nexus-core github --repo ...` → should be `nexus-core create ...`

#### windsurf-fastapi-context/
- References `--split-rules` and `--max-chars` flags on `nexus-core package`
- **These flags do not exist**

#### Other Example Issues
- Many reference `nexus-core v2.10.0` — current is 3.6.0
...
- `nexus-core-configs/README.md` (root copy): Claims **24 configs / 7 categories**
- **These wildly diverge**

#### CONTRIBUTING.md (configs_repo)
- References `nexus-core scrape --config` — command removed

### 8.4 nexus-core-configs/ — 🔴 COUNTS DON'T ADD UP
...
- `python3 mcp/server.py` → should be `python -m nexus_core.mcp.server_fastmcp`
...
### 9.3 skills/nexus-core/SKILL.md — ⚠️ STALE COUNT
- Claims **35 MCP tools** — actual is **40**
- Otherwise well-structured

### 9.4 src/nexus_core/mcp/README.md — 🔴 INCONSISTENT & BROKEN LINKS
...
| `nexus-core-configs/README.md` (root copy) | Wildly diverges from submodule | Remove or sync with `api/configs_repo/` |
...
4. **Update ALL example READMEs** to use `nexus-core create` instead of removed `scrape`/`github`/`video`/`pdf`/`unified` commands
...
15. **Fix `src/nexus_core/mcp/README.md`** broken links and inconsistent counts
...
18. **Sync or remove `nexus-core-configs/README.md`** (root copy)
...
| nexus-core-configs/ | 13 | Counts don't add up |
