...
>
> 欢迎通过 [GitHub Issue #260](https://github.com/yusufkaraaslan/Nexus_Core/issues/260) 帮助改进翻译！您的反馈对我们非常宝贵。

[![版本](https://img.shields.io/badge/version-3.7.0-blue.svg)](https://github.com/yusufkaraaslan/Nexus_Core/releases)
...
[![PyPI 版本](https://badge.fury.io/py/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - 下载量](https://img.shields.io/pypi/dm/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - Python 版本](https://img.shields.io/pypi/pyversions/nexus-core.svg)](https://pypi.org/project/nexus-core/)
...
[![官方网站](https://img.shields.io/badge/Website-nexuscoreweb.com-blue.svg)](https://nexuscoreweb.com/)
...
**🧠 AI 系统的数据层。** Nexus Core 将文档网站、GitHub 仓库、PDF、视频、笔记本、Wiki 等 18 种来源类型转换为结构化知识资产——可在几分钟（而非几小时）内为 AI 技能（Claude、Gemini、OpenAI）、RAG 流水线（LangChain、LlamaIndex、Pinecone）和 AI 编程助手（Cursor、Windsurf、Cline）提供支持。

> 🌐 **[访问 SkillSeekersWeb.com](https://nexuscoreweb.com/)** - 浏览 24+ 个预设配置，分享您的配置，访问完整文档！

> 📋 **[查看开发路线图和任务](https://github.com/users/yusufkaraaslan/projects/2)** - 10 个类别的 134 个任务，选择任意一个参与贡献！

## 🌐 生态系统

Nexus Core 是一个多仓库项目。以下是各部分所在位置：

| 仓库 | 描述 | 链接 |
|------|------|------|
| **[Nexus_Core](https://github.com/yusufkaraaslan/Nexus_Core)** | 核心 CLI 和 MCP 服务器（本仓库） | [PyPI](https://pypi.org/project/nexus-core/) |
| **[nexuscoreweb](https://github.com/yusufkaraaslan/nexuscoreweb)** | 网站和文档 | [在线](https://nexuscoreweb.com/) |
| **[nexus-core-configs](https://github.com/yusufkaraaslan/nexus-core-configs)** | 社区配置仓库 | |
| **[nexus-core-action](https://github.com/yusufkaraaslan/nexus-core-action)** | GitHub Action CI/CD | |
| **[nexus-core-plugin](https://github.com/yusufkaraaslan/nexus-core-plugin)** | Claude Code 插件 | |
...
**Nexus Core 是通用预处理层**，位于原始文档和所有使用它的 AI 系统之间。无论您是在构建 Claude 技能、LangChain RAG 流水线，还是 Cursor `.cursorrules` 文件——数据准备工作完全相同。只需执行一次，即可导出到所有目标平台。

```bash
# 一条命令 → 结构化知识资产
nexus-core create https://docs.react.dev/
# 或: nexus-core create facebook/react
# 或: nexus-core create ./my-project

# 导出到任意 AI 系统
nexus-core package output/react --target claude      # → Claude AI 技能 (ZIP)
...
nexus-core package output/react --target ibm-bob     # → IBM Bob 技能目录
```
...
如果某个检测结果没有现成预设，AI 会生成全新配置；退出时您可以选择将其发布回 [社区注册表](https://github.com/yusufkaraaslan/nexus-core-configs)。
...
# 视频（YouTube、Vimeo 或本地文件 — 需要 nexus-core[video]）
...
Nexus Core 是 **AI 系统的数据层**，将 18 种来源类型——文档网站、GitHub 仓库、PDF、视频、Jupyter 笔记本、Word/EPUB/AsciiDoc 文档、OpenAPI 规范、PowerPoint 演示文稿、RSS 订阅源、Man 手册页、Confluence 维基、Notion 页面、Slack/Discord 聊天记录等——转换为适用于所有 AI 目标的结构化知识资产：
...
Nexus Core 可以调用您已登录的编程代理 CLI，因此增强会使用您现有的订阅计划而非按量计费的 API token：

```bash
nexus-core create <source> --agent codex     # OpenAI Codex CLI → 您的 ChatGPT Plus
nexus-core create <source> --agent claude    # Claude Code      → 您的 Claude Pro/Max
```
...
这些提供商都暴露 OpenAI 兼容的 `/v1` 端点。只需三个环境变量即可让 Nexus Core 指向其中之一——它会检测 `OPENAI_API_KEY`，而 OpenAI SDK 会自动识别 `OPENAI_BASE_URL`：
...
nexus-core package output/django --target claude  # 或 --target markdown
...
- ✅ **多 Token 配置系统** - 管理多个 GitHub 账号（个人、工作、开源）
  - 安全配置存储在 `~/.config/nexus-core/config.json`（权限 600）
...
将 nexus-core 自身生成为技能，在您的 AI 代理（Claude Code、Kimi、Codex 等）中使用：
...
- ✅ **用户自定义预设** — 将自定义工作流添加到 `~/.config/nexus-core/workflows/`
...
# 管理预设
nexus-core workflows list                          # 列出所有（内置 + 用户）
nexus-core workflows show security-focus           # 显示 YAML 内容
nexus-core workflows copy security-focus           # 复制到用户目录以便编辑
nexus-core workflows add ./my-workflow.yaml        # 安装自定义预设
nexus-core workflows remove my-workflow            # 删除用户预设
nexus-core workflows validate security-focus       # 验证预设结构
...
| 安装命令 | 功能 |
|---------|------|
| `pip install nexus-core` | 抓取、GitHub 分析、PDF、所有平台 |
| `pip install nexus-core[gemini]` | + Google Gemini 支持 |
| `pip install nexus-core[openai]` | + OpenAI ChatGPT 支持 |
| `pip install nexus-core[all-llms]` | + 所有 LLM 平台 |
| `pip install nexus-core[mcp]` | + 适用于 Claude Code、Cursor 等的 MCP 服务器 |
| `pip install nexus-core[video]` | + YouTube/Vimeo 字幕和元数据提取 |
| `pip install nexus-core[video-full]` | + Whisper 转录和视觉帧提取 |
| `pip install nexus-core[jupyter]` | + Jupyter 笔记本支持 |
| `pip install nexus-core[pptx]` | + PowerPoint 支持 |
| `pip install nexus-core[confluence]` | + Confluence 维基支持 |
| `pip install nexus-core[notion]` | + Notion 页面支持 |
| `pip install nexus-core[rss]` | + RSS/Atom 订阅源支持 |
| `pip install nexus-core[chat]` | + Slack/Discord 聊天记录支持 |
| `pip install nexus-core[asciidoc]` | + AsciiDoc 文档支持 |
| `pip install nexus-core[all]` | 全部功能 |

> **视频视觉依赖（GPU 感知）：** 安装 `nexus-core[video-full]` 后，运行
> `nexus-core create --setup` 自动检测您的 GPU 并安装正确的 PyTorch
...
Nexus Core 支持 **12 个 LLM 平台**、**8 个 RAG/向量目标**、**18 种来源类型**，并在所有目标上实现完全的功能对等。
...
```bash
# 安装视频支持
pip install nexus-core[video]        # 字幕 + 元数据
pip install nexus-core[video-full]   # + Whisper 转录 + 视觉帧提取
...
完整 UML 图：**[docs/UML_ARCHITECTURE.md](docs/UML_ARCHITECTURE.md)** | StarUML 项目：`docs/UML/nexus_core.mdj` | HTML API 参考：`docs/UML/html/`
...
Nexus Core 可自动将技能安装到 19 个 AI 编程代理。
...
Nexus Core 提供 MCP 服务器，可在 Claude Code、Cursor、Windsurf、VS Code + Cline 或 IntelliJ IDEA 中使用。
...
```bash
# 列出所有预设
# nexus-core list-configs  # v3.7.0 中不可用
```
...
1. 提供的确切路径
2. `./configs/`（当前目录）
3. `~/.config/nexus-core/configs/`（用户配置目录）
...
```bash
nexus-core create https://docs.react.dev/ --preset quick      # 快速，浅层分析
nexus-core create https://docs.react.dev/ --preset standard   # 平衡（默认）
nexus-core create https://docs.react.dev/ --preset comprehensive # 深度，详尽分析
```

### 生命周期标志
```bash
nexus-core create https://docs.react.dev/ --dry-run    # 不抓取预览
nexus-core create https://docs.react.dev/ --fresh      # 忽略缓存，完整重新抓取
nexus-core create https://docs.react.dev/ --resume     # 恢复中断的任务
nexus-core create https://docs.react.dev/ --skip-scrape # 重新打包现有输出
```

### 健康检查与工具
```bash
nexus-core doctor                    # 诊断安装与环境
nexus-core sync-config               # 配置漂移检测
nexus-core stream <source>           # 大型文档流式摄取
nexus-core update output/react/      # 增量更新
nexus-core multilang <source>        # 多语言技能生成
nexus-core quality output/react/     # 质量报告（添加 --threshold 7 设置门槛：低于 7/10 时非零退出）
```
...
| Extra | 安装 | 用途 |
|-------|------|------|
| `browser` | `pip install "nexus-core[browser]"` | 适用于 SPA 网站的无头 Playwright |
| `embedding` | `pip install "nexus-core[embedding]"` | 嵌入服务器支持 |
| `s3` / `gcs` / `azure` | `pip install "nexus-core[s3]"` 等 | 云存储上传 |
| `rag-upload` | `pip install "nexus-core[rag-upload]"` | 组合向量数据库上传依赖 |
...
- **[docs/UML/nexus_core.mdj](docs/UML/nexus_core.mdj)** - StarUML 项目文件（使用 [StarUML](https://staruml.io/) 打开）
...
[![MseeP.ai 安全评估徽章](https://mseep.net/pr/yusufkaraaslan-nexus-core-badge.png)](https://mseep.ai/app/yusufkaraaslan-nexus-core)
...
[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=nexus_core) — 全模态、OpenAI 兼容的 AI 推理平台。Nexus Core 通过 `--target atlas`（配置 `ATLAS_API_KEY`）支持将其作为打包/增强目标。
