...
>
> अनुवाद सुधारने में मदद करने के लिए [GitHub Issue #260](https://github.com/yusufkaraaslan/Nexus_Core/issues/260) पर सम्पर्क करें! आपकी प्रतिक्रिया हमारे लिए बहुत मूल्यवान है।

[![संस्करण](https://img.shields.io/badge/version-3.7.0-blue.svg)](https://github.com/yusufkaraaslan/Nexus_Core/releases)
...
[![PyPI संस्करण](https://badge.fury.io/py/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - डाउनलोड](https://img.shields.io/pypi/dm/nexus-core.svg)](https://pypi.org/project/nexus-core/)
[![PyPI - Python संस्करण](https://img.shields.io/pypi/pyversions/nexus-core.svg)](https://pypi.org/project/nexus-core/)
...
[![वेबसाइट](https://img.shields.io/badge/Website-nexuscoreweb.com-blue.svg)](https://nexuscoreweb.com/)
...
**🧠 AI सिस्टम के लिए डेटा लेयर।** Nexus Core डॉक्यूमेंटेशन वेबसाइटों, GitHub रिपॉज़िटरी, PDF, वीडियो, Jupyter नोटबुक, विकी और 10+ अन्य स्रोत प्रकारों को संरचित ज्ञान संपत्ति में बदलता है—जो मिनटों में AI कौशल (Claude, Gemini, OpenAI), RAG पाइपलाइन (LangChain, LlamaIndex, Pinecone) और AI कोडिंग सहायकों (Cursor, Windsurf, Cline) को शक्ति प्रदान कर सकती हैं।

> 🌐 **[SkillSeekersWeb.com पर जाएँ](https://nexuscoreweb.com/)** - 24+ प्रीसेट कॉन्फ़िगरेशन ब्राउज़ करें, अपने कॉन्फ़िग साझा करें और पूर्ण दस्तावेज़ देखें!
...
Nexus Core एक मल्टी-रिपॉजिटरी प्रोजेक्ट है। यहां सब कुछ मौजूद है:

| रिपॉजिटरी | विवरण | लिंक |
|-----------|--------|------|
| **[Nexus_Core](https://github.com/yusufkaraaslan/Nexus_Core)** | कोर CLI और MCP सर्वर (यह रिपो) | [PyPI](https://pypi.org/project/nexus-core/) |
| **[nexuscoreweb](https://github.com/yusufkaraaslan/nexuscoreweb)** | वेबसाइट और डॉक्यूमेंटेशन | [साइट](https://nexuscoreweb.com/) |
| **[nexus-core-configs](https://github.com/yusufkaraaslan/nexus-core-configs)** | सामुदायिक कॉन्फिग रिपॉजिटरी | |
| **[nexus-core-action](https://github.com/yusufkaraaslan/nexus-core-action)** | GitHub Action CI/CD | |
| **[nexus-core-plugin](https://github.com/yusufkaraaslan/nexus-core-plugin)** | Claude Code प्लगइन | |
| **[homebrew-nexus-core](https://github.com/yusufkaraaslan/homebrew-nexus-core)** | macOS के लिए Homebrew tap | |
...
**Nexus Core एक सार्वभौमिक प्रीप्रोसेसिंग लेयर है** जो कच्चे दस्तावेज़ों और उनका उपयोग करने वाले सभी AI सिस्टम के बीच स्थित है। चाहे आप Claude कौशल, LangChain RAG पाइपलाइन, या Cursor `.cursorrules` फ़ाइल बना रहे हों—डेटा तैयारी पूरी तरह समान है। बस एक बार करें, और सभी लक्ष्यों पर निर्यात करें।

```bash
# एक कमांड → संरचित ज्ञान संपत्ति
nexus-core create https://docs.react.dev/
# या: nexus-core create facebook/react
# या: nexus-core create ./my-project

# किसी भी AI सिस्टम पर निर्यात करें
nexus-core package output/react --target claude      # → Claude AI कौशल (ZIP)
...
nexus-core package output/react --target ibm-bob     # → IBM Bob कौशल डायरेक्टरी
```
...
करता है; समाप्ति पर आप इसे वैकल्पिक रूप से [सामुदायिक रजिस्ट्री](https://github.com/yusufkaraaslan/nexus-core-configs) में प्रकाशित कर सकते हैं।
...
# वीडियो (YouTube, Vimeo, या स्थानीय फ़ाइल — nexus-core[video] आवश्यक)
...
दिनों की मैन्युअल प्रीप्रोसेसिंग के बजाय, Nexus Core:
...
## Nexus Core का उपयोग क्यों करें?
...
Nexus Core उस कोडिंग-एजेंट CLI को कॉल कर सकता है जिसमें आप पहले से लॉग इन हैं, ताकि एन्हांसमेंट मीटर्ड API टोकन के बजाय आपके मौजूदा प्लान पर चले:

```bash
nexus-core create <source> --agent codex     # OpenAI Codex CLI → आपका ChatGPT Plus
nexus-core create <source> --agent claude    # Claude Code      → आपका Claude Pro/Max
```
...
ये सभी एक OpenAI-संगत `/v1` एंडपॉइंट प्रदान करते हैं। तीन पर्यावरण चर के साथ Nexus Core को किसी एक पर पॉइंट करें — यह `OPENAI_API_KEY` का पता लगाता है, और OpenAI SDK स्वचालित रूप से `OPENAI_BASE_URL` का सम्मान करता है:
...
nexus-core package output/django --target claude  # या --target markdown
...
- ✅ **बहु-टोकन कॉन्फ़िगरेशन सिस्टम** - एकाधिक GitHub खातों का प्रबंधन (व्यक्तिगत, कार्य, OSS)
  - `~/.config/nexus-core/config.json` पर सुरक्षित कॉन्फ़िग भंडारण (600 अनुमतियाँ)
...
Nexus Core को अपने AI एजेंट (Claude Code, Kimi, Codex आदि) में उपयोग के लिए एक कौशल के रूप में जनरेट करें:
...
- ✅ **उपयोगकर्ता-परिभाषित प्रीसेट** — `~/.config/nexus-core/workflows/` में कस्टम वर्कफ़्लो जोड़ें
...
# प्रीसेट प्रबंधन
nexus-core workflows list                          # सभी सूचीबद्ध करें (बंडल + उपयोगकर्ता)
nexus-core workflows show security-focus           # YAML सामग्री प्रिंट करें
nexus-core workflows copy security-focus           # संपादन के लिए उपयोगकर्ता डायरेक्टरी में कॉपी करें
nexus-core workflows add ./my-workflow.yaml        # कस्टम प्रीसेट इंस्टॉल करें
nexus-core workflows remove my-workflow            # उपयोगकर्ता प्रीसेट हटाएँ
nexus-core workflows validate security-focus       # प्रीसेट संरचना मान्य करें
...
|---------------|----------|
| `pip install nexus-core` | स्क्रैपिंग, GitHub विश्लेषण, PDF, सभी प्लेटफ़ॉर्म |
| `pip install nexus-core[gemini]` | + Google Gemini समर्थन |
| `pip install nexus-core[openai]` | + OpenAI ChatGPT समर्थन |
| `pip install nexus-core[all-llms]` | + सभी LLM प्लेटफ़ॉर्म |
| `pip install nexus-core[mcp]` | + MCP सर्वर |
| `pip install nexus-core[video]` | + YouTube/Vimeo ट्रांसक्रिप्ट और मेटाडेटा निष्कर्षण |
| `pip install nexus-core[video-full]` | + Whisper ट्रांसक्रिप्शन और विज़ुअल फ़्रेम निष्कर्षण |
| `pip install nexus-core[jupyter]` | + Jupyter Notebook समर्थन |
| `pip install nexus-core[pptx]` | + PowerPoint समर्थन |
| `pip install nexus-core[confluence]` | + Confluence विकी समर्थन |
| `pip install nexus-core[notion]` | + Notion पेज समर्थन |
| `pip install nexus-core[rss]` | + RSS/Atom फ़ीड समर्थन |
| `pip install nexus-core[chat]` | + Slack/Discord चैट एक्सपोर्ट समर्थन |
| `pip install nexus-core[asciidoc]` | + AsciiDoc दस्तावेज़ समर्थन |
| `pip install nexus-core[all]` | सब कुछ सक्षम |

> **वीडियो विज़ुअल डिपेंडेंसी (GPU-सक्षम):** `nexus-core[video-full]` इंस्टॉल करने के बाद,
> `nexus-core create --setup` चलाएँ ताकि आपका GPU स्वचालित रूप से पहचाना जा सके और सही PyTorch
...
Nexus Core **12 LLM प्लेटफ़ॉर्म**, **8 RAG/वेक्टर लक्ष्य**, **18 स्रोत प्रकार** और सभी लक्ष्यों पर पूर्ण फ़ीचर समानता का समर्थन करता है।
...
pip install nexus-core[video]        # ट्रांसक्रिप्ट + मेटाडेटा
pip install nexus-core[video-full]   # + Whisper ट्रांसक्रिप्शन + विज़ुअल फ़्रेम निष्कर्षण
...
```mermaid
graph LR
    A[डॉक्यूमेंटेशन वेबसाइट] --> B[Nexus Core]
    B --> C[स्क्रैपर]
    B --> D[AI एन्हांसमेंट]
    B --> E[पैकेजर]
...
पूर्ण UML आरेख: **[docs/UML_ARCHITECTURE.md](docs/UML_ARCHITECTURE.md)** | StarUML प्रोजेक्ट: `docs/UML/nexus_core.mdj` | HTML API संदर्भ: `docs/UML/html/`
...
Nexus Core स्वचालित रूप से 19 AI कोडिंग एजेंट में कौशल इंस्टॉल कर सकता है।
...
Nexus Core Claude Code, Cursor, Windsurf, VS Code + Cline, या IntelliJ IDEA से उपयोग के लिए MCP सर्वर प्रदान करता है।
...
# nexus-core list-configs  # v3.7.0 में उपलब्ध नहीं
```
...
3. `~/.config/nexus-core/configs/` (उपयोगकर्ता कॉन्फ़िग डायरेक्टरी)
...
```bash
nexus-core create https://docs.react.dev/ --preset quick      # तेज़, सतही स्तर
nexus-core create https://docs.react.dev/ --preset standard   # संतुलित (डिफ़ॉल्ट)
nexus-core create https://docs.react.dev/ --preset comprehensive # गहन, विस्तृत
```

### लाइफ़साइकिल फ्लैग्स
```bash
nexus-core create https://docs.react.dev/ --dry-run    # स्क्रैपिंग के बिना पूर्वावलोकन
nexus-core create https://docs.react.dev/ --fresh      # कैश को अनदेखा करें, पूर्ण पुनः स्क्रैप
nexus-core create https://docs.react.dev/ --resume     # रुके हुए कार्य को फिर से शुरू करें
nexus-core create https://docs.react.dev/ --skip-scrape # मौजूदा आउटपुट को पुनः पैकेज करें
```

### स्वास्थ्य जाँच और उपयोगिताएँ
```bash
nexus-core doctor                    # इंस्टॉलेशन और परिवेश का निदान करें
nexus-core sync-config               # कॉन्फ़िगरेशन ड्रिफ्ट का पता लगाएँ
nexus-core stream <source>           # बड़े दस्तावेज़ों के लिए स्ट्रीमिंग इनजेस्टन
nexus-core update output/react/      # वृद्धिशील अपडेट
nexus-core multilang <source>        # बहुभाषी कौशल जनरेशन
nexus-core quality output/react/     # गुणवत्ता रिपोर्ट (गेट के लिए --threshold 7 जोड़ें: 7/10 से नीचे गैर-शून्य निकास कोड)
```
...
|----------|---------|---------|
| `browser` | `pip install "nexus-core[browser]"` | SPA साइटों के लिए हेडलेस Playwright |
| `embedding` | `pip install "nexus-core[embedding]"` | एम्बेडिंग सर्वर समर्थन |
| `s3` / `gcs` / `azure` | `pip install "nexus-core[s3]"` आदि | क्लाउड स्टोरेज अपलोड |
| `rag-upload` | `pip install "nexus-core[rag-upload]"` | संयुक्त वेक्टर DB अपलोड डिपेंडेंसी |
...
- **[docs/UML/nexus_core.mdj](docs/UML/nexus_core.mdj)** - StarUML प्रोजेक्ट फ़ाइल ([StarUML](https://staruml.io/) से खोलें)
...
[![MseeP.ai सुरक्षा मूल्यांकन बैज](https://mseep.net/pr/yusufkaraaslan-nexus-core-badge.png)](https://mseep.ai/app/yusufkaraaslan-nexus-core)
...
[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=nexus_core) — एक फुल-मोडल, OpenAI-संगत AI इन्फ़रेंस प्लेटफ़ॉर्म। Nexus Core इसे `ATLAS_API_KEY` के साथ `--target atlas` के माध्यम से पैकेजिंग/एन्हांसमेंट लक्ष्य के रूप में समर्थन करता है।
