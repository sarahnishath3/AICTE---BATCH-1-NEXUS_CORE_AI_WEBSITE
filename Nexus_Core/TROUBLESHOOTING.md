...
**Error:**
```
FileNotFoundError: [Errno 2] No such file or directory: 'src/nexus_core/cli/main.py'
```

**Solutions:**
1. **Check you're in the Nexus_Core directory:**
   ```bash
   pwd
   # Should show: .../Nexus_Core
...
2. **Change to the correct directory:**
   ```bash
   cd ~/Projects/Nexus_Core  # Adjust path
   ```
...
1. **Place config in user directory (recommended for custom configs):**
   ...
   cp myconfig.json ~/.config/nexus-core/configs/

   # Now you can use it from anywhere
   nexus-core create --config myconfig.json
   ```
...
2. **Place config in current directory (project-specific):**
   ...
   nexus-core create --config configs/myconfig.json
   ```

3. **Use absolute path:**
   ```bash
   nexus-core create --config /full/path/to/myconfig.json
   ```
...
4. **Check if it's a preset config (auto-downloads):**
   ...
   nexus-core create --config react.json
   ```
...
2. **Verify paths are ABSOLUTE (not placeholders):**
   ...
   ```
   ❌ **Bad:** `$REPO_PATH` or `/path/to/Nexus_Core`
   ✅ **Good:** `/Users/john/Projects/Nexus_Core`
...
2. **Use smaller max_pages for testing:**
   ```bash
   nexus-core create --config configs/test.json --max-pages 5
   ```
...
ls src/nexus_core/cli/
ls src/nexus_core/mcp/
ls configs/
...
# 6. Test MCP server
timeout 3 python -m nexus_core.mcp.server_fastmcp || echo "Server OK"
...
3. **Include this debug info:**
   ...
   # Skill Seeker info
   cd ~/Projects/Nexus_Core  # Your path
   pwd
   git log --oneline -1
...
- [ ] In the Nexus_Core directory? (`pwd`)
- [ ] Python 3.10+ installed? (`python3 --version`)
- [ ] Package installed? (`pip list | grep nexus-core`)
...
**Still stuck?** Open an issue: https://github.com/yusufkaraaslan/Nexus_Core/issues/new
